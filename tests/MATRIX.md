...
| --- | --- | --- |
| Core portable unit + nested suites | `npm test -w @kaicreator/domain-harness` (recursive `tests/**/*.test.ts` + `tsconfig.test.json` typecheck) | 46 files / 243 tests, incl. `tests/discovery/nested/sentinel.test.ts` proving recursion and `tests/architecture-v03/deepseek-harness-mini-core.test.ts` covering the #185 research mini core |

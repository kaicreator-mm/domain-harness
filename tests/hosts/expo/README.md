...
4. **Failure Handling** — rejected message errors are normalized only at the host observation boundary; deterministic settle uses Query + Subscription with a timeout guard; restart validation requires a real OS force-stop/relaunch.
5. **Reference** — Frozen PRD AC-42, L2 AD-10, G4/G30, and Issue #132.

The adapter never imports `tests/conformance/reference-host.ts`.
...
Then install the standalone Expo validation app dependencies and start Android:
...
2. Tap **Prepare restart journey**. Wait for `DOMAIN_HARNESS_T019_RESTART_PREPARE_PREPARED`.
...
Fast Refresh, Metro reload, Jest, Node, or an in-memory/mock SQLite implementation is not valid restart evidence.

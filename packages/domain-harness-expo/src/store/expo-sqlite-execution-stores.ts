...
/**
 * T-023 Expo SQLite adapters for the three durable-execution ports owned by
 * the v0.3 authority wave:
 *
 * - `ExpoSqliteExactSemanticCacheStore`   (T-013 exact semantic cache)
 * - `ExpoSqliteHarnessExecutionJournalStore` (T-016 harness execution journal)
 * - `ExpoSqliteAdmissionEffectJournal`    (T-019 admission effect journal)
 *
 * Each adapter mirrors the T-022 Node SQLite adapter — and through it the
 * volatile reference store — exactly: same key functions, same validation,
 * same conflict messages, same first-writer-wins and idempotent-settlement
 * semantics, backed by the versioned `dh_v3_*` tables from migration 2. All
 * multi-step mutations run inside a single exclusive transaction serialized by
 * `ExclusiveTransactionQueue`; stored bodies are canonical JSON so byte
 ...
 */
export class ExpoSqliteExactSemanticCacheStore<TResult extends JsonValue = JsonValue> implements ExactSemanticCacheStore<TResult> {
  ...
  public constructor(database: ExpoSqliteExecutorLike, writes: ExclusiveTransactionQueue, retention: VolatileSemanticCacheRetentionPolicy = {}) {
  ...
  async read(key: SemanticCacheKey, nowEpochMs: number): Promise<SemanticCacheRead<TResult>> {
    ...
    const row = await this.#database.getFirstAsync<SemanticCacheEntryRow>(`
      SELECT entry_json FROM dh_v3_semantic_cache_entries WHERE storage_key = ?
    `, [id]);
    if (row === null) return { status: 'miss', reason: 'not-found' };
    ...
    if (this.isExpired(entry, nowEpochMs)) {
      await this.#writes.run(async (transaction) => {
        await this.removeWithinTransaction(transaction, id);
      });
  ...
  async putIfAbsent(entry: SemanticCacheEntry<TResult>, nowEpochMs: number): Promise<SemanticCachePut<TResult>> {
    ...
    return this.#writes.run(async (transaction) => {
      const existing = await transaction.getFirstAsync<SemanticCacheEntryRow>(`
        SELECT entry_json FROM dh_v3_semantic_cache_entries WHERE storage_key = ?
      `, [id]);
      if (existing !== null) {
      ...
      await this.evictCapacityForIncomingWithinTransaction(transaction);
      ...
      `, [id, namespace, producer, createdAt, expiresAt === undefined ? null : expiresAt, encoded]);
  ...
  async quarantine(key: SemanticCacheKey, reason: string, nowEpochMs: number): Promise<void> {
    ...
    await this.#writes.run(async (transaction) => {
      ...
      await transaction.runAsync(`
        ...
        VALUES (?, ?, ?)
      `, [id, nowEpochMs, encodedRecord]);
      const max = this.#retention.maxQuarantineRecords;
      if (max === undefined) return;
      const row = (await transaction.getFirstAsync<CountRow>(
  ...
  async invalidateByProducer(value: CompiledArtifactIdentity, _reason: string): Promise<number> {
    ...
    return this.#writes.run(async (transaction) => {
      const rows = await transaction.getAllAsync<StorageKeyRow>(`
        SELECT storage_key FROM dh_v3_semantic_cache_entries
        WHERE producer_key = ? ORDER BY storage_key ASC
      `, [producer]);
      let count = 0;
      for (const row of rows) if (await this.removeWithinTransaction(transaction, row.storage_key)) count += 1;
      return count;
    });
  }
  ...
  async evict(nowEpochMs: number): Promise<SemanticCacheEvictionReport> {
    epoch(nowEpochMs, 'nowEpochMs');
    return this.#writes.run(async (transaction) => ({
      expired: await this.evictExpiredWithinTransaction(transaction, nowEpochMs),
      capacity: await this.evictCapacityWithinTransaction(transaction),
    }));
  }

  /** Inspection surface for tests/review tooling; not part of the cache port. */
  async getQuarantineRecords(): Promise<readonly SemanticCacheQuarantineRecord[]> {
  ...
  private async removeWithinTransaction(transaction: ExpoSqliteExecutorLike, id: string): Promise<boolean> {
    const existing = await transaction.getFirstAsync<unknown>(
      'SELECT 1 FROM dh_v3_semantic_cache_entries WHERE storage_key = ?',
      [id],
    );
    if (existing === null) return false;
    await transaction.runAsync('DELETE FROM dh_v3_semantic_cache_dependencies WHERE storage_key = ?', [id]);
    await transaction.runAsync('DELETE FROM dh_v3_semantic_cache_entries WHERE storage_key = ?', [id]);
    return true;
  }

  /** Caller must already be inside a transaction; iterates storage keys sorted. */
  private async evictExpiredWithinTransaction(transaction: ExpoSqliteExecutorLike, now: number): Promise<number> {
    const rows = await transaction.getAllAsync<SemanticCacheClockRow>(`
    ...
    for (const row of rows) {
      ...
      if (expired && (await this.removeWithinTransaction(transaction, row.storage_key))) count += 1;
    }
    return count;
  }

  /** Caller must already be inside a transaction; used by evict() only. */
  private async evictCapacityWithinTransaction(transaction: ExpoSqliteExecutorLike): Promise<number> {
    ...
    return this.removeOldestWithinTransaction(transaction, row.count - max);
  }

  /** Caller must already be inside a transaction; used by putIfAbsent only. */
  private async evictCapacityForIncomingWithinTransaction(transaction: ExpoSqliteExecutorLike): Promise<number> {
    const max = this.#retention.maxEntries;
    if (max === undefined) return 0;
    const row = (await transaction.getFirstAsync<CountRow>(
      'SELECT COUNT(*) AS count FROM dh_v3_semantic_cache_entries',
    ...
    if (row.count < max) return 0;
    return this.removeOldestWithinTransaction(transaction, row.count - max + 1);
  }

  /** Caller must already be inside a transaction; oldest by (createdAt, storage_key). */
  private async removeOldestWithinTransaction(transaction: ExpoSqliteExecutorLike, count: number): Promise<number> {
    if (count <= 0) return 0;
    const rows = await transaction.getAllAsync<StorageKeyRow>(`
      SELECT storage_key FROM dh_v3_semantic_cache_entries
    ...
    `, [count]);
    let removed = 0;
    for (const row of rows) if (await this.removeWithinTransaction(transaction, row.storage_key)) removed += 1;
    return removed;
  }

  /** Outside-transaction expired sweep (mirrors the volatile evictExpired step). */
  private async evictExpired(now: number): Promise<number> {
    return this.#writes.run(async (transaction) => this.evictExpiredWithinTransaction(transaction, now));
  }
}
 ...
 */
export class ExpoSqliteHarnessExecutionJournalStore implements HarnessExecutionJournalStore {
  readonly #database: ExpoSqliteExecutorLike;
  readonly #writes: ExclusiveTransactionQueue;
  ...
  async read(slot: HarnessExecutionSlotIdentity): Promise<HarnessExecutionJournalRecord | null> {
    ...
    const row = await this.#database.getFirstAsync<JournalRecordRow>(`
  ...
  async begin(identity: HarnessExecutionOperationIdentity): Promise<{
    ...
    return this.#writes.run(async (transaction) => {
      const existing = await transaction.getFirstAsync<JournalRecordRow>(`
        SELECT record_json FROM dh_v3_harness_execution_journal WHERE slot_key = ?
      `, [key]);
      if (existing !== null) {
        return {
      ...
      await transaction.runAsync(`
        ...
        VALUES (?, ?, ?)
      `, [key, started.state, canonicalText(started as unknown as JsonValue, 'harness execution journal record')]);
  ...
  async commit(
    ...
    const key = slotKey(identity.slot);
    return this.#writes.run(async (transaction) => {
      const row = await transaction.getFirstAsync<JournalRecordRow>(`
        SELECT record_json FROM dh_v3_harness_execution_journal WHERE slot_key = ?
      `, [key]);
      if (row === null) throw new Error('cannot commit a Harness operation that was never begun');
      ...
      await transaction.runAsync(`
        ...
        WHERE slot_key = ?
      `, [committed.state, canonicalText(committed as unknown as JsonValue, 'harness execution journal record'), key]);
      return committed;
    });
  }

  /** Inspection surface for tests/review tooling; not part of the port. */
  async getRecords(): Promise<readonly HarnessExecutionJournalRecord[]> {
    const rows = await this.#database.getAllAsync<JournalRecordRow>(`
 ...
 */
export class ExpoSqliteAdmissionEffectJournal implements AdmissionDurableEffectJournal {
  readonly #database: ExpoSqliteExecutorLike;
  readonly #writes: ExclusiveTransactionQueue;
  ...
  async getEffect(effectId: string): Promise<AdmissionEffectJournalRecord | null> {
    const row = await this.#database.getFirstAsync<EffectRecordRow>(`
  ...
  async beginEffect(record: AdmissionEffectJournalRecord): Promise<{
    ...
    return this.#writes.run(async (transaction) => {
      const existing = await transaction.getFirstAsync<EffectRecordRow>(`
        SELECT record_json FROM dh_v3_admission_effect_journal WHERE effect_id = ?
      `, [record.effectId]);
      if (existing !== null) {
      ...
      await transaction.runAsync(`
        ...
        VALUES (?, ?, ?)
      `, [record.effectId, record.status, encoded]);
  ...
  async completeEffect(
    ...
    return this.#writes.run(async (transaction) => {
      const row = await transaction.getFirstAsync<EffectRecordRow>(`
        SELECT record_json FROM dh_v3_admission_effect_journal WHERE effect_id = ?
      `, [effectId]);
      if (row === null) {
        throw new CentralAdmissionError(
      ...
      await transaction.runAsync(`
        ...
        WHERE effect_id = ?
      `, [completed.status, canonicalText(completed as unknown as JsonValue, 'admission effect journal record'), effectId]);
      return completed;
    });
  }

  /** Inspection surface for tests/review tooling; not part of the port. */
  async getRecords(): Promise<readonly AdmissionEffectJournalRecord[]> {
    const rows = await this.#database.getAllAsync<EffectRecordRow>(`

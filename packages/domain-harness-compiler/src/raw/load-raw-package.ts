...
function assertOnlyKeys(record: UnknownRecord, allowed: readonly string[], label: string): void {
  ...
  if (unexpected.length) throw new Error(`${label} has unsupported keys: ${unexpected.sort().join(', ')}`);
}
...
function optionalString(record: UnknownRecord, key: string, label: string): string | undefined {
  ...
  if (typeof value !== 'string' || value.length === 0) throw new Error(`${label}.${key} must be a non-empty string when present`);
  return value;
}

function assertContained(root: string, candidate: string, ref: string): void {
  const rel = relative(root, candidate);
  if (rel === '..' || rel.startsWith(`..${sep}`) || isAbsolute(rel)) throw new Error(`path escapes Raw Domain Package root: ${ref}`);
}
...
function parseRoute(value: unknown, label: string): RawRoute {
...
function parseEvent(value: unknown, label: string): { schemaPath?: string; routes: RawRoute[] } {
  if (Array.isArray(value)) {
    if (value.length === 0) throw new Error(`${label} route list must not be empty`);
    return { routes: value.map((route, index) => parseRoute(route, `${label}[${index}]`)) };
  }
  const raw = asRecord(value, label);
  if ('routes' in raw) {
    assertOnlyKeys(raw, ['schema', 'routes'], label);
    if (!Array.isArray(raw.routes) || raw.routes.length === 0) throw new Error(`${label}.routes must be a non-empty array`);
...
function parseInvoke(value: unknown, label: string): RawInvoke {
  const raw = asRecord(value, label);
  assertOnlyKeys(raw, ['skill', 'tool', 'script', 'expr', 'workflow', 'input', 'timeoutMs'], label);
  const declared = (['skill', 'tool', 'script', 'expr', 'workflow'] as const).filter((key) => raw[key] !== undefined);
  if (declared.length !== 1) throw new Error(`${label} must declare exactly one of skill/tool/script/expr/workflow`);
  const key = declared[0];
  if (!key) throw new Error(`${label} invoke kind is missing`);
  const common: Pick<RawInvoke, 'input' | 'timeoutMs'> = {};
  const input = optionalString(raw, 'input', label);
  if (input) common.input = input;
  if (raw.timeoutMs !== undefined) {
    if (!Number.isInteger(raw.timeoutMs) || (raw.timeoutMs as number) <= 0) throw new Error(`${label}.timeoutMs must be a positive integer`);
    common.timeoutMs = raw.timeoutMs as number;
  }
  const body = requiredString(raw, key, label);
  if (key === 'expr') return { kind: 'expr', expression: body, ...common };
...
async function loadJsonSchema(
  ...
  try {
    ...
    const schema = asRecord(parsed, logicalRef) as JsonSchema;
    ajv.compile(schema);
...
async function loadSkills(root: string, schemas: Map<string, JsonSchema>, ajv: Ajv2020, issues: string[]): Promise<Map<string, RawSkill>> {
  const skills = new Map<string, RawSkill>();
  ...
  for (const entry of entries) {
    ...
    try {
      ...
      const resources = [] as Array<{ path: string; content: string }>;
      for (const ref of resourceRefs as string[]) {
...
async function loadWorkflows(root: string, issues: string[]): Promise<Map<string, RawWorkflow>> {
  const workflows = new Map<string, RawWorkflow>();
  ...
  for (const entry of entries) {
    ...
    try {
      ...
      const states: Record<string, RawState> = {};
      for (const [stateId, stateValue] of Object.entries(statesRaw)) {
        ...
        if (stateRaw.final !== undefined && typeof stateRaw.final !== 'boolean') throw new Error(`workflow:${id}.${stateId}.final must be boolean when present`);
        const eventsRaw = stateRaw.on === undefined ? {} : asRecord(stateRaw.on, `workflow:${id}.${stateId}.on`);
        const events: Record<string, RawExternalEvent> = {};
        let done: RawRoute[] = [];
        let error: RawRoute[] = [];
        for (const [eventName, spec] of Object.entries(eventsRaw)) {
          const parsed = parseEvent(spec, `workflow:${id}.${stateId}.on.${eventName}`);
          if (eventName === 'done' || eventName === 'error') {
            if (parsed.schemaPath) issues.push(`workflow:${id}.${stateId}.on.${eventName} cannot declare an event payload schema`);
            if (eventName === 'done') done = parsed.routes;
            else error = parsed.routes;
          } else events[eventName] = parsed;
        }
        const invoke = stateRaw.invoke === undefined ? undefined : parseInvoke(stateRaw.invoke, `workflow:${id}.${stateId}.invoke`);
        if (invoke && error.length === 0) error = [{ target: 'failed' }];
        const effects = stateRaw.effects === undefined ? undefined : stateRaw.effects;
        if (effects !== undefined && !Array.isArray(effects)) throw new Error(`workflow:${id}.${stateId}.effects must be an array`);
        states[stateId] = {
          id: stateId,
...
export async function loadRawDomainPackage(options: LoadRawDomainPackageOptions): Promise<LoadedRawDomainPackage> {
  ...
  if (manifestRaw.schemaVersion !== '0.1') throw new RawPackageDefinitionError([`harness.yaml.schemaVersion must be '0.1'`]);
  const limits = asRecord(manifestRaw.limits, 'harness.yaml.limits');
  assertOnlyKeys(limits, ['maxSteps'], 'harness.yaml.limits');
  if (!Number.isInteger(limits.maxSteps) || (limits.maxSteps as number) <= 0) throw new RawPackageDefinitionError(['harness.yaml.limits.maxSteps must be a positive integer']);
  ...
  for (const workflow of workflows.values()) {
    ...
    for (const state of Object.values(workflow.states)) {
      ...
      if (state.invoke?.kind === 'expr' && state.invoke.expression) compileExpression(state.invoke.expression, `${workflow.id}.${state.id}.invoke.expr`, issues);
      for (const route of [...state.done, ...state.error, ...Object.values(state.events).flatMap((event) => event.routes)]) {
        if (route.when) compileExpression(route.when, `${workflow.id}.${state.id}.route.when`, issues);
      }
      ...
      if (!invoke) continue;
      if (invoke.kind === 'skill' && invoke.ref && !skills.has(invoke.ref)) issues.push(`${workflow.id}.${state.id}: referenced Skill '${invoke.ref}' does not exist or lacks a valid sidecar`);
      if (invoke.kind === 'tool' && invoke.ref && !options.registeredTools?.has(invoke.ref)) issues.push(`${workflow.id}.${state.id}: referenced Tool '${invoke.ref}' is not registered`);
      if (invoke.kind === 'script' && invoke.ref) {
        try {
          const scriptPath = await safeExistingPath(root, invoke.ref);
          if (!(await stat(scriptPath)).isFile()) throw new Error('not a file');
  ...
  if (issues.length) throw new RawPackageDefinitionError(issues);
  return {
    root,
    schemaVersion: '0.1',
    domainId: requiredString(manifestRaw, 'id', 'harness.yaml'),
    limits: { maxSteps: limits.maxSteps as number },
    workflows,
    skills,
    scripts,
    schemas,

...
function validateFallback(routes, label, issues) {
...
export function validateWorkflowStructure(workflow) {
    ...
    for (const [stateId, state] of Object.entries(workflow.states)) {
        if (!STATE_ID_PATTERN.test(stateId))
            issues.push(`${workflow.id}: invalid state id '${stateId}'`);
        ...
        if (state.invoke) {
            ...
            if (state.done.length || state.error.length)
    ...
    for (const route of allRoutes(workflow)) {
        if (!workflow.states[route.target])
            issues.push(`${workflow.id}: transition target '${route.target}' does not exist`);
    }

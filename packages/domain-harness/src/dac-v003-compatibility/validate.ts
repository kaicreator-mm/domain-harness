...
import type { RuntimeHostBindingRequirementRef } from '../dac-v003/contracts.js';
import {
  ...
  type DacV003CompatibilityValidationRequest,
  type DacV003PortRequirement,
  type DacV003RequirementClosureEntry,
...
import {
  ...
  registerMintedCompatibilityResult,
  registerMintedCompatibilityValidation,
...
function isUxSemanticRole(value: unknown): value is DacV003UxSemanticRole {
  return (
    typeof value === 'string' &&
    (DAC_V003_UX_SEMANTIC_ROLES as readonly string[]).indexOf(value) !== -1
  );
}
 ...
 * vocabulary.
 */
function capabilityConstraints(requirement: DacV003CapabilityRequirement): readonly string[] {
  return [
...
function portConstraints(requirement: DacV003PortRequirement): readonly string[] {
  return [
    ...(requirement.reference.contractProfileIdentity === undefined
      ? []
      : [`profile:${requirement.reference.contractProfileIdentity}`]),
...
function hostBindingConstraints(
  requirement: RuntimeHostBindingRequirementRef,
): readonly string[] {
...
interface ParsedRequest {
  readonly target: DacV003CompatibilityTargetRef | undefined;
  readonly targetProfileKey: string | undefined;
  readonly supportedTargetProfiles: readonly string[];
  readonly applicableConditions: ReadonlySet<string>;
  readonly uxRequirements: readonly {
  ...
  readonly evidence: readonly DacV003RequirementSatisfactionEvidence[];
}
...
function validateRequest(request: DacV003CompatibilityValidationRequest): ParsedRequest {
  if (!isRecord(request)) invalid('compatibility validation request must be an object');
  if (
    request.sha256 === null ||
  ...
  if (!isSelectedCompositionValidation(request.selectionValidation)) {
    throw new DacV003CompatibilityError(
      ...
      'the upstream selection evidence was not minted by the #306 composition intake; a genuine stage-3 exact-selection validation is mandatory and cannot be claimed by a forged object',
    );
  }
  ...
  refuteDacV003LifecycleBindingInput(
    ...
    ...(request.hostBindingRequirements ?? []),
    ...(request.capabilityRequirements ?? []),
    ...(request.portRequirements ?? []),
    ...(request.satisfactionEvidence ?? []),
  );

  let target: DacV003CompatibilityTargetRef | undefined;
  let targetProfileKey: string | undefined;
  if (request.compatibilityTarget !== undefined) {
    if (!isDacV003CompatibilityTargetRefValue(request.compatibilityTarget)) {
      invalid(
        'compatibilityTarget must be an adopted DAC v0.0.3 compatibility-target reference (P5)',
      );
    }
  ...
  const supportedTargetProfiles = [
    ...new Set(
      request.supportedTargetProfiles.map((key) => {
  ...
  if (
    ...
    throw new DacV003CompatibilityError(
      'UX_ROLE_IDENTITY_COLLAPSE',
      'DomainUXDefinitionRef and RuntimeInteractionContractRef carry the same identity tuple; the two semantic contracts must remain separately referrable',
    );
  }
  ...
  const coverage = request.interactionCoverage.map((role) => {
    if (!isUxSemanticRole(role)) invalid(`"${String(role)}" is not a UX semantic role`);
    return role;
  });
  ...
  const uxRequirements = (
    request.uxSemanticRoleRequirements as readonly unknown[]
  ).map((requirement): { readonly role: DacV003UxSemanticRole; readonly required: boolean } => {
    if (!isRecord(requirement)) invalid('each UX semantic-role requirement must be an object');
    ...
    expectDacV003UxRoleAnchor(
      requirement.role,
      requirement.anchor as DacV003UxRoleAnchor | undefined,
    );
    return { role: requirement.role, required: requirement.required };
  });

  const capabilityRequirements = requireArray(
    request.capabilityRequirements,
    isDacV003CapabilityRequirementValue,
    'capabilityRequirements',
  );
  const portRequirements = requireArray(
    request.portRequirements,
    isDacV003PortRequirementValue,
    'portRequirements',
  );
  const hostBindingRequirements = requireArray(
    request.hostBindingRequirements,
    isRuntimeHostBindingRequirementRef,
  ...
  const applicableConditions = new Set<string>(
    (request.applicableConditions ?? []).map((condition) => {
  ...
  const seenRequirementIdentities = new Set<string>();
  for (const requirementRef of [
    ...
    if (seenRequirementIdentities.has(identity)) {
      invalid(
        `requirement identity "${identity}" is declared more than once; the requirement set must be unambiguous`,
      );
    }
...
function canonicalValidationIdentityMaterial(input: {
  ...
  readonly ux: {
    ...
    readonly interactionContract: readonly string[];
  ...
  return JSON.stringify({
    ...
    evidence: sorted(
      input.evidence.map((e) => [e.identity, e.satisfies, e.provider, e.validForTargetProfile]),
    ),
    ux: {
 ...
 */
export async function validateDacV003Compatibility(
  request: DacV003CompatibilityValidationRequest,
  ...
  if (targetProfileKey === undefined) {
    ...
    findings.push(
      'missing required compatibility target: no explicit DAC v0.0.3 CompatibilityTargetRef was declared, and an ambient "current runtime" is never assumed (CROSS_LAYER_REFERENCES §8; conformance C43)',
    );
  } else if (!parsed.supportedTargetProfiles.includes(targetProfileKey)) {
    incompatible = true;
    findings.push(
      `explicit compatibility target "${targetProfileKey}" is declared but unsupported by this validator environment (conformance C44)`,
    );
  } else {
  ...
  try {
    ...
    structuralFailure = true;
    findings.push(
      'same immutable revision identity with different authoritative digests inside the evaluated closure: identity/integrity contradiction with no identity-preserving reconciliation (C40)',
    );
  }

  // --- Capability / Port / Host-Binding requirement-vs-satisfaction closure.
  const evidenceByRequirement = new Map<string, DacV003RequirementSatisfactionEvidence[]>();
  ...
  for (const evidence of parsed.evidence) {
    ...
    if (!declaredRequirementIdentities.has(identity)) {
      findings.push(
        `satisfaction evidence "${evidence.reference.primaryIdentity}" links requirement "${identity}" outside the declared closed requirement set; it is recorded but proves nothing here (§8.3)`,
      );
    }
  }

  const requirementClosure: DacV003RequirementClosureEntry[] = [];
  const evaluateRequirement = (
    kind: 'capability' | 'port' | 'host-binding',
    reference: { readonly primaryIdentity: string },
    strength: 'required' | 'optional' | 'conditional',
    condition: string | undefined,
    declaredConstraints: readonly string[],
  ): void => {
    let effectiveStrength: 'required' | 'optional' | 'not-applicable';
    ...
    const matching = (evidenceByRequirement.get(reference.primaryIdentity) ?? []).filter(
      (evidence) =>
        targetProfileKey !== undefined && evidence.validForTargetProfile === targetProfileKey,
    );
    requirementClosure.push(
      freeze({
        kind,
    ...
    if (effectiveStrength === 'required' && matching.length === 0) {
      ...
      findings.push(
        `required ${kind} requirement "${reference.primaryIdentity}" is unsatisfied: ${reason} (conformance C57)`,
      );
    }
  };
  ...
  for (const requirement of parsed.capabilityRequirements) {
    evaluateRequirement(
      ...
      capabilityConstraints(requirement),
    );
  }
  ...
  for (const requirement of parsed.hostBindingRequirements) {
    evaluateRequirement(
      ...
      'required',
      undefined,
      hostBindingConstraints(requirement),
    );
  }
  ...
  if (missingRequiredRoles.length > 0) {
    incompatible = true;
    findings.push(
      `the selected UX definition requires semantic roles not covered by the runtime interaction contract: ${missingRequiredRoles.join(', ')} (conformance C58)`,
    );
  }
  ...
  if (dispositionValue === 'COMPATIBLE' && !targetSupported) {
    throw new DacV003CompatibilityError(
      ...
      'internal closure guard: COMPATIBLE requires a supported explicit target (unreachable by construction)',
    );
  }
  ...
  }) as DacV003CompatibilityValidationRef;
  assertDacV003ExactnessProfile(validationRef, 'P6');

  const resultIdentity = `sha256:${await request.sha256.digestUtf8(
    JSON.stringify([validationIdentity, dispositionValue, 'dac-v003-compatibility-result/1']),
  )}`;
  const resultRef = adoptDacV003RegistryReference('compatibility-result', {
    ...
    provenanceRefs: [upstreamEvidence],
  }) as DacV003CompatibilityResultRef;
 ...
 */
export function deriveDacV003CompatibilityResult(
  ...
  if (
    ...
    throw new DacV003CompatibilityError(
      ...
      'result derivation requires a validation record actually minted by the DAC v0.0.3 compatibility authority',
    );
  }
  ...
  const result = freeze({
    result: 'dac-v003-compatibility-result/1' as const,

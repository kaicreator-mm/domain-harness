...
export const RUNTIME_CONTROL_CONTRACT_VERSION = 'runtime-control/1' as const;
...
export type RuntimeControlAuthorizationDecision =
  ...
  | { readonly status: 'UNKNOWN'; readonly code: string; readonly reason?: string };
...
export type RuntimeControlOutcome =
  ...
  | 'INTERRUPTED_TO_RECOVERY_REQUIRED'
  | 'ALREADY_TERMINAL'
  | 'TOO_LATE_TURN_COMMITTED'
...
export const RUNTIME_CONTROL_PROVENANCE_KIND = 'runtime-control' as const;
...
export function runtimeControlProvenanceOf(
  instance: WorkflowInstanceSnapshot,
): RuntimeControlProvenance | null {
  ...
  if (
    ...
    return details as RuntimeControlProvenance;
  }
  return null;
}

/** Terminalize reason JSON carrying exact control provenance (stored as failure evidence). */
export function runtimeControlTerminalizeReason(
  provenance: RuntimeControlProvenance,
): JsonValue {
  return {

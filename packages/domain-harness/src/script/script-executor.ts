        ...
        finish(() => {
          ...
          if (!response.ok || typeof response.outputJson !== 'string') {
            rejectPromise(
              new ScriptExecutorError(
                'script_error',
                response.message ?? 'Script Worker returned an invalid response',
              ),
            );
            return;
          }
          try {
            resolvePromise(JSON.parse(response.outputJson) as JsonValue);
          } catch (error) {
            rejectPromise(
              new ScriptExecutorError(
                'script_error',
                `Script Worker returned invalid JSON: ${error instanceof Error ? error.message : String(error)}`,
              ),
            );
          }
        });
        ...
        if (!settled) {
          finish(() =>
            rejectPromise(
              new ScriptExecutorError(
                'script_error',
                `Script Worker exited before returning a result with code ${code}`,
              ),
            ),
          );
        }
...
async function readScriptInsideRoot(harnessRoot: string, scriptRef: string): Promise<string> {
  ...
  try {
    ...
    throw new ScriptExecutorError(
      'script_error',
      `Harness root cannot be resolved: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
  ...
  try {
    ...
    if (error instanceof ScriptExecutorError) throw error;
    throw new ScriptExecutorError(
      'script_error',
      `Script cannot be loaded '${scriptRef}': ${error instanceof Error ? error.message : String(error)}`,
    );
  }
}
...
function serializeInput(value: JsonValue): string {
  try {
    ...
    throw new ScriptExecutorError(
      'script_error',
      `Script input is not JSON-serializable: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
}

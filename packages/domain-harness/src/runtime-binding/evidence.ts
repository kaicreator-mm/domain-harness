...
import {
  ...
  type RuntimeBindingOptions,
} from './contracts.js';

function requireSha256Port(options: unknown, stage: string): void {
  if (options === null || typeof options !== 'object') {
    throw new RuntimeBindingError(
      'INVALID_RUNTIME_BINDING_INPUT',
      `${stage} options must be an object`,
    );
  }
  const candidate = options as { sha256?: unknown };
  if (
    candidate.sha256 === null ||
    typeof candidate.sha256 !== 'object' ||
    typeof (candidate.sha256 as { digestUtf8?: unknown }).digestUtf8 !== 'function'
  ) {
    throw new RuntimeBindingError(
      'INVALID_RUNTIME_BINDING_INPUT',
      `${stage} options must declare a sha256 port (digestUtf8) for exact evidence identity derivation`,
    );
  }
}
 ...
 */
function canonicalBindingMaterial(validation: SelectedCompositionValidation): string {
 ...
 */
function canonicalActivationMaterial(
 ...
 */
const MINTED_BINDINGS = new WeakSet<object>();

/** True only for binding evidence actually minted by `bindValidatedComposition`. */
export function isRuntimeBindingEvidence(value: unknown): value is RuntimeBindingEvidence {
  if (value === null || typeof value !== 'object' || !MINTED_BINDINGS.has(value)) {
    return false;
  }
  const candidate = value as Partial<RuntimeBindingEvidence>;
  return (
    candidate.binding === RUNTIME_BINDING_ADAPTER_VERSION &&
 ...
 * activation evidence.
 */
export async function bindValidatedComposition(
  validation: SelectedCompositionValidation,
  options: RuntimeBindingOptions,
): Promise<RuntimeBindingEvidence> {
  requireSha256Port(options, 'bind');
  if (!isSelectedCompositionValidation(validation)) {
    throw new RuntimeBindingError(
      'NOT_A_VALIDATED_COMPOSITION',
      'binding input is not a verdict minted by the composition intake; compatibility PASS ' +
  ...
  const evidence: RuntimeBindingEvidence = Object.freeze({
...
export async function activateRuntimeBinding(
  ...
  const instanceId = (options as { activationInstanceId?: unknown }).activationInstanceId;
  if (typeof instanceId !== 'string' || instanceId.trim().length === 0) {
    throw new RuntimeBindingError(
      'INVALID_RUNTIME_BINDING_INPUT',
      'activation options must declare a non-empty exact activationInstanceId',
    );
  }
  if (!isRuntimeBindingEvidence(binding)) {
    throw new RuntimeBindingError(
      'NOT_A_RUNTIME_BINDING',
      'activation input is not binding evidence minted by the runtime-binding module; a verdict, ' +

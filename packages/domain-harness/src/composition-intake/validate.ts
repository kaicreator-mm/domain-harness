 ...
 */
const MINTED_VALIDATIONS = new WeakSet<object>();

/** True only for verdicts actually minted by `validateSelectedComposition`. */
export function isSelectedCompositionValidation(
  value: unknown,
): value is SelectedCompositionValidation {
  if (value === null || typeof value !== 'object' || !MINTED_VALIDATIONS.has(value)) {
    return false;
  }
  const candidate = value as Partial<SelectedCompositionValidation>;
  return (
    candidate.intake === COMPOSITION_INTAKE_ADAPTER_VERSION &&
    ...
    isPromotionDecisionRef((candidate.provenance as { promotionDecision?: unknown }).promotionDecision) &&
    isApplicationSelectionRef((candidate.provenance as { applicationSelection?: unknown }).applicationSelection) &&
    candidate.declared !== null &&
    typeof candidate.declared === 'object' &&
    isRuntimeContractRef((candidate.declared as { runtimeContract?: unknown }).runtimeContract) &&
    isRuntimeImplementationRef((candidate.declared as { runtimeImplementation?: unknown }).runtimeImplementation) &&

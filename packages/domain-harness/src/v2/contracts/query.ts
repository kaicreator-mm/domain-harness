...
export type DomainQuery =
  ...
  | { kind: 'package-pins' }
  | { kind: 'projection'; projectionId: string; key: string; input?: JsonValue };

export type DomainQueryResult =
  | { kind: 'instance'; value: WorkflowInstanceSnapshot | null }
  | { kind: 'message-disposition'; value: MessageDispositionSnapshot | null }
  | { kind: 'runtime-failure'; value: RuntimeFailure | null }
  | { kind: 'package-pins'; value: readonly string[] }

...
export const AMBIGUOUS_NON_IDEMPOTENT_EFFECT_FAILURE_CODE =
  'ambiguous_non_idempotent_effect' as const;

export type RecoveryStore = Pick<
  RuntimeStore,
  | 'getInstance'
  | 'getMessageDisposition'
  | 'getNextAcceptedMessage'
  | 'failMessageProcessing'
  | 'terminalizeInstance'
  | 'resetRecovery'
>;
...
export type RecoveryRetryAuthorization =
  | DomainPolicyRetryAuthorization
  | AmbiguousNonIdempotentRetryAuthorization;
...
export type TerminalizeWorkflowRequest =
  | NormalTerminalizeRequest
  | RecoveryTerminalizeRequest;

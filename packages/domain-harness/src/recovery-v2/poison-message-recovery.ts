...
import {
  ...
  type RecordProcessingFailureRequest,
  type RecordToolRecoveryRequiredRequest,
  type RecoveryInspection,
  type RecoveryRetryResult,
  type RecoveryStore,
  type RetryRecoveryRequest,
  type TerminalizeWorkflowRequest,
} from './contracts.js';
import {
  AmbiguousNonIdempotentResolutionRequiredError,
  RecoveryMessageNotFoundError,
  RecoveryRetryNotAuthorizedError,
  RecoveryStateError,
  RecoveryStoreInvariantError,
  RecoveryTargetNotFoundError,
} from './errors.js';
...
export class PoisonMessageRecoveryCoordinator {
  ...
  constructor(
    ...
    options: PoisonMessageRecoveryCoordinatorOptions = {},
  ) {
    this.#now = options.now ?? (() => new Date().toISOString());
  }

  async inspect(target: WorkflowAddress, messageId: string): Promise<RecoveryInspection> {
  ...
  async nextProcessableMessage(target: WorkflowAddress): Promise<StoredAcceptedMessage | null> {
    ...
    if (next !== null && !sameAddress(next.message.target, target)) {
      throw new RecoveryStoreInvariantError(
        `RuntimeStore returned a message for another target while reading ${formatTarget(target)}`,
      );
    }
    return next;
  }
  ...
  async recordProcessingFailure(
    request: RecordProcessingFailureRequest,
    ...
    if (current.lifecycle === 'recovery_required') {
      ...
      throw new RecoveryStateError(
        `Workflow target ${formatTarget(request.target)} already requires recovery for another unresolved message`,
      );
    }
    if (isTerminal(current.lifecycle)) {
      throw new RecoveryStateError(
        `Cannot record processing failure for terminal workflow ${formatTarget(request.target)} (${current.lifecycle})`,
      );
    }
    if (disposition.disposition !== 'accepted' && disposition.disposition !== 'processing') {
      throw new RecoveryStateError(
        `Message ${request.messageId} cannot become poison from disposition ${disposition.disposition}`,
      );
    }
    ...
    if (result.instance.lifecycle !== 'recovery_required') {
      throw new RecoveryStoreInvariantError(
        `RuntimeStore did not enter recovery_required after message ${request.messageId} failed`,
      );
    }
    if (result.message.disposition !== 'failed') {
      throw new RecoveryStoreInvariantError(
        `RuntimeStore did not persist failed disposition for poison message ${request.messageId}`,
      );
    }
    return result;
  }
  ...
  recordToolRecoveryRequired(
    request: RecordToolRecoveryRequiredRequest,
  ): Promise<RecoveryInspection> {
  ...
  async retry(request: RetryRecoveryRequest): Promise<RecoveryRetryResult> {
    const inspection = await this.inspect(request.target, request.messageId);
    if (inspection.instance.lifecycle !== 'recovery_required') {
      throw new RecoveryStateError(
        `Workflow target ${formatTarget(request.target)} is ${inspection.instance.lifecycle}, not recovery_required`,
      );
    }
    if (inspection.message.disposition !== 'failed') {
      throw new RecoveryStateError(
        `Recovery retry requires failed poison message ${request.messageId}; found ${inspection.message.disposition}`,
      );
    }
    ...
    if (reset.lifecycle !== 'active' && reset.lifecycle !== 'waiting') {
      throw new RecoveryStoreInvariantError(
        `RuntimeStore resetRecovery returned non-accepting lifecycle ${reset.lifecycle}`,
      );
    }
    ...
    if (message.disposition !== 'accepted') {
      throw new RecoveryStoreInvariantError(
        `RuntimeStore resetRecovery did not restore poison message ${request.messageId} to accepted; found ${message.disposition}`,
      );
    }
  ...
  async terminalize(request: TerminalizeWorkflowRequest): Promise<WorkflowInstanceSnapshot> {
    const current = await this.#requireInstance(request.target);

    if (request.mode === 'normal') {
      if (current.lifecycle === 'recovery_required') {
        throw new RecoveryStateError(
          `Recovery-required workflow ${formatTarget(request.target)} needs an explicit recovery terminalization`,
        );
      }
      if (isTerminal(current.lifecycle)) {
        if (current.lifecycle === request.lifecycle) return current;
        throw new RecoveryStateError(
          `Workflow ${formatTarget(request.target)} is already terminal as ${current.lifecycle}`,
        );
      }
    } else {
      if (current.lifecycle !== 'recovery_required') {
        throw new RecoveryStateError(
          `Recovery terminalization requires recovery_required; ${formatTarget(request.target)} is ${current.lifecycle}`,
        );
      }
      if (request.authorization.reason.trim().length === 0) {
        throw new RecoveryRetryNotAuthorizedError(
          'Recovery terminalization requires a non-empty domain authorization reason',
        );
      }
    }
    ...
    if (terminal.lifecycle !== request.lifecycle) {
      throw new RecoveryStoreInvariantError(
        `RuntimeStore terminalized ${formatTarget(request.target)} as ${terminal.lifecycle}; expected ${request.lifecycle}`,
      );
    }
    return terminal;
  }

  async #requireInstance(target: WorkflowAddress): Promise<WorkflowInstanceSnapshot> {
    const instance = await this.store.getInstance(target);
    if (instance === null) throw new RecoveryTargetNotFoundError(target);
    if (!sameAddress(instance.address, target)) {
      throw new RecoveryStoreInvariantError(
        `RuntimeStore returned ${formatTarget(instance.address)} for requested ${formatTarget(target)}`,
      );
    }
    return instance;
  }

  async #requireDisposition(target: WorkflowAddress, messageId: string) {
    const disposition = await this.store.getMessageDisposition(target, messageId);
    if (disposition === null) throw new RecoveryMessageNotFoundError(target, messageId);
    if (!sameAddress(disposition.target, target) || disposition.messageId !== messageId) {
      throw new RecoveryStoreInvariantError(
        `RuntimeStore returned mismatched disposition while reading ${messageId} for ${formatTarget(target)}`,
      );
    }
    return disposition;
  }

  #assertRetryAuthorization(
    failure: RuntimeFailure | undefined,
    authorization: RetryRecoveryRequest['authorization'],
  ): void {
    if (authorization.reason.trim().length === 0) {
      throw new RecoveryRetryNotAuthorizedError(
        'Recovery retry requires a non-empty authorization reason',
      );
    }
    ...
    if (failure?.code !== AMBIGUOUS_NON_IDEMPOTENT_EFFECT_FAILURE_CODE) {
      if (authorization.kind !== 'domain-policy') {
        throw new RecoveryRetryNotAuthorizedError(
          'Ordinary poison-message retry requires domain-policy authorization',
        );
      }
      return;
    }
    ...
    if (!effectId) {
      throw new RecoveryStoreInvariantError(
        'Ambiguous non-idempotent recovery failure is missing effectId',
      );
    }
    if (
      authorization.kind !== 'ambiguous-non-idempotent-resolved' ||
...
function normalizeFailure(failure: RuntimeFailure, messageId: string): RuntimeFailure {
  ...
  if (failure.sourceMessageId !== undefined && failure.sourceMessageId !== messageId) {
    throw new RecoveryStateError(
      `Runtime failure sourceMessageId ${failure.sourceMessageId} does not match poison message ${messageId}`,
    );
  }
  return {
    ...failure,
...
function failureToJson(failure: RuntimeFailure): JsonObject {
  return {
    code: failure.code,
...
function assertSequence(actual: number, expected: number, messageId: string): void {
  if (actual !== expected) {
    throw new RecoveryStateError(
      `Message ${messageId} has target sequence ${actual}; expected ${expected}`,
    );
  }
}

// Issue #310 / A2 I-007 core: consuming an adopted PROVISIONAL Application
// Manifest as already-selected composition metadata. The composition path is
// deliberately a thin exact-consumption wrapper over the #306 intake:
//
//   adopted manifest + caller-stated exact selected entry identity
//     -> SelectedCompositionRequest built ONLY from manifest-declared refs
//     -> #306 validateSelectedComposition (the compatibility authority)
//     -> separate ManifestCompositionEvidence referencing the exact
//        manifest identity/digest (never stored back into the manifest)
//
// The manifest's entry order/defaults never choose a revision (L2 A2 §7.2):
// the caller must state the full exact identity triple and a miss fails
// closed. No step selects, substitutes, binds or activates; binding and
// activation correlation records are minted ONLY from the exact #307
// evidence objects and stay separate from the manifest definition (C38/N18).
import type {
  ApplicationManifestIdentity,
  ManifestCompositionEvidence,
  ManifestCompositionRequest,
  ManifestExactSelectedIdentity,
...
import {
  isRecord,
  mintActivationCorrelation,
  mintBindingCorrelation,
  mintCompositionEvidence,
} from './registry.js';
import { validateSelectedComposition } from '../composition-intake/validate.js';
import type { SelectedCompositionRequest } from '../composition-intake/contracts.js';
...
} from '../runtime-binding/contracts.js';
import { isRuntimeActivationRef } from '../dac/guards.js';

function requireExactSelectedIdentity(value: unknown): ManifestExactSelectedIdentity {
  if (!isRecord(value)) {
    throw new ApplicationManifestError(
      'INVALID_MANIFEST_COMPOSITION_REQUEST',
      'composition request must state the exact selected entry identity (semanticIdentity, revisionIdentity, contentDigest) being consumed; the manifest never chooses by order or default',
    );
  }
  for (const field of ['semanticIdentity', 'revisionIdentity', 'contentDigest'] as const) {
    const fieldValue = value[field];
    if (typeof fieldValue !== 'string' || fieldValue.length === 0) {
      throw new ApplicationManifestError(
        'INVALID_MANIFEST_COMPOSITION_REQUEST',
        `exactSelected.${field} must be a non-empty string; a partial identity can never resolve an exact selected entry`,
      );
    }
  }
  return value as unknown as ManifestExactSelectedIdentity;
}
 ...
 */
export async function composeSelectedApplicationManifest(
  request: ManifestCompositionRequest,
): Promise<ManifestCompositionEvidence> {
  if (!isRecord(request)) {
    throw new ApplicationManifestError(
      'INVALID_MANIFEST_COMPOSITION_REQUEST',
      'composition request must be an object',
    );
  }
  ...
  if (!isApplicationManifest(manifest)) {
    throw new ApplicationManifestError(
      ...
      'composition input is not a manifest adopted by the application-manifest adapter; a foreign/forged manifest object can never be composed',
    );
  }
  ...
  const entry = manifest.selectedDomainData.find(
    (candidate) =>
      candidate.selected.semanticIdentity === exactSelected.semanticIdentity &&
  ...
  if (entry === undefined) {
    throw new ApplicationManifestError(
      ...
      'the exact stated selected Domain Data identity is not carried by this manifest; the manifest never chooses a revision by order/defaults and no substitution is attempted',
      [
        `requested=${exactSelected.semanticIdentity}/${exactSelected.revisionIdentity}/${exactSelected.contentDigest}`,
        ...manifest.selectedDomainData.map(
          (candidate, index) =>
            `available[${index}]=${candidate.selected.semanticIdentity}/${candidate.selected.revisionIdentity}/${candidate.selected.contentDigest}`,
        ),
      ],
    );
  }
  ...
  const intakeRequest: SelectedCompositionRequest = {
  ...
  const provided = new Set<string>(validation.compatibility.providedCapabilities);
  const missing = manifest.requiredCapabilities.filter(
    (capability) => !provided.has(capability as string),
  );
  if (missing.length > 0) {
    throw new ApplicationManifestError(
      'CAPABILITY_DECLARATION_UNSATISFIED',
      'the manifest declares required capabilities the validated compatibility target does not provide; a declaration is a requirement, not proof of availability, and no substitution is attempted',
      missing,
    );
  }

  const manifestIdentity: ApplicationManifestIdentity = Object.freeze({
  ...
  return mintCompositionEvidence(
    Object.freeze({
 ...
 */
export function correlateManifestRuntimeBinding(
  evidence: ManifestCompositionEvidence,
  binding: RuntimeBindingEvidence,
): ManifestRuntimeBindingCorrelation {
  if (!isManifestCompositionEvidence(evidence)) {
    throw new ApplicationManifestError(
      'NOT_A_MANIFEST_COMPOSITION',
      'binding correlation input is not manifest composition evidence minted by the application-manifest adapter',
    );
  }
  if (!isRuntimeBindingEvidence(binding)) {
    throw new ApplicationManifestError(
      'NOT_A_RUNTIME_BINDING_EVIDENCE',
      'binding correlation input is not #307 runtime binding evidence; a verdict, a bare reference, a selection or a forged lookalike can never stand in for a genuine binding',
    );
  }
  if (binding.validation !== evidence.validation) {
    throw new ApplicationManifestError(
      'CORRELATION_MISMATCH',
      'the binding evidence was minted from a different composition verdict object than the one this manifest composition produced; a correlation references only the exact evidence chain it was derived from',
    );
  }
  return mintBindingCorrelation(
    Object.freeze({
...
function isRuntimeActivationEvidence(value: unknown): value is RuntimeActivationEvidence {
  if (!isRecord(value)) return false;
  return (
    (value as { activation?: unknown }).activation === RUNTIME_ACTIVATION_ADAPTER_VERSION &&
    isRuntimeActivationRef((value as { activationRef?: unknown }).activationRef) &&
    isRuntimeBindingEvidence((value as { binding?: unknown }).binding)
  );
}
 ...
 */
export function correlateManifestRuntimeActivation(
  correlation: ManifestRuntimeBindingCorrelation,
  activation: RuntimeActivationEvidence,
): ManifestRuntimeActivationCorrelation {
  if (!isManifestRuntimeBindingCorrelation(correlation)) {
    throw new ApplicationManifestError(
      'NOT_A_MANIFEST_BINDING_CORRELATION',
      'activation correlation input is not a manifest binding correlation minted by the application-manifest adapter',
    );
  }
  if (!isRuntimeActivationEvidence(activation)) {
    throw new ApplicationManifestError(
      'NOT_A_RUNTIME_ACTIVATION_EVIDENCE',
      'activation correlation input is not #307 runtime activation evidence minted under a genuine binding',
    );
  }
  if (activation.binding !== correlation.binding) {
    throw new ApplicationManifestError(
      'CORRELATION_MISMATCH',
      'the activation evidence was minted under a different binding than the one this manifest correlation references; a correlation references only the exact evidence chain it was derived from',
    );
  }
  return mintActivationCorrelation(
    Object.freeze({

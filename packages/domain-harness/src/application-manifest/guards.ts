...
import {
  ...
  type ApplicationManifestDigestOptions,
  type ApplicationManifestIdentity,
  type ManifestCompositionEvidence,
  type ManifestExternalAuthorityDeclaration,
  type ManifestRuntimeActivationCorrelation,
  type ManifestRuntimeBindingCorrelation,
  type ManifestSelectedDomainDataEntry,
  type ManifestUxContractRole,
  type UxBridgeContractReference,
  type UxInteractionContractRequirement,
} from './contracts.js';
import {
  isApplicationManifestIdentity,
  isMutableAliasToken,
  isMintedActivationCorrelation,
  isMintedBindingCorrelation,
  isMintedCompositionEvidence,
  isMintedManifest,
  isRecord,
  mintManifest,
} from './registry.js';

const INSTANCE_STATE_FIELDS = new Set<string>(MANIFEST_INSTANCE_STATE_FIELD_VOCABULARY);
const UX_CONTRACT_ROLES = new Set<string>(MANIFEST_UX_CONTRACT_ROLES);
...
function rejectEvidenceAbsorption(value: unknown, where: string): void {
  ...
  if (
    isDacReference(value) &&
    (value.role === 'runtime-binding' || value.role === 'runtime-activation')
  ) {
    throw new ApplicationManifestError(
      'MANIFEST_EVIDENCE_ABSORPTION',
      `${where}: a "${value.role}" DAC reference was presented as manifest definition content; binding/activation identity is minted at stages 4/5 after compatibility validation and never inside the Manifest definition`,
    );
  }
}
  ...
  for (const key of Object.keys(area)) {
    if (INSTANCE_STATE_FIELDS.has(key)) {
      throw new ApplicationManifestError(
        ...
        `${where}: field "${key}" is live instance state; the Manifest definition must not absorb live Business/Process/Execution/UX instance facts`,
      );
    }
  }
}
...
function validateSelectedEntry(entry: unknown, index: number): ManifestSelectedDomainDataEntry {
  const where = `selectedDomainData[${index}]`;
  if (!isRecord(entry)) {
    throw new ApplicationManifestError(
      'INVALID_MANIFEST',
      `${where} must be an object carrying the exact selected identity plus promotion-decision and application-selection provenance`,
    );
  }
...
function validateUxRequirement(
  requirement: unknown,
  index: number,
): UxInteractionContractRequirement {
  const where = `uxContractRequirements[${index}]`;
  if (!isRecord(requirement)) {
    throw new ApplicationManifestError(
      'INVALID_MANIFEST',
      `${where} must be an object (UX interaction-contract requirement)`,
    );
  }
  ...
  if (!UX_CONTRACT_ROLES.has(contractRole)) {
    throw new ApplicationManifestError(
      'INVALID_MANIFEST',
      `${where}.contractRole "${contractRole}" is not one of the declared UX interaction-contract requirement roles`,
    );
  }
  ...
  if (
    ...
    throw new ApplicationManifestError(
      'INVALID_MANIFEST',
      `${where}.bridgeReference is not a reference minted by the #308 UX<->Runtime correlation bridge; a renderer/component-tree object is never a UX contract anchor`,
    );
  }
  const opaque = requireOpaqueRecord(requirement.opaque, `${where}.opaque`);
  screenOpaqueArea(opaque, `${where}.opaque`);
  const adopted: UxInteractionContractRequirement = {
    contractRole: contractRole as ManifestUxContractRole,
    semanticIdentity: requirement.semanticIdentity as string,
    ...(requirement.revisionIdentity === undefined
      ? {}
      : { revisionIdentity: requirement.revisionIdentity as string }),
    ...(requirement.bridgeReference === undefined
      ? {}
      : {
          bridgeReference:
            requirement.bridgeReference as unknown as UxBridgeContractReference,
        }),
...
function validateExternalAuthorityDeclaration(
  declaration: unknown,
  index: number,
): ManifestExternalAuthorityDeclaration {
  ...
  if (!isExternalAuthorityRef(authority)) {
    ...
    throw new ApplicationManifestError(
      ...
      `${where}.authority is not an ExternalAuthorityRef adopted by the #309 external-authority adapter; a provider URL/locator alone is not an authority contract and no Runtime identity can substitute external Business SoR identity`,
    );
  }
...
function canonicalManifestMaterial(
  manifest: Omit<ApplicationManifest, 'manifestContentDigest'>,
): string {
...
async function materializeManifest(input: unknown): Promise<{
  material: string;
  record: Omit<ApplicationManifest, 'manifestContentDigest'>;
}> {
  if (!isRecord(input)) {
    throw new ApplicationManifestError(
      'INVALID_MANIFEST',
  ...
  if (
    ...
    throw new ApplicationManifestError(
      ...
      `manifest baseline must be exactly ${DAC_REFERENCE_BASELINE.contract}@${DAC_REFERENCE_BASELINE.version} commit ${DAC_REFERENCE_BASELINE.baselineCommit} (the single frozen DAC baseline; no parallel manifest identity authority is forked)`,
    );
  }
  if (input.contractVersion !== APPLICATION_MANIFEST_CONTRACT_VERSION) {
    throw new ApplicationManifestError(
      'UNSUPPORTED_MANIFEST_CONTRACT_VERSION',
      `manifest contractVersion must be exactly "${APPLICATION_MANIFEST_CONTRACT_VERSION}"; got "${String(input.contractVersion)}"`,
    );
  }
  ...
  const manifestIdentity = requireExactImmutableIdentity(
  ...
  const selectedDomainData: readonly ManifestSelectedDomainDataEntry[] =
    input.selectedDomainData.map((entry: unknown, index: number) =>
      validateSelectedEntry(entry, index),
    );

  for (const field of ['runtimeContract', 'runtimeImplementation', 'compatibilityTarget'] as const) {
  ...
  const uxContractRequirements = Array.isArray(input.uxContractRequirements)
    ? input.uxContractRequirements.map((r: unknown, i: number) => validateUxRequirement(r, i))
    : [];
  const externalAuthorityDeclarations = Array.isArray(input.externalAuthorityDeclarations)
    ? input.externalAuthorityDeclarations.map((d: unknown, i: number) =>
        validateExternalAuthorityDeclaration(d, i),
      )
    : [];
  ...
  if (input.requiredCapabilities !== undefined) {
    if (!Array.isArray(input.requiredCapabilities)) {
      throw new ApplicationManifestError(
        'INVALID_MANIFEST',
        'manifest field "requiredCapabilities" must be an array of name@major capability ids',
      );
    }
    for (const capability of input.requiredCapabilities) {
      if (typeof capability !== 'string' || !CAPABILITY_ID_PATTERN.test(capability)) {
        throw new ApplicationManifestError(
          'INVALID_MANIFEST',
          `manifest required capability "${String(capability)}" is not a valid name@major capability id`,
        );
      }
    }
  }
  const requiredCapabilities: readonly CapabilityId[] =
    (input.requiredCapabilities as readonly CapabilityId[] | undefined) ?? [];

  const compositionProvenance = requireOpaqueRecord(
  ...
  const record: Omit<ApplicationManifest, 'manifestContentDigest'> = Object.freeze({
    adapter: APPLICATION_MANIFEST_ADAPTER_VERSION,
...
function requireDigestOptions(options: unknown): ApplicationManifestDigestOptions {
  if (
    !isRecord(options) ||
  ...
  return options as unknown as ApplicationManifestDigestOptions;
}
 ...
 */
export async function computeApplicationManifestDigest(
  input: ApplicationManifestAdoptionInput,
  options: ApplicationManifestDigestOptions,
): Promise<string> {
 ...
 */
export async function adoptApplicationManifest(
  input: ApplicationManifestAdoptionInput,
  options: ApplicationManifestDigestOptions,
  ...
  if (declaredDigest === undefined || declaredDigest.length === 0) {
    throw new ApplicationManifestError(
      'INVALID_MANIFEST',
      'manifest field "manifestContentDigest" must be a non-empty string; compute it with computeApplicationManifestDigest and present content and digest together',
    );
  }
  ...
  return mintManifest(
    Object.freeze({ ...record, manifestContentDigest: canonicalDigest }) as ApplicationManifest,
  );
}

/** True only for manifests actually adopted by `adoptApplicationManifest`. */
export function isApplicationManifest(value: unknown): value is ApplicationManifest {
  if (!isMintedManifest(value)) return false;
  const candidate = value as Partial<ApplicationManifest>;
  return (
    candidate.adapter === APPLICATION_MANIFEST_ADAPTER_VERSION &&
...
export function manifestIdentityOf(manifest: ApplicationManifest): ApplicationManifestIdentity {
  if (!isApplicationManifest(manifest)) {
    throw new ApplicationManifestError(
      'NOT_AN_ADOPTED_APPLICATION_MANIFEST',
      'identity projection requires a manifest actually adopted by the application-manifest adapter',
    );
  }
  return Object.freeze({
    applicationSemanticIdentity: manifest.applicationSemanticIdentity,
...
export function isManifestCompositionEvidence(value: unknown): value is ManifestCompositionEvidence {
  if (!isMintedCompositionEvidence(value)) return false;
  const candidate = value as Partial<ManifestCompositionEvidence>;
  return (
    candidate.manifestComposition === MANIFEST_COMPOSITION_ADAPTER_VERSION &&
...
export function isManifestRuntimeBindingCorrelation(
  value: unknown,
): value is ManifestRuntimeBindingCorrelation {
  if (!isMintedBindingCorrelation(value)) return false;
  const candidate = value as Partial<ManifestRuntimeBindingCorrelation>;
  return (
    candidate.manifestBinding === MANIFEST_BINDING_CORRELATION_VERSION &&
    isApplicationManifestIdentity(candidate.manifestIdentity) &&
...
export function isManifestRuntimeActivationCorrelation(
  value: unknown,
): value is ManifestRuntimeActivationCorrelation {
  if (!isMintedActivationCorrelation(value)) return false;
  const candidate = value as Partial<ManifestRuntimeActivationCorrelation>;
  return (
    candidate.manifestActivation === MANIFEST_ACTIVATION_CORRELATION_VERSION &&
    isApplicationManifestIdentity(candidate.manifestIdentity)
  );
}

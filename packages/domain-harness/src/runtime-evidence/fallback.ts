...
import {
  ...
  type ExperimentalRollbackOutcome,
  type ExperimentalRollbackRequest,
} from './contracts.js';
 ...
 */
export async function requestExperimentalRollback(
  request: ExperimentalRollbackRequest,
  ports: ExperimentalRollbackPorts,
): Promise<ExperimentalRollbackOutcome> {
  ...
  if (
    request.operator.kind !== 'human-operator' ||
    request.operator.actorId.trim().length === 0 ||
    request.operator.operatorId.trim().length === 0
  ) {
    throw new RuntimeEvidenceIntegrationError(
      'INVALID_RUNTIME_EVIDENCE_INTEGRATION',
      'rollback requires an explicit human-operator identity',
    );
  }

  const evidence: RuntimeEvidenceRecord[] = [];
  evidence.push(
    await ports.capture.captureFallback({
  ...
  evidence.push(
    await ports.capture.captureOperatorOverride({
      ...
      detail: {
        ...
        stableFallback: {
          ...
          governanceBaselineContentDigest:
            request.experimentalArtifact.stableFallback.governanceBaselineContentDigest,
          artifact: {

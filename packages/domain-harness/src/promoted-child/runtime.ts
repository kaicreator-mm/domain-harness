...
import { isContentDigest, type Sha256Port } from '../contracts/identity.js';
...
} from '../harness/execution-journal.js';
import type { PromotedArtifactContractError } from '../promoted-artifact/contracts.js';
import {
...
import {
  assertPromotedChildApplicable,
  assertPromotedChildCompatible,
} from './compatibility.js';
import {
  assertInvokingContextMatchesGovernancePin,
...
function resolvePath(value: JsonValue, path: string, label: string): JsonValue {
  ...
  for (const segment of path.split('.')) {
    if (Array.isArray(current) && /^\d+$/.test(segment)) {
      ...
      throw new DynamicChildExecutionError(
        'DYNAMIC_CHILD_VALUE_RESOLUTION_FAILED',
        `${label} cannot resolve path segment ${JSON.stringify(segment)}`,
      );
    }
    if (current === undefined) {
      throw new DynamicChildExecutionError(
        'DYNAMIC_CHILD_VALUE_RESOLUTION_FAILED',
        `${label} has no value at path ${JSON.stringify(path)}`,
      );
    }
  }
...
export class PromotedChildExecutionSession {
  ...
  private resolveSource(
    source: PromotedChildValueSource,
    input: JsonValue,
    stepOutputs: ReadonlyMap<string, JsonValue>,
    label: string,
  ): JsonValue {
    switch (source.kind) {
      case 'literal':
      ...
      case 'step-output': {
        ...
        if (produced === undefined) {
          throw new DynamicChildExecutionError(
            'DYNAMIC_CHILD_VALUE_RESOLUTION_FAILED',
            `${label} references step node ${source.node} that produced no output`,
          );
        }
      ...
      default:
        throw new DynamicChildExecutionError(
          'DYNAMIC_CHILD_VALUE_RESOLUTION_FAILED',
          `${label} uses an unknown value source`,
        );
    }
  }
   ...
   */
  async run(options: RunPromotedChildOptions): Promise<PromotedChildTerminalResult> {
    ...
    for (const compiled of this.compiled.steps) {
      ...
      if (executed > this.compiled.maxSteps) {
        throw new DynamicChildExecutionError(
          'DYNAMIC_CHILD_BOUND_EXCEEDED',
          `promoted child executed more than the declared maxSteps bound ${this.compiled.maxSteps}`,
        );
      }
      ...
      switch (step.kind) {
        case 'query': {
          ...
          if (result.status === 'journal-failure') {
            throw new DynamicChildExecutionError(
              'DYNAMIC_CHILD_JOURNAL_FAILURE',
              `journaled query step ${compiled.node} failed closed: ${result.code} ${result.message}`,
            );
          }
          if (result.status === 'cancelled') {
            throw new DynamicChildExecutionError(
              'DYNAMIC_CHILD_JOURNAL_FAILURE',
              `journaled query step ${compiled.node} was cancelled`,
            );
          }
          if (result.evidence.outcome.status !== 'succeeded') {
            throw new DynamicChildExecutionError(
              'DYNAMIC_CHILD_JOURNAL_FAILURE',
              `query step ${compiled.node} failed: ${result.evidence.outcome.code} ${result.evidence.outcome.message}`,
            );
          }
        ...
        case 'reasoned':
          throw new DynamicChildExecutionError(
            ...
            `reasoned step ${compiled.node} cannot execute in the v0.3 promoted child runtime`,
          );
        default:
          throw new DynamicChildExecutionError(
            'DYNAMIC_CHILD_BODY_INVALID',
            `step ${compiled.node} has an unsupported kind`,
          );
      }
    }
    ...
    if (output === undefined) {
      throw new DynamicChildExecutionError(
        'DYNAMIC_CHILD_BODY_INVALID',
        'promoted child completed without producing its declared terminal output',
      );
    }
...
function isPromotedRegistryError(error: unknown, code: string): boolean {
  return typeof error === 'object'
    && error !== null
    && (error as PromotedArtifactContractError).name === 'PromotedArtifactContractError'
    && (error as PromotedArtifactContractError).code === code;
}

export class PromotedChildRuntime {
  private readonly pins: DynamicChildPinCoordinator;

  constructor(
    private readonly artifactPort: PromotedChildArtifactPort,
    pinStore: DynamicChildPinStore,
    private readonly sha256: Sha256Port,
  ) {
   ...
   */
  async beginFreshExecution(input: BeginFreshPromotedChildInput): Promise<PromotedChildExecutionSession> {
   ...
   */
  async recoverExecution(input: RecoverPromotedChildInput): Promise<PromotedChildExecutionSession> {
    ...
    if (pin.invokingPackageId !== input.invoking.packageId) {
      throw new DynamicChildExecutionError(
        ...
        'recovery invoking package does not match the pinned invocation package; recovery never switches to the active package',
      );
    }
    ...
    if (
      ...
      throw new DynamicChildExecutionError(
        ...
        'recovery authority does not match the pinned invocation authority',
      );
    }
    ...
    try {
      ...
      if (isPromotedRegistryError(error, 'PROMOTED_ARTIFACT_NOT_FOUND')) {
        throw new DynamicChildExecutionError(
          'DYNAMIC_CHILD_BODY_MISSING',
          'the exact promoted artifact body required by a pinned recovery is missing from the registry',
          error,
        );
      }
      if (isPromotedRegistryError(error, 'PROMOTED_ARTIFACT_DIGEST_MISMATCH')) {
        throw new DynamicChildExecutionError(
          ...
          'the recovered promoted artifact body re-hashes to a different digest; fail closed',
          error,
        );
      }
      throw error;
    ...
    if (compiled.artifact.contentDigest !== pin.artifact.contentDigest) {
      throw new DynamicChildExecutionError(
        'DYNAMIC_CHILD_DIGEST_MISMATCH',
        'recompiled child identity does not match the durable pin',
      );
    }
 ...
 */
export async function requireDynamicChildOperationGate(
  store: DynamicChildPinStore,
  slot: DynamicChildInvocationSlot,
): Promise<DynamicChildExecutionPin> {
  const pin = await store.get(dynamicChildSlotKey(slot));
  if (pin === undefined) {
    throw new DynamicChildExecutionError(
      'DYNAMIC_CHILD_WORK_BEFORE_PIN',

...
export function assertDacV003IdempotencyReuseForLogicalEffect(
  ...
  if (
    ...
    throw new DacV003ExternalError(
      'IDEMPOTENCY_REUSE_FORBIDDEN',
      `logical operation "${logicalOperation.reference.primaryIdentity}" already carries a different idempotency identity (key, issuer or promised deduplication scope diverge); one logical operation is bound to one idempotency identity within its scope`,
    );
  }
}

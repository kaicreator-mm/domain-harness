...
function deriveConclusion(
  ...
  const evidencePolarities = new Set<
    'commit' | 'non-commit' | 'provider-effect-success'
  >();
  for (const o of currentObservations) {
    const polarity = truthPolarity(o.observedClass);
    if (polarity !== 'neutral') evidencePolarities.add(polarity);
  }
  if (linkedEffectRecord !== undefined) evidencePolarities.add('commit');
  for (const e of upstreamV002Evidence) {
    if (isExternalReconciliationOutcome(e)) {
      if (e.result === 'RECONCILED_COMMITTED') evidencePolarities.add('commit');
      ...
      const polarity = truthPolarity(
        DAC_V003_OUTCOME_CLASS_FROM_V002_CLASSIFICATION[e.classification],
      );
      if (polarity !== 'neutral') evidencePolarities.add(polarity);
    }
  }
  ...
  if (evidencePolarities.size >= 2) {
    notes.push(
      `materially contradictory authoritative evidence: distinct non-neutral polarities [${[...evidencePolarities].join(', ')}] coexist across current observations, effect-record links and/or genuine #309 upstream evidence; no precedence resolves them and the truth stays unresolved`,
    );
    return Object.freeze({

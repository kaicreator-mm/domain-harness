...
import {
  ...
  type ExternalAuthorityBaselineInput,
  type ExternalAuthorityFamilyReference,
  type ExternalAuthorityRef,
  type ExternalAuthorityRefAdoptionInput,
  type ExternalAuthorityReferenceRole,
  type ExternalDispatchAttemptEvidence,
  type ExternalEffectCorrelation,
  type ExternalEffectCorrelationExpectation,
  type ExternalObservationClaim,
  type ExternalObservationClassification,
  type ExternalObservationEvidence,
  type ExternalObservationRef,
  type ExternalObservationRefAdoptionInput,
  type ProviderOperationRef,
  type ProviderOperationRefAdoptionInput,
  type ExternalReconciliationOutcome,
  type ExternalReconciliationRef,
  type ExternalReconciliationRefAdoptionInput,
  type ExternalReconciliationResult,
  type RuntimeLogicalOperationRef,
  type RuntimeLogicalOperationRefAdoptionInput,
} from './contracts.js';
import type {
...
function baselineMatches(input: ExternalAuthorityBaselineInput): boolean {
  return (
    input.contract === EXTERNAL_AUTHORITY_BASELINE.contract &&
...
function requireNonEmptyString(value: unknown, field: string): void {
  if (typeof value !== 'string' || value.trim().length === 0) {
    throw new ExternalAuthorityError(
      'INVALID_REFERENCE',
...
function requireEvidenceNonEmptyString(value: unknown, field: string): void {
  if (typeof value !== 'string' || value.trim().length === 0) {
    throw new ExternalAuthorityError(
      'INVALID_EVIDENCE',
      `${field} must be a non-empty string`,
...
function requireEvidencePositiveSafeInteger(value: unknown, field: string): void {
  if (!Number.isSafeInteger(value) || (value as number) <= 0) {
    throw new ExternalAuthorityError(
      'INVALID_EVIDENCE',
      `${field} must be a positive safe integer`,
...
function validateOpaque(value: unknown): void {
  if (
    value !== undefined &&
...
const ADOPTED_CORRELATIONS = new WeakSet<object>();
const ADOPTED_EVIDENCE = new WeakSet<object>();

/** DAC EXTERNAL_AUTHORITY §4 authority matrix — claim ceilings, frozen. */
const CLAIM_CEILING: Readonly<
  Record<ExternalObservationClassification, ExternalObservationClaim>
> = Object.freeze({
 ...
 */
export function maxClaimableForExternalObservation(
  classification: ExternalObservationClassification,
): ExternalObservationClaim {
  const ceiling = CLAIM_CEILING[classification];
  if (ceiling === undefined) {
    throw new ExternalAuthorityError(
      'INVALID_EVIDENCE',
      `unknown observation classification "${String(classification)}"`,
    );
  }
  return ceiling;
}
...
export function observationSupportsCommitClaim(
  evidence: ExternalObservationEvidence,
): boolean {
 ...
 */
export function observationSupportsNonCommitClaim(
  evidence: ExternalObservationEvidence,
): boolean {
  return (
    evidence.classification === 'rejected' ||
    evidence.classification === 'known-failed-before-commit'
  );
}

function baselineOf(input: { readonly baseline: ExternalAuthorityBaselineInput }): void {
  if (
    input === null ||
    typeof input !== 'object' ||
    input.baseline === null ||
    ...
    throw new ExternalAuthorityError(
      ...
      `reference baseline must be exactly ${EXTERNAL_AUTHORITY_BASELINE.contract}@${EXTERNAL_AUTHORITY_BASELINE.version} commit ${EXTERNAL_AUTHORITY_BASELINE.baselineCommit}`,
    );
  }
}

/** Shared reference adoption core: validates, nominalizes and freezes. */
function adoptExternalReference<R extends ExternalAuthorityReferenceRole>(
  role: R,
  input: object,
): ExternalAuthorityFamilyReference & { readonly role: R } {
  if (input === null || typeof input !== 'object') {
    throw new ExternalAuthorityError('INVALID_REFERENCE', 'adoption input must be an object');
  }
  baselineOf(input as { baseline: ExternalAuthorityBaselineInput });
  const record = input as Record<string, unknown>;
  validateOpaque(record.opaque);
  const opaque = freezeAdopted({ ...((record.opaque as Record<string, unknown>) ?? {}) });

  let adopted: ExternalAuthorityFamilyReference & { readonly role: R };
  switch (role) {
    case 'external-authority': {
      ...
      adopted = freezeAdopted({
        ...
        opaque,
      }) as ExternalAuthorityFamilyReference & { readonly role: R };
      break;
    }
    case 'runtime-logical-operation': {
      ...
      if (
        ...
        const shown =
          typeof record.effectSemantics === 'string' ? record.effectSemantics : '<non-string>';
        throw new ExternalAuthorityError(
          'INVALID_REFERENCE',
          `effectSemantics must be one of none|idempotent|non-idempotent, got "${shown}"`,
        );
      }
    ...
    case 'provider-operation': {
      if (
        ...
        throw new ExternalAuthorityError(
          'INVALID_REFERENCE',
          'provider-operation requires at least one of providerOperationId/idempotencyKey/requestRef (adopt only where the provider contract makes one available)',
        );
      }
...
export function adoptExternalAuthorityRef(
  input: ExternalAuthorityRefAdoptionInput,
...
export function adoptRuntimeLogicalOperationRef(
  input: RuntimeLogicalOperationRefAdoptionInput,
 ...
 */
export function adoptRuntimeLogicalOperationRefFromJournal(
  baseline: ExternalAuthorityBaselineInput,
  record: Pick<EffectJournalRecord, 'effectId' | 'attempt' | 'effectSemantics'>,
): RuntimeLogicalOperationRef {
 ...
 */
export function adoptRuntimeLogicalOperationRefFromExecutionContext(
  baseline: ExternalAuthorityBaselineInput,
  context: Pick<EffectExecutionContext, 'effectId' | 'attempt' | 'idempotencyKey'>,
): RuntimeLogicalOperationRef {
  return adoptExternalReference('runtime-logical-operation', {
    baseline,
...
export function adoptProviderOperationRef(
  input: ProviderOperationRefAdoptionInput,
): ProviderOperationRef {
  return adoptExternalReference('provider-operation', input);
}

/** Adopt the identity of one externally-originated observation. */
export function adoptExternalObservationRef(
  input: ExternalObservationRefAdoptionInput,
...
export function adoptExternalReconciliationRef(
  input: ExternalReconciliationRefAdoptionInput,
): ExternalReconciliationRef {
  return adoptExternalReference('external-reconciliation', input);
}

function structurallyValidAdoptedReference(
  value: unknown,
): value is ExternalAuthorityFamilyReference {
  if (value === null || typeof value !== 'object') return false;
  if (!ADOPTED_REFERENCES.has(value)) return false;
  const candidate = value as Partial<ExternalAuthorityFamilyReference> & { role?: unknown };
  return (
    candidate.adapter === EXTERNAL_AUTHORITY_ADAPTER_VERSION &&
    typeof candidate.role === 'string' &&
    (EXTERNAL_AUTHORITY_REFERENCE_ROLES as readonly string[]).indexOf(candidate.role) !== -1
  );
}

/** Structural guard for any adopted reference of this family (unknown-safe). */
export function isExternalAuthorityFamilyReference(
  value: unknown,
): value is ExternalAuthorityFamilyReference {
...
export function getExternalAuthorityFamilyRole(
  value: unknown,
): ExternalAuthorityReferenceRole | undefined {
  return structurallyValidAdoptedReference(value) ? value.role : undefined;
}

function roleGuard<R extends ExternalAuthorityReferenceRole>(role: R, value: unknown): boolean {
  return structurallyValidAdoptedReference(value) && value.role === role;
}

function expectRole<R extends ExternalAuthorityReferenceRole>(
  role: R,
  value: unknown,
  description: string,
): void {
  if (!roleGuard(role, value)) {
    ...
    throw new ExternalAuthorityError(
      'ROLE_MISMATCH',
      `expected a ${description} reference, but received ${actual}; external-effect identity roles are never interchangeable`,
    );
  }
}

export function isExternalAuthorityRef(v: unknown): v is ExternalAuthorityRef {
  return roleGuard('external-authority', v);
}
export function isRuntimeLogicalOperationRef(v: unknown): v is RuntimeLogicalOperationRef {
  return roleGuard('runtime-logical-operation', v);
}
export function isProviderOperationRef(v: unknown): v is ProviderOperationRef {
  return roleGuard('provider-operation', v);
}
export function isExternalObservationRef(v: unknown): v is ExternalObservationRef {
  return roleGuard('external-observation', v);
}
export function isExternalReconciliationRef(v: unknown): v is ExternalReconciliationRef {
  return roleGuard('external-reconciliation', v);
}

export function expectExternalAuthorityRef(v: unknown): asserts v is ExternalAuthorityRef {
  expectRole('external-authority', v, 'external-authority');
}
export function expectRuntimeLogicalOperationRef(
  v: unknown,
): asserts v is RuntimeLogicalOperationRef {
  expectRole('runtime-logical-operation', v, 'runtime-logical-operation');
}
export function expectProviderOperationRef(v: unknown): asserts v is ProviderOperationRef {
  expectRole('provider-operation', v, 'provider-operation');
}
export function expectExternalObservationRef(v: unknown): asserts v is ExternalObservationRef {
  expectRole('external-observation', v, 'external-observation');
}
export function expectExternalReconciliationRef(
  v: unknown,
): asserts v is ExternalReconciliationRef {
 ...
 */
export function refuteNonExternalAuthorityIdentity(value: unknown): void {
  if (structurallyValidAdoptedReference(value) && value.role !== 'external-authority') {
    throw new ExternalAuthorityError(
      'IDENTITY_MISMATCH',
      `a "${value.role}" reference is ${value.role === 'runtime-logical-operation' || value.role === 'provider-operation' ? 'Runtime/provider-side' : 'evidence/material'} identity and can never substitute external Business SoR authority identity`,
    );
  }
}
 ...
 */
const LOCAL_CAUSE_FIELDS: ReadonlySet<string> = new Set([
  'cause',
  'error',
  'signal',
  'abortSignal',
...
function rejectLocalCauseFields(input: object, what: string): void {
  for (const key of Object.keys(input)) {
    if (LOCAL_CAUSE_FIELDS.has(key)) {
      throw new ExternalAuthorityError(
        'LOCAL_CAUSE_FORBIDDEN',
        `${what} input carries local execution cause field "${key}"; a local timeout/abandonment/cancellation is never evidence of remote commit or non-commit — record it outside external evidence (DAC EXTERNAL_AUTHORITY §2)`,
      );
    }
  }
}
 ...
 */
export function correlateExternalEffect(
  input: ExternalEffectCorrelationInput,
): ExternalEffectCorrelation {
  if (input === null || typeof input !== 'object') {
  ...
  if (
    input.runtimeOperation.idempotencyKey !== undefined &&
    ...
    throw new ExternalAuthorityError(
      ...
      `runtime logical operation idempotency key "${input.runtimeOperation.idempotencyKey}" conflicts with provider operation idempotency key "${input.providerOperation.idempotencyKey}"; one logical effect never rebinds to a different idempotency identity`,
    );
  }

  const correlation: ExternalEffectCorrelation = freezeAdopted({
    adapter: EXTERNAL_AUTHORITY_ADAPTER_VERSION,
    baseline: EXTERNAL_AUTHORITY_BASELINE,
...
function expectCorrelation(value: unknown): asserts value is ExternalEffectCorrelation {
  if (value === null || typeof value !== 'object' || !ADOPTED_CORRELATIONS.has(value)) {
    throw new ExternalAuthorityError(
      'ROLE_MISMATCH',
      'expected an adopted ExternalEffectCorrelation, but received a foreign object; correlations are never interchangeable with references or evidence',
    );
  }
}

export function isExternalEffectCorrelation(v: unknown): v is ExternalEffectCorrelation {
  return v !== null && typeof v === 'object' && ADOPTED_CORRELATIONS.has(v);
}

export function expectExternalEffectCorrelation(
  v: unknown,
): asserts v is ExternalEffectCorrelation {
  expectCorrelation(v);
}
 ...
 */
export function verifyExternalEffectCorrelation(
  correlation: ExternalEffectCorrelation,
  expectation: ExternalEffectCorrelationExpectation,
): void {
  ...
  if (expectation.correlationId !== undefined && correlation.correlationId !== expectation.correlationId) {
    mismatches.push(
      `correlationId: expected "${expectation.correlationId}", got "${correlation.correlationId}"`,
    );
  }
  if (
    expectation.effectId !== undefined &&
    correlation.runtimeOperation.effectId !== expectation.effectId
  ) {
    mismatches.push(
      `effectId: expected "${expectation.effectId}", got "${correlation.runtimeOperation.effectId}"`,
    );
  }
  if (
    expectation.authorityId !== undefined &&
    correlation.externalAuthority.authorityId !== expectation.authorityId
  ) {
    mismatches.push(
      `authorityId: expected "${expectation.authorityId}", got "${correlation.externalAuthority.authorityId}"`,
    );
  }
  if (
    expectation.authorityScope !== undefined &&
    correlation.externalAuthority.authorityScope !== expectation.authorityScope
  ) {
    mismatches.push(
      `authorityScope: expected "${expectation.authorityScope}", got "${correlation.externalAuthority.authorityScope}"`,
    );
  }
  if (expectation.providerOperationId !== undefined) {
    ...
    if (actual !== expectation.providerOperationId) {
      mismatches.push(
        `providerOperationId: expected "${expectation.providerOperationId}", got "${actual ?? '<absent>'}"`,
      );
    }
  }
  if (mismatches.length > 0) {
    throw new ExternalAuthorityError(
      'IDENTITY_MISMATCH',
      `exact correlation mismatch on "${correlation.correlationId}": ${mismatches.join('; ')}`,
    );
  }
}
 ...
 */
export function adoptExternalDispatchAttemptEvidence(
  input: ExternalDispatchAttemptEvidenceInput,
): ExternalDispatchAttemptEvidence {
  if (input === null || typeof input !== 'object') {
  ...
  const evidence: ExternalDispatchAttemptEvidence = freezeAdopted({
 ...
 */
export function adoptExternalObservationEvidence(
  input: ExternalObservationEvidenceInput,
  ...
  if ('claim' in input) {
    throw new ExternalAuthorityError(
      'INVALID_EVIDENCE',
      'claim is derived from the classification ceiling and can never be caller-supplied (DAC EXTERNAL_AUTHORITY §4)',
    );
  }
  expectCorrelation(input.correlation);
  ...
  if (
    (EXTERNAL_OBSERVATION_CLASSIFICATIONS as readonly string[]).indexOf(
      input.classification,
    ) === -1
  ) {
    throw new ExternalAuthorityError(
      'INVALID_EVIDENCE',
      `classification must be one of ${EXTERNAL_OBSERVATION_CLASSIFICATIONS.join('|')}, got "${String(input.classification)}" — unrecognized or semantically insufficient external statements map to unknown-ambiguous`,
    );
  }
  ...
  const evidence: ExternalObservationEvidence = freezeAdopted({
...
function evidenceIsAdoptedObservation(
  member: unknown,
): member is ExternalObservationEvidence {
  return (
    member !== null &&
    typeof member === 'object' &&
    ADOPTED_EVIDENCE.has(member) &&
    (member as { evidenceClass?: unknown }).evidenceClass === 'external-observation'
  );
}
 ...
 */
export function adoptExternalReconciliationOutcome(
  input: ExternalReconciliationOutcomeInput,
  ...
  for (const member of basis) {
    if (!evidenceIsAdoptedObservation(member)) {
      throw new ExternalAuthorityError(
        'EVIDENCE_CONFLICT',
        'reconciliation basis must contain only adopted external-observation evidence; dispatch-attempt evidence or foreign objects never establish remote truth',
      );
    }
  }
  const hasCommitBasis = basis.some((observation) => observationSupportsCommitClaim(observation));
  const hasNonCommitBasis = basis.some((observation) =>
    observationSupportsNonCommitClaim(observation),
  );

  let remoteTruth: 'committed' | 'not-committed' | 'unresolved';
  switch (input.result) {
    case 'RECONCILED_COMMITTED': {
      if (!hasCommitBasis) {
        throw new ExternalAuthorityError(
          'EVIDENCE_CONFLICT',
          'RECONCILED_COMMITTED requires an adopted observation with classification commit-observed; provider-scope effect success is not Business SoR commit',
        );
      }
    ...
    case 'RECONCILED_NOT_COMMITTED': {
      if (!hasNonCommitBasis) {
        throw new ExternalAuthorityError(
          'EVIDENCE_CONFLICT',
          'RECONCILED_NOT_COMMITTED requires an adopted observation with classification rejected or known-failed-before-commit; local timeout/abandonment never establishes remote non-commit',
        );
      }
    ...
    case 'TERMINAL_ABANDONMENT': {
      if (hasCommitBasis || hasNonCommitBasis) {
        throw new ExternalAuthorityError(
          'EVIDENCE_CONFLICT',
          `${input.result} keeps remote truth unresolved, but its basis already contains a decisive commit/non-commit observation; adopt the corresponding RECONCILED_* result instead`,
        );
      }
    ...
    default:
      throw new ExternalAuthorityError(
        'INVALID_EVIDENCE',
        `result must be one of RECONCILED_COMMITTED|RECONCILED_NOT_COMMITTED|STILL_UNKNOWN|TERMINAL_ABANDONMENT, got "${String(input.result)}"`,
      );
  }

  const outcome: ExternalReconciliationOutcome = freezeAdopted({
...
function evidenceClassGuard<C extends 'dispatch-attempt' | 'external-observation' | 'reconciliation-outcome'>(
  evidenceClass: C,
  value: unknown,
): boolean {
  return (
    ...
    (value as { evidenceClass?: unknown }).evidenceClass === evidenceClass
  );
}

function expectEvidenceClass<
  C extends 'dispatch-attempt' | 'external-observation' | 'reconciliation-outcome',
>(evidenceClass: C, value: unknown, description: string): void {
  if (!evidenceClassGuard(evidenceClass, value)) {
    const actual =
      value !== null &&
      typeof value === 'object' &&
      ADOPTED_EVIDENCE.has(value) &&
      typeof (value as { evidenceClass?: unknown }).evidenceClass === 'string'
        ? `"${(value as { evidenceClass: string }).evidenceClass}"`
        : 'not adopted external evidence';
    throw new ExternalAuthorityError(
      'ROLE_MISMATCH',
      `expected ${description}, but received ${actual}; external evidence classes are never interchangeable`,
    );
  }
}

export function isExternalDispatchAttemptEvidence(
  v: unknown,
): v is ExternalDispatchAttemptEvidence {
  return evidenceClassGuard('dispatch-attempt', v);
}
export function isExternalObservationEvidence(v: unknown): v is ExternalObservationEvidence {
  return evidenceClassGuard('external-observation', v);
}
export function isExternalReconciliationOutcome(
  v: unknown,
): v is ExternalReconciliationOutcome {
  return evidenceClassGuard('reconciliation-outcome', v);
}

export function expectExternalDispatchAttemptEvidence(
  v: unknown,
): asserts v is ExternalDispatchAttemptEvidence {
  expectEvidenceClass('dispatch-attempt', v, 'dispatch-attempt evidence');
}
export function expectExternalObservationEvidence(
  v: unknown,
): asserts v is ExternalObservationEvidence {
  expectEvidenceClass('external-observation', v, 'external-observation evidence');
}
export function expectExternalReconciliationOutcome(
  v: unknown,
): asserts v is ExternalReconciliationOutcome {

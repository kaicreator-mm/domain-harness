    ...
    async execute(request: AdmissionEffectToolRequest) {
      ...
      if (request.binding.effectSemantics !== descriptor.effect) {
        throw new HostLocalDomainToolBindingError(
          ...
          `binding effect semantics '${request.binding.effectSemantics}' do not match the compiled Tool descriptor '${descriptor.effect}' for effect type '${request.binding.effectType}'`,
        );
      }

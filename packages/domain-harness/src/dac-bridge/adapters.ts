...
function requireWorkflowAddress(value: unknown, field: string): asserts value is WorkflowAddress {
  if (
    value === null ||
    typeof value !== 'object' ||
    typeof (value as Partial<WorkflowAddress>).workflowId !== 'string' ||
    (value as Partial<WorkflowAddress>).workflowId!.length === 0 ||
    typeof (value as Partial<WorkflowAddress>).instanceKey !== 'string' ||
    (value as Partial<WorkflowAddress>).instanceKey!.length === 0
  ) {
...
export function validateObservedBasis(basis: unknown, field: string): asserts basis is DacObservedBasis {
  if (basis === null || typeof basis !== 'object') {
    throw new DacBridgeError('INVALID_CORRELATION', `${field} must be an observed-basis object`);
  }
  const candidate = basis as Partial<DacObservedBasis>;
  if (candidate.kind === 'snapshot-ref') {
    if (!isSnapshotRef(candidate.snapshotRef)) {
      throw new DacBridgeError(
        'INVALID_CORRELATION',
        `${field}.snapshotRef must be a bridge-minted SnapshotRef (forged snapshot identities fail closed)`,
      );
    }
    return;
  }
  if (candidate.kind === 'revision') {
    ...
    if (typeof candidate.revision === 'string' && isMutableAliasToken(candidate.revision)) {
      throw new DacBridgeError(
        ...
        `${field}.revision "${candidate.revision}" is a mutable alias and can never prove an exact observation basis`,
      );
    }
  ...
  throw new DacBridgeError(
    'INVALID_CORRELATION',
    `${field}.kind must be 'snapshot-ref' or 'revision'`,
  );
}

/** Adopt a domain/application semantic-target identity (DAC `SemanticTargetRef`). */
export function adoptSemanticTargetRef(input: DacIntentAdoptionInput): SemanticTargetRef {
...
export function adoptDomainIntentRef(input: DacIntentAdoptionInput): DomainIntentRef {
  if (input === null || typeof input !== 'object') {
    throw new DacBridgeError('INVALID_REFERENCE', 'adoption input must be an object');
  }
  requireExactBaseline(input.baseline);
  ...
  if (input.semanticTarget !== undefined && !isSemanticTargetRef(input.semanticTarget)) {
    throw new DacBridgeError(
      'INVALID_CORRELATION',
      'semanticTarget must be a bridge-minted SemanticTargetRef (forged targets fail closed)',
    );
  }
  if (input.observedBasis !== undefined) {
 ...
 */
export function commandRefFromDomainMessage(message: DomainMessage): CommandRef {
...
function sameObservedBasis(
  left: DacObservedBasis | undefined,
  right: DacObservedBasis | undefined,
): boolean {
 ...
 */
export function correlateDomainCommand(
  ...
  if (intent !== undefined && !isDomainIntentRef(intent)) {
    throw new DacBridgeError(
      'INVALID_CORRELATION',
      'intent must be a bridge-minted DomainIntentRef (a forged intent is not UX-origin correlation)',
    );
  }
  if (semanticTarget !== undefined && !isSemanticTargetRef(semanticTarget)) {
    throw new DacBridgeError(
      'INVALID_CORRELATION',
      'semanticTarget must be a bridge-minted SemanticTargetRef',
    );
  }
  ...
  if (
    ...
    throw new DacBridgeError(
      ...
      'correlation.semanticTarget and intent.semanticTarget are different references; resolve explicitly instead of silently preferring one',
    );
  }
  if (
    intent !== undefined &&
    ...
    throw new DacBridgeError(
      'CORRELATION_CONFLICT',
      'correlation.observedBasis conflicts with intent.observedBasis; resolve explicitly instead of silently preferring one',
    );
  }
...
function requireDispositionSnapshot(
  value: MessageDispositionSnapshot,
): asserts value is MessageDispositionSnapshot {
  ...
  if (!DISPOSITIONS.has(value.disposition)) {
    throw new DacBridgeError(
      'INVALID_REFERENCE',
      `disposition "${String(value.disposition)}" is not a Runtime message disposition`,
    );
  }
 ...
 */
export function outcomeRefFromMessageDisposition(
 ...
 */
export function outcomeRefFromAcceptedAck(ack: MessageAcceptedAck): OutcomeRef {
  ...
  if (ack.status !== 'accepted' && ack.status !== 'duplicate') {
    throw new DacBridgeError(
      'INVALID_REFERENCE',
      `ack.status "${String(ack.status)}" is not a MessageAcceptedAck status`,
    );
  }
...
export function correlateDomainOutcome(input: {
  ...
  if ((input.disposition === undefined) === (input.ack === undefined)) {
    throw new DacBridgeError(
      'INVALID_CORRELATION',
      'exactly one of disposition or ack must be supplied',
    );
  }
  const outcome =
  ...
  if (correlation !== undefined) {
    if (
      ...
      throw new DacBridgeError(
        'INVALID_CORRELATION',
        'correlation must be a bridge-minted DomainCommandCorrelation',
      );
    }
    const command = correlation.command;
    if (
      command.messageId !== outcome.messageId ||
      command.target.workflowId !== outcome.target.workflowId ||
      command.target.instanceKey !== outcome.target.instanceKey
    ) {
      throw new DacBridgeError(
        'CORRELATION_CONFLICT',
        `outcome ${outcome.messageId} does not resolve correlated command ${command.messageId}; causal chains are never assembled across different commands`,
      );
    }
    return mintCorrelation({
      outcome,
 ...
 */
export function viewRefFromQueryResult(
  request: DomainQuery,
  ...
  if (request.kind !== result.kind) {
    throw new DacBridgeError(
      'INVALID_CORRELATION',
      `result kind "${String(result.kind)}" does not answer query kind "${String(request.kind)}"`,
    );
  }
  ...
  switch (result.kind) {
    case 'instance': {
      const query = request as Extract<DomainQuery, { kind: 'instance' }>;
      ...
      return mintReference({
        ...
        target: query.target,
        opaque: freezeOpaque(
          result.value === null ? {} : { stateRevision: result.value.stateRevision },
        ),
      });
    }
    case 'message-disposition': {
      const query = request as Extract<DomainQuery, { kind: 'message-disposition' }>;
      requireWorkflowAddress(query.target, 'request.target');
    ...
    case 'runtime-failure': {
      const query = request as Extract<DomainQuery, { kind: 'runtime-failure' }>;
      requireWorkflowAddress(query.target, 'request.target');
      return mintReference({
        adapter: DAC_BRIDGE_ADAPTER_VERSION,
        baseline: DAC_BRIDGE_BASELINE,
    ...
    case 'projection': {
      ...
      const query = request as Extract<DomainQuery, { kind: 'projection' }>;
      ...
      if (snapshot.projectionId !== query.projectionId || snapshot.key !== query.key) {
        throw new DacBridgeError(
          'INVALID_CORRELATION',
          'projection result does not answer the requested projection view',
        );
      }
      if (isMutableAliasToken(snapshot.revision)) {
        throw new DacBridgeError(
          'MUTABLE_ALIAS_REJECTED',
          `projection revision "${snapshot.revision}" is a mutable alias and can never be an exact view basis`,
        );
      }
      return mintReference({
        adapter: DAC_BRIDGE_ADAPTER_VERSION,
    ...
    default:
      throw new DacBridgeError(
        'INVALID_REFERENCE',
        `unknown DomainQuery kind "${String((request as { kind?: unknown }).kind)}"`,
      );
  }
}

/** SnapshotRef basis from a `WorkflowInstanceSnapshot` (stateRevision preserved). */
export function snapshotRefFromWorkflowInstanceSnapshot(
  ...
  if (
    ...
    throw new DacBridgeError(
      'INVALID_REFERENCE',
      'snapshot.stateRevision must be a non-negative safe integer',
    );
  }
  return mintReference({
    adapter: DAC_BRIDGE_ADAPTER_VERSION,
 ...
 * (L2 A2 6.5).
 */
export function snapshotRefFromProjectionSnapshot(
  ...
  if (isMutableAliasToken(snapshot.revision)) {
    throw new DacBridgeError(
      'MUTABLE_ALIAS_REJECTED',
      `projection revision "${snapshot.revision}" is a mutable alias and can never be an exact snapshot basis`,
    );
  }
  const workflowSources = snapshot.workflowSources.map((source) => {
    ...
    if (
      ...
      throw new DacBridgeError(
        'INVALID_REFERENCE',
        'snapshot.workflowSources[].stateRevision must be a non-negative safe integer',
      );
    }
...
export function snapshotRefFromBusinessSnapshot(snapshot: BusinessSnapshot): SnapshotRef {
  if (snapshot === null || typeof snapshot !== 'object') {
  ...
  if (isMutableAliasToken(snapshot.revision)) {
    throw new DacBridgeError(
      'MUTABLE_ALIAS_REJECTED',
      `business-source revision "${snapshot.revision}" is a mutable alias and can never be an exact snapshot basis`,
    );
  }
  return mintReference({
    adapter: DAC_BRIDGE_ADAPTER_VERSION,
...
function requireSubscription(
...
): asserts value is DomainSubscription {
  if (value === null || typeof value !== 'object') {
  ...
  switch (value.kind) {
    ...
    default:
      throw new DacBridgeError(
        'INVALID_REFERENCE',
        `unknown DomainSubscription kind "${String((value as { kind?: unknown }).kind)}"`,
      );
  }
}
 ...
 */
export function watchRefFromSubscription(subscription: DomainSubscription): WatchRef {
 ...
 */
export function watchRefFromObservedChange(
  ...
  if (change === null || typeof change !== 'object' || change.kind !== subscription.kind) {
    throw new DacBridgeError(
      'INVALID_CORRELATION',
      'change.kind must match the subscription kind (a foreign change is never reinterpreted)',
    );
  }
  ...
  if (isMutableAliasToken(change.revision)) {
    throw new DacBridgeError(
      'MUTABLE_ALIAS_REJECTED',
      `change.revision "${change.revision}" is a mutable alias and can never prove an exact observed revision`,
    );
  }
  return mintReference({
    adapter: DAC_BRIDGE_ADAPTER_VERSION,

...
export class InMemoryObservationStore implements RuntimeObservationStore, RuntimeStore {
  ...
  /** Test hook: drop the whole durable stream binding (host deletion edge). */
  dropObservationStreamBinding(target: WorkflowAddress, epochId: string): void {
    this.#streams.delete(`${addressKey(target)}\u0000${epochId}`);
  }

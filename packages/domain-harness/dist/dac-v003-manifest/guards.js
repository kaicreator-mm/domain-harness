...
import { dacV003TargetProfileKey, isDacV003CapabilityRequirementValue, isDacV003CompatibilityTargetRefValue, isDacV003CompatibilityValidationValue, isDacV003PortRequirementValue, isDacV003RequirementSatisfactionEvidenceValue, isDomainUXDefinitionRefValue, } from '../dac-v003-compatibility/guards.js';
...
import { DAC_V003_MANIFEST_ADAPTER_VERSION, DAC_V003_MANIFEST_CONTRACT_VERSION, DAC_V003_MANIFEST_INSTANCE_STATE_FIELD_VOCABULARY, DAC_V003_MANIFEST_VALIDATION_ASSOCIATION_VERSION, DacV003ManifestError, } from './contracts.js';
import { isMintedValidationAssociation, isMintedV003Manifest, isMutableAliasToken, isRecord, isV003ManifestIdentitySet, mintValidationAssociation, mintV003Manifest, } from './registry.js';
const INSTANCE_STATE_FIELDS = new Set(DAC_V003_MANIFEST_INSTANCE_STATE_FIELD_VOCABULARY);
/**
...
function requireNonEmptyString(value, field) {
    if (typeof value !== 'string' || value.trim().length === 0) {
        throw new DacV003ManifestError('INVALID_MANIFEST_INPUT', `manifest field "${field}" must be a non-empty string`);
    }
    return value;
}
function requireExactImmutableIdentity(value, field) {
    ...
    if (isMutableAliasToken(identity)) {
        throw new DacV003ManifestError('MUTABLE_ALIAS_REJECTED', `manifest field "${field}" carries the mutable alias "${identity}"; an exact immutable identity is required and a locator/alias never substitutes it`);
    }
    return identity;
}
function requireOpaqueRecord(value, field) {
    if (value === undefined || value === null)
        return {};
    if (!isRecord(value)) {
        throw new DacV003ManifestError('INVALID_MANIFEST_INPUT', `manifest field "${field}" must be an object of unknown/provisional source fields`);
    }
    return value;
}
/** Structural role test for a genuinely adopted v0.0.3 reference. */
function isAdoptedRoleRef(value, role) {
...
function rejectEvidenceAbsorption(value, where) {
    if (isDacV003CompatibilityValidationValue(value)) {
        throw new DacV003ManifestError('MANIFEST_EVIDENCE_ABSORPTION', `${where}: a V3-002 compatibility validation act was presented as manifest definition content; the validation result is only ever an associated external record, never a field inside the content its digest covers (R1 P2 self-reference rule)`);
    }
    if (isRecord(value) && value.result === 'dac-v003-compatibility-result/1') {
        throw new DacV003ManifestError('MANIFEST_EVIDENCE_ABSORPTION', `${where}: a V3-002 compatibility result record was presented as manifest definition content; the result view is only ever an associated external record, never manifest content`);
    }
    if (isAdoptedRoleRef(value, 'compatibility-validation') || isAdoptedRoleRef(value, 'compatibility-result')) {
        throw new DacV003ManifestError('MANIFEST_EVIDENCE_ABSORPTION', `${where}: a "${getDacV003ReferenceRole(value)}" reference was presented as manifest definition content; compatibility validation identity belongs to the separate V3-002 authority and only ever associates externally`);
    }
    if (isSelectedCompositionValidation(value)) {
        throw new DacV003ManifestError('MANIFEST_EVIDENCE_ABSORPTION', `${where}: a #306 composition-intake verdict was presented as manifest definition content; compatibility validation is a separate stage that consumes the manifest, never part of its definition`);
    }
    if (isRuntimeBindingEvidence(value)) {
        throw new DacV003ManifestError('MANIFEST_EVIDENCE_ABSORPTION', `${where}: #307 runtime binding evidence was presented as manifest definition content; binding evidence stays separate from the Manifest definition and only references its identity/digest`);
    }
    if (isRecord(value) &&
        value.activation === RUNTIME_ACTIVATION_ADAPTER_VERSION) {
        throw new DacV003ManifestError('MANIFEST_EVIDENCE_ABSORPTION', `${where}: #307 runtime activation evidence was presented as manifest definition content; activation evidence stays separate from the Manifest definition`);
    }
    if (isAdoptedRoleRef(value, 'runtime-binding') || isAdoptedRoleRef(value, 'runtime-activation')) {
        throw new DacV003ManifestError('MANIFEST_EVIDENCE_ABSORPTION', `${where}: a "${getDacV003ReferenceRole(value)}" DAC reference was presented as manifest definition content; binding/activation identity is minted at stages 4/5 after compatibility validation and never inside the Manifest definition`);
    }
}
/**
 ...
 */
function rejectLiveExternalState(value, where) {
    if (isRecord(value) && value.adapter === DAC_V003_EXTERNAL_OPERATION_ADAPTER_VERSION) {
        ...
        if (role !== undefined && LIVE_EXTERNAL_STATE_ROLES.has(role)) {
            throw new DacV003ManifestError('LIVE_EXTERNAL_STATE_ABSORPTION', `${where}: a live V3-003 "${role}" record was presented as manifest definition content; the Manifest carries external-authority identity declarations only, never live external operation/reconciliation state`);
        }
    }
    if (isDacV003Reference(value)) {
        const role = getDacV003ReferenceRole(value);
        if (role !== undefined && LIVE_EXTERNAL_STATE_ROLES.has(role)) {
            throw new DacV003ManifestError('LIVE_EXTERNAL_STATE_ABSORPTION', `${where}: a "${role}" reference was presented as manifest definition content; live external operation/reconciliation state is never manifest composition metadata`);
        }
    }
}
/**
...
function authorityCoversSelected(authority, selected) {
    if (authority.semanticIdentity !== selected.semanticIdentity)
        return false;
    if (authority.revisionIdentity !== selected.revisionIdentity)
        return false;
    if (authority.contentDigest !== undefined &&
...
function validateSelectedEntry(entry, index) {
    ...
    if (!isRecord(entry)) {
        throw new DacV003ManifestError('INVALID_MANIFEST_INPUT', `${where} must be an object carrying the exact selected v0.0.3 reference plus promotion-decision and application-selection provenance`);
    }
    ...
    if (!isAdoptedRoleRef(selected, 'selected-domain-data')) {
        throw new DacV003ManifestError('INVALID_MANIFEST_INPUT', `${where}.selected is not a genuinely adopted DAC v0.0.3 "selected-domain-data" reference; forged or foreign objects fail closed`);
    }
    if (!isAdoptedRoleRef(promotionEvidence, 'promotion-decision')) {
        throw new DacV003ManifestError('INVALID_MANIFEST_INPUT', `${where}.promotionEvidence is not a genuinely adopted DAC v0.0.3 "promotion-decision" reference; promotion authority is upstream and never synthesized here`);
    }
    if (!isAdoptedRoleRef(applicationSelection, 'application-selection')) {
        throw new DacV003ManifestError('INVALID_MANIFEST_INPUT', `${where}.applicationSelection is not a genuinely adopted DAC v0.0.3 "application-selection" reference; selection authority is upstream and never synthesized here`);
    }
    ...
    for (const slot of ['semanticIdentity', 'revisionIdentity', 'contentDigest']) {
        const value = selected[slot];
        if (typeof value !== 'string' || value.length === 0) {
            throw new DacV003ManifestError('INVALID_MANIFEST_INPUT', `${where}.selected.${slot} must be present (P3 exactness for selected Domain Data: semantic, revision and content identities together)`);
        }
    }
    const lifecycle = selected.lifecycleAuthorityRefs;
    const bound = (ref) => lifecycle.some((candidate) => candidate === ref);
    if (!bound(promotionEvidence)) {
        throw new DacV003ManifestError('SELECTED_LIFECYCLE_AUTHORITY_UNBOUND', `${where}: the stated promotion-decision provenance is not the exact object bound in the selected reference's lifecycleAuthorityRefs; the manifest never restates a lifecycle authority the adopted P3 closure does not carry`);
    }
    if (!bound(applicationSelection)) {
        throw new DacV003ManifestError('SELECTED_LIFECYCLE_AUTHORITY_UNBOUND', `${where}: the stated application-selection provenance is not the exact object bound in the selected reference's lifecycleAuthorityRefs; the manifest never restates a lifecycle authority the adopted P3 closure does not carry`);
    }
    ...
    if (!authorityCoversSelected(promotionEvidence, selected)) {
        throw new DacV003ManifestError('SELECTED_PROMOTION_EVIDENCE_NOT_EFFECTIVE', `${where}: the promotion-decision evidence does not cover the selected entry's exact semantic/revision identity; effective upstream promotion evidence is mandatory per entry and is never defaulted`, [
    ...
    if (!authorityCoversSelected(applicationSelection, selected)) {
        throw new DacV003ManifestError('SELECTED_APPLICATION_SELECTION_COVERAGE_INCOMPLETE', `${where}: the application-selection evidence does not cover the selected entry's exact semantic/revision identity; total upstream ApplicationSelection coverage is mandatory and never defaulted`, [
            `selected=${selected.semanticIdentity}/${selected.revisionIdentity}`,
...
// ---------------------------------------------------------------------------
function validatePrimaryRuntime(value) {
    if (!isRecord(value)) {
        throw new DacV003ManifestError('PRIMARY_RUNTIME_CARDINALITY', 'manifest must declare exactly one primary runtime closure (one runtime contract + one explicit compatibility target); a missing/malformed declaration is never defaulted to an ambient runtime');
    }
    ...
    if (!isAdoptedRoleRef(value.runtimeContract, 'runtime-contract')) {
        throw new DacV003ManifestError('INVALID_MANIFEST_INPUT', 'primaryRuntime.runtimeContract is not a genuinely adopted DAC v0.0.3 "runtime-contract" reference');
    }
    const contract = value.runtimeContract;
    if (typeof contract.semanticIdentity !== 'string' ||
        contract.semanticIdentity.length === 0 ||
        typeof contract.revisionIdentity !== 'string' ||
        contract.revisionIdentity.length === 0) {
        throw new DacV003ManifestError('INVALID_MANIFEST_INPUT', 'primaryRuntime.runtimeContract requires semanticIdentity and exact revisionIdentity (immutable/versioned contract; a floating "current" contract is never a primary declaration)');
    }
    if (!isDacV003CompatibilityTargetRefValue(value.compatibilityTarget)) {
        throw new DacV003ManifestError('PRIMARY_RUNTIME_CARDINALITY', 'primaryRuntime.compatibilityTarget is not a genuine V3-002 DacV003CompatibilityTargetRef; exactly one explicit compatibility target is mandatory and never inferred from the environment');
    }
    return Object.freeze({
...
function validateUxClosure(value) {
    if (!isRecord(value)) {
        throw new DacV003ManifestError('UX_CLOSURE_CARDINALITY', 'the supported composition requires exactly one DomainUXDefinitionRef and exactly one RuntimeInteractionContractRef; a missing/malformed UX closure is never defaulted');
    }
    ...
    if (!isDomainUXDefinitionRefValue(value.domainUxDefinition)) {
        throw new DacV003ManifestError('UX_CLOSURE_CARDINALITY', 'ux.domainUxDefinition is not a genuine V3-002 DomainUXDefinitionRef (immutable/versioned semantic UX contract; renderer/presentation identity is never the UX definition)');
    }
    if (!isRuntimeInteractionContractRef(value.runtimeInteractionContract)) {
        throw new DacV003ManifestError('UX_CLOSURE_CARDINALITY', 'ux.runtimeInteractionContract is not a genuine V3-001 RuntimeInteractionContractRef (semantic UX↔Runtime interaction contract; not a renderer contract)');
    }
    const definition = value.domainUxDefinition;
    const interaction = value.runtimeInteractionContract;
    if (definition.authorityScope === interaction.authorityScope &&
        definition.semanticIdentity === interaction.semanticIdentity &&
        definition.revisionIdentity === interaction.revisionIdentity) {
        throw new DacV003ManifestError('UX_CLOSURE_CARDINALITY', 'ux.domainUxDefinition and ux.runtimeInteractionContract share one identity tuple; the Domain UX definition and the Runtime interaction contract are separately referrable semantic roles and never collapse');
    }
...
function validateRequirements(input) {
    ...
    for (const requirement of port) {
        if (!isDacV003PortRequirementValue(requirement)) {
            throw new DacV003ManifestError('INVALID_MANIFEST_INPUT', 'portRequirements contains a descriptor not genuinely minted by the V3-002 compatibility authority');
        }
    ...
    for (const requirement of hostBinding) {
        if (!isAdoptedRoleRef(requirement, 'runtime-host-binding-requirement')) {
            throw new DacV003ManifestError('INVALID_MANIFEST_INPUT', 'hostBindingRequirements contains a reference that is not a genuinely adopted V3-001 RuntimeHostBindingRequirementRef (a requirement declaration, never concrete binding evidence)');
        }
        rejectLiveExternalState(requirement, 'hostBindingRequirements[]');
    }
    for (const item of evidence) {
        if (!isDacV003RequirementSatisfactionEvidenceValue(item)) {
            throw new DacV003ManifestError('INVALID_MANIFEST_INPUT', 'satisfactionEvidence contains a record not genuinely minted by the V3-002 compatibility authority');
        }
    ...
    // here.
    const declaredRequirementKeys = new Set();
    ...
    for (const item of evidence) {
        ...
        if (!declaredRequirementKeys.has(key)) {
            throw new DacV003ManifestError('REQUIREMENT_EVIDENCE_FOREIGN', 'a satisfaction-evidence reference links a requirement this manifest does not declare; the manifest carries exact references for its own declared requirements only', [`evidence=${item.reference.primaryIdentity}`, `links=${item.requirement.primaryIdentity}`]);
        }
    }
...
function validateExternalAuthorityPath(value) {
    if (!isRecord(value)) {
        throw new DacV003ManifestError('EXTERNAL_AUTHORITY_APPLICABILITY_UNDECIDED', 'the manifest must record the external-authority applicability decision explicitly (APPLICABLE with declarations, or NOT_APPLICABLE with applicability evidence); an absent/malformed decision is never guessed');
    }
    if (value.applicability === 'NOT_APPLICABLE') {
        ...
        if (typeof applicabilityEvidence !== 'string' || applicabilityEvidence.trim().length === 0) {
            throw new DacV003ManifestError('EXTERNAL_AUTHORITY_APPLICABILITY_UNDECIDED', 'a NOT_APPLICABLE external-authority decision must record non-empty applicability evidence stating why the supported Manifest path requires no external-authority declarations');
        }
        return Object.freeze({ applicability: 'NOT_APPLICABLE', applicabilityEvidence });
    }
    if (value.applicability !== 'APPLICABLE') {
        throw new DacV003ManifestError('EXTERNAL_AUTHORITY_APPLICABILITY_UNDECIDED', `externalAuthority.applicability must be "APPLICABLE" or "NOT_APPLICABLE"; got "${String(value.applicability)}"`);
    }
    const declarationsInput = value.declarations;
    if (!Array.isArray(declarationsInput) || declarationsInput.length === 0) {
        throw new DacV003ManifestError('EXTERNAL_AUTHORITY_APPLICABILITY_UNDECIDED', 'an APPLICABLE external-authority decision must carry at least one declaration; a materially-applicable path with zero declarations is an undecided applicability, not an applicable one');
    }
    const declarations = declarationsInput.map((declaration, index) => {
        const where = `externalAuthority.declarations[${index}]`;
        if (!isRecord(declaration)) {
            throw new DacV003ManifestError('INVALID_MANIFEST_INPUT', `${where} must be an object (external-authority declaration)`);
        }
        const authority = declaration.authority;
        if (!isDacV003ExternalAuthorityRef(authority)) {
            throw new DacV003ManifestError('EXTERNAL_IDENTITY_SUBSTITUTION', `${where}.authority is not a DacV003ExternalAuthorityRef genuinely adopted by the V3-003 external-operation surface; a provider URL/locator alone is not an authority contract and no Runtime identity can substitute external Business SoR identity`);
        }
...
async function materializeManifest(input) {
    if (!isRecord(input)) {
        throw new DacV003ManifestError('INVALID_MANIFEST_INPUT', 'manifest adoption input must be an object');
    }
    ...
    if (!isRecord(baseline) ||
        ...
        throw new DacV003ManifestError('UNSUPPORTED_MANIFEST_BASELINE', `manifest baseline must be exactly ${DAC_V003_BASELINE.contract}@${DAC_V003_BASELINE.version} semantic freeze commit ${DAC_V003_BASELINE.semanticFreezeCommit} / tree ${DAC_V003_BASELINE.semanticFreezeTree} (the single frozen DAC v0.0.3 baseline; no parallel manifest identity authority is forked)`);
    }
    if (input.contractVersion !== DAC_V003_MANIFEST_CONTRACT_VERSION) {
        throw new DacV003ManifestError('UNSUPPORTED_MANIFEST_CONTRACT_VERSION', `manifest contractVersion must be exactly "${DAC_V003_MANIFEST_CONTRACT_VERSION}"; got "${String(input.contractVersion)}"`);
    }
    ...
    if (!Array.isArray(input.selectedDomainData) || input.selectedDomainData.length === 0) {
        throw new DacV003ManifestError('SELECTED_DATA_CARDINALITY_ZERO', 'manifest must carry at least one authoritative selected Domain Data entry; a manifest that selects nothing cannot be composed and is never defaulted');
    }
    ...
    const { requirements, satisfactionEvidenceRefs } = validateRequirements(input);
...
function requireDigestOptions(options) {
    if (!isRecord(options) ||
        ...
        throw new DacV003ManifestError('INVALID_MANIFEST_INPUT', 'options must declare a sha256 port (digestUtf8) for exact manifest content identity derivation');
    }
    return options;
}
/**
 ...
 */
export async function computeDacV003ApplicationManifestDigest(input, options) {
 ...
 */
export async function adoptDacV003ApplicationManifest(input, options) {
    requireDigestOptions(options);
    ...
    if (declaredDigest === undefined || declaredDigest.length === 0) {
        throw new DacV003ManifestError('INVALID_MANIFEST_INPUT', 'manifest field "manifestContentDigest" must be a non-empty string; compute it with computeDacV003ApplicationManifestDigest and present content and digest together');
    }
    ...
    if (canonicalDigest !== declaredDigest) {
        throw new DacV003ManifestError('MANIFEST_DIGEST_MISMATCH', 'the declared manifest content digest does not match the canonical digest of the presented content; manifest content and declared digest must be adopted together exactly', [`declared=${declaredDigest}`, `canonical=${canonicalDigest}`]);
    }
    ...
    return mintV003Manifest(Object.freeze({
        ...record,
        manifestContentDigest: canonicalDigest,
    }));
}
/** True only for manifests actually adopted by `adoptDacV003ApplicationManifest`. */
export function isDacV003ApplicationManifest(value) {
    if (!isMintedV003Manifest(value))
        return false;
    const candidate = value;
    return (candidate.adapter === DAC_V003_MANIFEST_ADAPTER_VERSION &&
...
export function dacV003ManifestIdentityOf(manifest) {
    if (!isDacV003ApplicationManifest(manifest)) {
        throw new DacV003ManifestError('NOT_AN_ADOPTED_V003_MANIFEST', 'identity projection requires a manifest actually adopted by the DAC v0.0.3 manifest adapter');
    }
    return Object.freeze({
        applicationSemanticIdentity: manifest.applicationSemanticIdentity,
        applicationRevisionIdentity: manifest.applicationRevisionIdentity,
...
function sameTuple(left, right) {
    return (left.authorityScope === right.authorityScope &&
 ...
 */
export function associateDacV003ManifestCompatibilityValidation(manifest, validation) {
    if (!isDacV003ApplicationManifest(manifest)) {
        throw new DacV003ManifestError('NOT_AN_ADOPTED_V003_MANIFEST', 'validation association requires a manifest actually adopted by the DAC v0.0.3 manifest adapter');
    }
    if (!isDacV003CompatibilityValidationValue(validation)) {
        throw new DacV003ManifestError('INVALID_ASSOCIATION_INPUT', 'validation association requires a compatibility validation actually minted by the V3-002 compatibility authority; a forged lookalike or a bare reference can never stand in for the exact result');
    }
    const mismatch = (reason) => new DacV003ManifestError('ASSOCIATION_SUBJECT_MISMATCH', `the validation was not evaluated over this manifest's exact declared closure: ${reason}; an association references only the exact subject/target it was derived from`, [reason]);
    ...
    if (validation.subject.targetProfile !== manifestTargetKey) {
        throw mismatch(`validation target profile "${String(validation.subject.targetProfile)}" != manifest compatibility target key "${manifestTargetKey}"`);
    }
    ...
    if (!sameTuple(validation.ux.runtimeInteractionContract, manifest.ux.runtimeInteractionContract)) {
        throw mismatch('validation interaction-contract tuple differs from the manifest interaction contract');
    }
    ...
    const closureIdentities = new Set(validation.requirementClosure.map((entry) => entry.requirementIdentity));
    if (declaredRequirementIdentities.size !== closureIdentities.size ||
    ...
    const carriedEvidenceIdentities = new Set(manifest.satisfactionEvidenceRefs.map((evidence) => evidence.reference.primaryIdentity));
    for (const entry of validation.requirementClosure) {
        for (const evidenceIdentity of entry.satisfiedBy) {
            if (!carriedEvidenceIdentities.has(evidenceIdentity)) {
                throw mismatch(`validation recorded satisfying evidence "${evidenceIdentity}" the manifest does not carry`);
            }
        }
    }
...
export function isDacV003ManifestCompatibilityAssociation(value) {
    if (!isMintedValidationAssociation(value))
        return false;
    const candidate = value;
    return (candidate.manifestValidationAssociation ===

...
function hasExactOptionalKeys(record, required, optional) {
...
function normalizeDependency(value) {
    ...
    if (record === null || !hasExactOptionalKeys(record, ['kind', 'identity'], ['revision']))
        return null;
    if (record.kind !== 'domain-fact'
        && record.kind !== 'compiled-intelligence'
        && record.kind !== 'query')
        return null;
    if (typeof record.identity !== 'string' || record.identity.trim().length === 0)
        return null;
    if (record.revision !== undefined && (typeof record.revision !== 'string' || record.revision.trim().length === 0))
        return null;
    return {
        kind: record.kind,
...
function appendTrace(context, type, detail) {
    return [
...
function parseModelEnvelope(value) {
    const record = asRecord(value);
    if (record === null || typeof record.kind !== 'string')
        return null;
    if (record.kind === 'query' && hasExactKeys(record, ['kind', 'call']))
        return 'query';
    if (record.kind === 'final' && hasExactKeys(record, ['kind', 'result']))
        return 'final';
    return null;
}
function parseQueryCall(value) {
    const response = asRecord(value);
    if (response === null || response.kind !== 'query' || !hasExactKeys(response, ['kind', 'call'])) {
        return null;
    }
    const call = asRecord(response.call);
    if (call === null || !hasExactKeys(call, ['capabilityId', 'input']))
        return null;
    if (typeof call.capabilityId !== 'string' || call.capabilityId.trim().length === 0)
        return null;
...
function parseStructuredResult(value, allowedDecisionOutcomes, allowedEventTypes) {
    const response = asRecord(value);
    if (response === null || response.kind !== 'final' || !hasExactKeys(response, ['kind', 'result'])) {
        return null;
    }
    const result = asRecord(response.result);
    if (result === null || !hasExactKeys(result, ['decision', 'event']))
        return null;
    const decision = asRecord(result.decision);
    if (decision === null || !hasExactKeys(decision, ['outcome', 'data']))
        return null;
    ...
    if (event === null || !hasExactKeys(event, ['type', 'payload']))
        return null;
    if (typeof event.type !== 'string' || !allowedEventTypes.includes(event.type))
        return null;
...
function parseQueryExecutionResult(value) {
    const record = asRecord(value);
...
function validateUniqueNonEmpty(values, label) {
    ...
    for (const value of values) {
        if (typeof value !== 'string' || value.trim().length === 0)
            return `${label} contains an empty value`;
        if (seen.has(value))
            return `${label} contains duplicate value ${value}`;
        seen.add(value);
    }
    return null;
}
function configurationError(input) {
    ...
    if (eventError !== null)
        return eventError;
    if (canonicalObject(input.domainFacts) === null)
        return 'domainFacts must be a canonical JSON object';
    ...
    for (const capability of input.capabilities) {
        if (capability.capabilityId.trim().length === 0)
            return 'capabilityId must not be empty';
    ...
    for (const dependency of input.selectedDependencies ?? []) {
        if (normalizeDependency(dependency) === null)
            return 'selectedDependencies contains an invalid dependency';
    }
    return null;
}
...
function buildModelRequest(context) {
    return {
...
function findCapability(context, capabilityId) {
    return context.capabilities.find((capability) => capability.capabilityId === capabilityId);
}
function requirePendingCall(context) {
    if (context.pendingCall === null)
        throw new Error('HarnessMachine expected a pending query call');
    return context.pendingCall;
}
function requireQueryBinding(context) {
...
function requireStructuredResult(context) {
    const result = parseStructuredResult(context.lastResponse, context.allowedDecisionOutcomes, context.allowedEventTypes);
    if (result === null)
        throw new Error('HarnessMachine expected a validated structured result');
    return result;
}
function outputFrom(context) {
    ...
    const observedDependencies = { items: context.dependencies };
...
const modelTask = fromPromise(async ({ input, signal }) => input.model.generate(input.request, signal));
const queryTask = fromPromise(async ({ input, signal }) => {
    try {
        ...
        if (signal.aborted || error instanceof QueryOutputContractError)
            throw error;
        return {
            observation: {
                capabilityId: input.binding.capabilityId,
...
export const HARNESS_DIRECT_ACTOR_ROLES = ['modelTask', 'queryTask'];
/**
...
export const HarnessMachine = setup({
    ...
    guards: {
        ...
        structuredResultValid: ({ context }) => parseStructuredResult(context.lastResponse, context.allowedDecisionOutcomes, context.allowedEventTypes) !== null,
            ...
            always: [
                {
                    ...
                    target: 'failed',
                    actions: assign({
                        terminal: ({ context }) => failure('INVALID_CONFIGURATION', context.configurationError ?? 'invalid Business Harness configuration'),
                ...
                { guard: 'stepsAvailable', target: 'model' },
                {
                    target: 'failed',
                    actions: assign({
                        terminal: ({ context }) => failure('MAX_STEPS_EXHAUSTED', `Business Harness model step limit ${context.maxSteps} exhausted`),
                        traceEntries: ({ context }) => appendTrace(context, 'failed', {
        ...
        handleModel: {
            always: [
                ...
                },
                {
                    target: 'failed',
                    actions: assign({
                        terminal: () => failure('INVALID_MODEL_RESPONSE', 'model response must be exactly a query call or structured final result envelope'),
                        traceEntries: ({ context }) => appendTrace(context, 'failed', {
                            code: 'INVALID_MODEL_RESPONSE',
                        }),
                    }),
        ...
        authorizeQuery: {
            always: [
                { guard: 'queryCapabilityAllowed', target: 'query' },
                {
                    ...
                    target: 'failed',
                    actions: assign({
                        terminal: ({ context }) => failure('MUTATION_CAPABILITY_FORBIDDEN', `Business Harness cannot execute mutation capability ${requirePendingCall(context).capabilityId}`),
                        traceEntries: ({ context }) => appendTrace(context, 'failed', {
                ...
                },
                {
                    target: 'failed',
                    actions: assign({
                        terminal: ({ context }) => failure('UNKNOWN_CAPABILITY', `unknown or unallowed Business Harness capability ${requirePendingCall(context).capabilityId}`),
                        traceEntries: ({ context }) => appendTrace(context, 'failed', {
        ...
        validateFinal: {
            always: [
                ...
                },
                {
                    target: 'failed',
                    actions: assign({
                        terminal: () => failure('INVALID_STRUCTURED_RESULT', 'model final output is not an allowed exact DomainDecision/DomainEvent result'),
                        traceEntries: ({ context }) => appendTrace(context, 'failed', {
...
//# sourceMappingURL=harness-machine.js.map
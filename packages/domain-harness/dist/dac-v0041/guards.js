...
// binding authority.
import { DAC_V0041_BASELINE, DAC_V0041_CAPABILITY_OUTCOME_PRODUCES_RESULT, DAC_V0041_CURRENTNESS_USE_STATES, DAC_V0041_FOUNDATION_REQUEST_ROLES, DAC_V0041_PREDECESSOR_BASELINES, DAC_V0041_PREDECESSOR_EVIDENCE_PURPOSE, DAC_V0041_REFERENCE_ADAPTER_VERSION, DAC_V0041_REQUEST_RESULT_ALIAS_CHAINS, DAC_V0041_ROLE_REGISTRY, DacV0041ReferenceError, } from './contracts.js';
/**
...
function baselineEqualsSuccessor(input) {
    return (input.contract === DAC_V0041_BASELINE.contract &&
...
function matchPredecessorBaselineInput(input) {
    ...
    return (DAC_V0041_PREDECESSOR_BASELINES.find((baseline) => candidate.contract === baseline.contract &&
        candidate.version === baseline.version &&
        candidate.semanticFreezeCommit === baseline.semanticFreezeCommit &&
        candidate.semanticFreezeTree === baseline.semanticFreezeTree) ?? null);
}
function isPredecessorBaselineInput(input) {
    return matchPredecessorBaselineInput(input) !== null;
}
function requireNonEmptyString(value, field) {
    if (typeof value !== 'string' || value.trim().length === 0) {
        throw new DacV0041ReferenceError('INVALID_REFERENCE', `${field} must be a non-empty string`);
    }
}
function requireExactIdentityString(value, field) {
    requireNonEmptyString(value, field);
    if (isMutableAliasToken(value)) {
        throw new DacV0041ReferenceError('MUTABLE_ALIAS_REJECTED', `${field} "${value}" is a mutable alias (latest/current/head-style) and can never substitute an exact identity`);
    }
}
function requireBaseline(input) {
    ...
    if (!baselineEqualsSuccessor(input)) {
        throw new DacV0041ReferenceError('UNSUPPORTED_DAC_BASELINE', `baseline must equal the exact DAC v0.0.4.1 successor freeze ${DAC_V0041_BASELINE.semanticFreezeCommit} (got ${String(input.semanticFreezeCommit)}); predecessor identities are transition/adoption evidence only`);
    }
}
function requirePredecessorOrigin(origin) {
    if (origin === undefined)
        return;
    if (!isPredecessorBaselineInput(origin)) {
        throw new DacV0041ReferenceError('INVALID_PREDECESSOR_ORIGIN', 'predecessorOrigin must name exactly one frozen predecessor baseline (v0.0.3 or v0.0.4 semantic freeze) carrying the exact evidence-only purpose, and is retained for transition/adoption evidence only');
    }
}
/**
 ...
 */
function predecessorOriginSlot(origin) {
    if (origin === undefined)
        return {};
 ...
 * versa.
 */
const MINTED_V0041_REFERENCES = new WeakSet();
function structurallyMintedReference(value) {
    if (value === null || typeof value !== 'object')
        return false;
    if (!MINTED_V0041_REFERENCES.has(value))
        return false;
    const candidate = value;
    return (candidate.adapter === DAC_V0041_REFERENCE_ADAPTER_VERSION &&
        typeof candidate.role === 'string' &&
        DAC_V0041_ROLE_REGISTRY.indexOf(candidate.role) !== -1 &&
...
export function isDacV0041Reference(value) {
    return structurallyMintedReference(value);
}
function sharedEnvelopeSlots(input) {
 ...
 */
export function adoptDacV0041RegistryReference(input) {
    if (DAC_V0041_ROLE_REGISTRY.indexOf(input.role) === -1) {
        throw new DacV0041ReferenceError('ROLE_MISMATCH', `role "${String(input.role)}" is not in the DAC v0.0.4.1 successor role registry`);
    }
    if (DAC_V0041_FOUNDATION_REQUEST_ROLES.indexOf(input.role) !== -1) {
        throw new DacV0041ReferenceError('ROLE_MISMATCH', `role "${String(input.role)}" is a foundation request role with a dedicated nominal constructor; generic adoption is reserved to prevent bypassing the request-envelope minimums`);
    }
...
function requireMintedRequestSlot(ref, field) {
    if (ref === undefined)
        return;
    if (!structurallyMintedReference(ref)) {
        throw new DacV0041ReferenceError('INVALID_REFERENCE', `${field} must be a reference minted by the DAC v0.0.4.1 adoption core (foreign/forged carriers fail closed)`);
    }
}
function mintRequestReference(role, input) {
    sharedEnvelopeSlots(input);
    ...
    if (input.bindingTargetRef !== undefined &&
        input.bindingTargetRef.role !== 'compatibility-target') {
        throw new DacV0041ReferenceError('ADVISORY_HINT_NOT_BINDING_TARGET', `bindingTargetRef must carry role "compatibility-target" (got "${String(input.bindingTargetRef.role)}"); advisory target hints are non-authoritative metadata and can never occupy the binding explicit target slot`);
    }
    const reference = freezeAdopted({
        adapter: DAC_V0041_REFERENCE_ADAPTER_VERSION,
        baseline: DAC_V0041_BASELINE,
 ...
 */
export function mintCompatibilityValidationRequestRef(input) {
...
export function isRuntimeBindingRequestRef(value) {
    return structurallyMintedReference(value) && value.role === 'runtime-binding-request';
}
function aliasChainIndexOfRole(role) {
    for (let chain = 0; chain < DAC_V0041_REQUEST_RESULT_ALIAS_CHAINS.length; chain += 1) {
        const index = DAC_V0041_REQUEST_RESULT_ALIAS_CHAINS[chain].indexOf(role);
        if (index !== -1)
            return { chain, index };
    }
    return null;
 ...
 */
export function verifyDacV0041RequestResultSeparation(request, counterparts) {
    if (!structurallyMintedReference(request)) {
        throw new DacV0041ReferenceError('INVALID_REFERENCE', 'request must be a reference minted by the DAC v0.0.4.1 adoption core');
    }
    const requestChain = aliasChainIndexOfRole(request.role);
    if (requestChain === null) {
        throw new DacV0041ReferenceError('ROLE_MISMATCH', `role "${String(request.role)}" is not a request role of any frozen request/result alias chain`);
    }
    for (const counterpart of counterparts) {
        if (!structurallyMintedReference(counterpart)) {
            throw new DacV0041ReferenceError('INVALID_REFERENCE', 'counterpart must be a reference minted by the DAC v0.0.4.1 adoption core');
        }
        ...
        if (counterpart.role !== request.role &&
            counterpart.primaryIdentity === request.primaryIdentity) {
            throw new DacV0041ReferenceError('REQUEST_RESULT_ALIAS', `request role "${String(request.role)}" and result/decision role "${String(counterpart.role)}" alias the same identity "${request.primaryIdentity}"; request identity must remain distinct from every result/decision/definition identity in its seam chain`);
        }
    }
}
/**
 ...
 */
function isCapabilityKindString(value) {
    return typeof value === 'string' && value.trim().length > 0;
}
function isCapabilityExchangeFacts(value) {
    if (value === null || typeof value !== 'object')
        return false;
    const candidate = value;
    if (typeof candidate.currentDescriptorEstablished !== 'boolean')
        return false;
    if (!Array.isArray(candidate.currentDescriptorOfferedCapabilityKinds))
        return false;
    for (const offeredKind of candidate.currentDescriptorOfferedCapabilityKinds) {
        if (!isCapabilityKindString(offeredKind))
            return false;
    }
    if (!isCapabilityKindString(candidate.requestedCapabilityKind))
        return false;
    if (typeof candidate.requiredInputsStructurallyValid !== 'boolean')
        return false;
    if (typeof candidate.materialStaleness !== 'boolean')
        return false;
 ...
 */
export function classifyDacV0041CapabilityExchange(facts) {
    if (!isCapabilityExchangeFacts(facts)) {
        throw new DacV0041ReferenceError('INVALID_FACTS', 'capability-exchange facts are malformed; classification must use externally recoverable request/descriptor facts and never invents one');
    }
    ...
    const offered = facts.currentDescriptorOfferedCapabilityKinds.indexOf(facts.requestedCapabilityKind);
    if (offered === -1) {
    ...
    if (facts.bindingTargetState.presence === 'explicit' &&
 ...
 */
export function classifyDacV0041CurrentnessUse(state) {
    if (DAC_V0041_CURRENTNESS_USE_STATES.indexOf(state) === -1) {
        throw new DacV0041ReferenceError('INVALID_CURRENTNESS_STATE', `currentness state "${String(state)}" is not in the closed v0.0.4.1 vocabulary; undecidable currentness fails closed`);
    }
    switch (state) {
        case 'current':
 ...
 */
export function dacV0041OutcomeProducesResult(outcome) {
    const produces = DAC_V0041_CAPABILITY_OUTCOME_PRODUCES_RESULT[outcome];
    if (typeof produces !== 'boolean') {
        throw new DacV0041ReferenceError('INVALID_FACTS', `outcome class "${String(outcome)}" is not in the frozen Authoring/Capability Exchange outcome namespace`);
    }
    return produces;
}

...
export declare const RUNTIME_OBSERVATION_CONTRACT_VERSION: "runtime-observation/1";
/**
...
export type RuntimeObservationGapKind = 'RETENTION_TRUNCATED' | 'STORAGE_GAP' | 'CURSOR_INVALID';
/**
...
export interface RuntimeObservationStore extends RuntimeStore {
    createInstanceWithObservation(snapshot: WorkflowInstanceSnapshot, intent: RuntimeObservationIntent | undefined): Promise<readonly RuntimeObservationRecord[]>;
    acceptMessageWithObservation(message: DomainMessage, intent: RuntimeObservationIntent | undefined): Promise<{
        readonly ack: MessageAcceptedAck;
        readonly records: readonly RuntimeObservationRecord[];
    }>;
    commitProcessedMessageWithObservation(request: CommitProcessedMessageRequest, intent: RuntimeObservationIntent | undefined): Promise<readonly RuntimeObservationRecord[]>;
    failMessageProcessingWithObservation(request: FailMessageProcessingRequest, intent: RuntimeObservationIntent | undefined): Promise<readonly RuntimeObservationRecord[]>;
    resetRecoveryWithObservation(target: WorkflowAddress, updatedAt: string, intent: RuntimeObservationIntent | undefined): Promise<{
        readonly instance: WorkflowInstanceSnapshot;
        readonly records: readonly RuntimeObservationRecord[];
    }>;
    terminalizeInstanceWithObservation(request: TerminalizeInstanceRequest, intent: RuntimeObservationIntent | undefined): Promise<readonly RuntimeObservationRecord[]>;
...
export declare function isRuntimeObservationStore(store: unknown): store is RuntimeObservationStore;
/**
 ...
 */
export declare function runtimePackageIdentityFromManifest(manifest: CompiledPackageManifest): DomainIntelligencePackageIdentity;

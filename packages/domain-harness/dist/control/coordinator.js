...
import { RUNTIME_CONTROL_PROVENANCE_KIND, runtimeControlProvenanceOf, runtimeControlTerminalizeReason, } from './contracts.js';
...
function sameRequestIdentity(record, request) {
    return (record.callerRef === request.callerRef &&
...
export class RuntimeControlCoordinator {
    ...
    async requestControl(request) {
        ...
        if (decision.status !== 'AUTHORIZED') {
            ...
            const record = await this.#createResolved(request, authorization, 'REJECTED', 'REJECTED');
            return receiptOf(record);
        }
        if (request.action === 'PAUSE' || request.action === 'RESUME') {
            // Explicit UNSUPPORTED, no hidden semantics, no mutation.
            const record = await this.#createResolved(request, authorization, 'UNSUPPORTED_ACTION', 'UNSUPPORTED');
    ...
    async getControlOutcome(controlRequestId) {
    ...
    async #createResolved(request, authorization, disposition, outcome) {
        const created = await this.#controlStore.createRequest({
            controlRequestId: request.controlRequestId,
            action: request.action,
            target: request.target.target,
    ...
    async #executeCancel(record) {
        ...
        try {
            ...
            const resolved = await this.#resolveOnLane(record.target, () => this.#resolveCancelFacts(record));
    ...
    async #executeInterrupt(record) {
        const active = this.#ports.activeTurn(record.target);
        if (active === undefined) {
            // No live turn: classify from facts (NO_ACTIVE_TURN / TOO_LATE / etc.).
            const resolved = await this.#resolveOnLane(record.target, () => this.#resolveInterruptFacts(record));
            return receiptOf(resolved);
        }
        ...
        const claiming = await this.#controlStore.transitionRequest(record.controlRequestId, 'accepted', {
            status: 'resolving',
            claimedTurn: claimed,
            signalIssued: true,
        });
    ...
    async #resolveInterruptFacts(record, claimed) {
        ...
        if (instance.lifecycle === 'recovery_required') {
            ...
            if (provenance?.kind === RUNTIME_CONTROL_PROVENANCE_KIND &&
    ...
    async #resolveCancelFacts(record) {
        const instance = await this.#ports.store.getInstance(record.target);
        if (instance === null) {
            return this.#resolve(record, 'REJECTED', 'REJECTED', {});
        }
        ...
        if (isTerminal(instance.lifecycle)) {
            const provenance = runtimeControlProvenanceOf(instance);
            if (instance.lifecycle === 'cancelled' &&
                provenance?.controlRequestId === record.controlRequestId) {
    ...
    async #effectEvidenceFor(effectId) {
    ...
    async #resolve(record, disposition, outcome, extra) {
        try {
            return await this.#controlStore.transitionRequest(record.controlRequestId, record.status, {
                status: 'resolved',
                receiptDisposition: disposition,
                outcome,
                resolvedAt: this.#ports.now(),
            ...
            if (current !== null)
                return current;
            throw new Error(`Runtime control record ${record.controlRequestId} disappeared during resolution`);
        }
    }
}
function turnSelectorMatches(expected, active) {
    if (expected === undefined)
        return true;
    if (expected.messageId !== active.messageId)
        return false;
    return expected.targetSequence === undefined || expected.targetSequence === active.targetSequence;
}
function authorizationEvidence(callerRef, decision) {
    if (decision.status === 'AUTHORIZED') {
        return {
            decision: 'AUTHORIZED',
            callerRef,
...
function receiptOf(record, overrideDisposition) {
    return {
        controlRequestId: record.controlRequestId,

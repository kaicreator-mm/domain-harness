...
import { PromotedArtifactContractError, } from './contracts.js';
import { assertValidatedCandidateAuthority, createPromotedArtifactBody, normalizePromotedArtifactAuthority, samePromotedArtifactAuthority, samePromotedArtifactIdentity, verifyPromotedArtifactBody, } from './identity.js';
...
function versionKey(artifactId, version) {
    return `${artifactId}\u0000${version}`;
}
function aliasKey(artifactId, alias) {
...
function sameRetention(left, right) {
...
function assertVersionBinding(binding, artifactId, version) {
    if (binding.artifactId !== artifactId
        || binding.version !== version
        || binding.artifact.artifactId !== artifactId) {
        throw new PromotedArtifactContractError('INVALID_PROMOTED_ARTIFACT', `corrupt exact-version binding returned for ${artifactId}@${version}`);
    }
}
function assertAliasBinding(binding, artifactId, alias) {
    if (binding.artifactId !== artifactId
        || binding.alias !== alias
        || binding.artifact.artifactId !== artifactId
        || !Number.isInteger(binding.revision)
        || binding.revision <= 0) {
        throw new PromotedArtifactContractError('INVALID_PROMOTED_ARTIFACT', `corrupt alias binding returned for ${artifactId}:${alias}`);
    }
}
function assertRevocationRecord(record, identity) {
    ...
    if (!samePromotedArtifactIdentity(record.artifact, identity)) {
        throw new PromotedArtifactContractError('PROMOTED_ARTIFACT_DIGEST_MISMATCH', 'registry returned revocation evidence for a different exact artifact');
    }
    if (record.revocationPolicy !== 'deny' && record.revocationPolicy !== 'fallthrough') {
        throw new PromotedArtifactContractError('INVALID_PROMOTED_ARTIFACT', 'revocation policy is invalid');
    }
    if (record.cachePolicy !== 'invalidate-produced-results' && record.cachePolicy !== 'preserve') {
        throw new PromotedArtifactContractError('INVALID_PROMOTED_ARTIFACT', 'revocation cache policy is invalid');
    }
}
/** Portable deterministic reference store; it does not claim host durability. */
export class MemoryPromotedArtifactStore {
    bodies = new Map();
    promotions = new Map();
    promotionRecordOwners = new Map();
    versions = new Map();
    aliases = new Map();
    revocations = new Map();
    liveRetentions = new Map();
    retentionTombstones = new Map();
    async getBody(identity) {
        const body = this.bodies.get(identityKey(identity));
        return body === undefined ? undefined : cloneJson(body);
    }
    async getPromotionRecords(identity) {
    ...
    async commitPromotion(body, promotion, versionBinding) {
        const bodyKey = identityKey(body.identity);
        if (!samePromotedArtifactIdentity(body.identity, promotion.artifact)
            ...
            throw new PromotedArtifactContractError('INVALID_PROMOTED_ARTIFACT', 'promotion transaction identities do not agree');
        }
        const existingBody = this.bodies.get(bodyKey);
        if (existingBody !== undefined && !sameJson(existingBody, body)) {
            throw new PromotedArtifactContractError('PROMOTED_ARTIFACT_BODY_CONFLICT', `immutable promoted body conflict for ${body.identity.artifactId}@${body.identity.contentDigest}`);
        }
        ...
        if (existingVersion !== undefined
            ...
            throw new PromotedArtifactContractError('PROMOTED_ARTIFACT_VERSION_REBIND', `version ${versionBinding.artifactId}@${versionBinding.version} is already bound to another digest`);
        }
        ...
        if (recordOwner !== undefined && !sameJson(recordOwner, promotion)) {
            throw new PromotedArtifactContractError('PROMOTED_ARTIFACT_BODY_CONFLICT', `promotion record ${promotion.recordId} cannot be rebound`);
        }
        // No mutation above this point: commit all three authorities together.
        if (existingBody === undefined)
            this.bodies.set(bodyKey, cloneJson(body));
        if (existingVersion === undefined)
            this.versions.set(logicalVersionKey, cloneJson(versionBinding));
        if (recordOwner === undefined) {
            this.promotionRecordOwners.set(promotion.recordId, cloneJson(promotion));
            const byArtifact = this.promotions.get(bodyKey) ?? new Map();
    ...
    async getAlias(artifactId, alias) {
        const binding = this.aliases.get(aliasKey(artifactId, alias));
        return binding === undefined ? undefined : cloneJson(binding);
    }
    async putAlias(input) {
        if (!Number.isInteger(input.expectedRevision) || input.expectedRevision < 0) {
            throw new PromotedArtifactContractError('INVALID_PROMOTED_ARTIFACT', 'alias expectedRevision must be a non-negative integer');
        }
        const key = aliasKey(input.artifactId, input.alias);
        const current = this.aliases.get(key);
        const currentRevision = current?.revision ?? 0;
        if (input.expectedRevision !== currentRevision) {
            throw new PromotedArtifactContractError('PROMOTED_ARTIFACT_STALE_SELECTION', `alias ${input.artifactId}:${input.alias} expected revision ${input.expectedRevision}, current ${currentRevision}`);
        }
        const next = {
    ...
    async putRevocation(record) {
        ...
        if (existing !== undefined && !sameJson(existing, record)) {
            throw new PromotedArtifactContractError('PROMOTED_ARTIFACT_REVOCATION_CONFLICT', `artifact ${record.artifact.artifactId}@${record.artifact.contentDigest} already has different revocation evidence`);
        }
        if (existing === undefined)
            this.revocations.set(key, cloneJson(record));
    }
    async getRetention(referenceId) {
        const reference = this.liveRetentions.get(referenceId);
        return reference === undefined ? undefined : cloneJson(reference);
    }
    async listRetentions(identity) {
    ...
    async putRetention(reference) {
        const released = this.retentionTombstones.get(reference.referenceId);
        if (released !== undefined) {
            throw new PromotedArtifactContractError('PROMOTED_ARTIFACT_RETENTION_REBIND', `released retention reference ${reference.referenceId} is tombstoned`);
        }
        const current = this.liveRetentions.get(reference.referenceId);
        if (current !== undefined) {
            if (sameRetention(current, reference))
                return;
            throw new PromotedArtifactContractError('PROMOTED_ARTIFACT_RETENTION_REBIND', `retention reference ${reference.referenceId} cannot be rebound`);
        }
        if (!this.bodies.has(identityKey(reference.artifact))) {
            throw new PromotedArtifactContractError('PROMOTED_ARTIFACT_NOT_FOUND', `cannot retain missing artifact ${reference.artifact.artifactId}@${reference.artifact.contentDigest}`);
        }
        this.liveRetentions.set(reference.referenceId, cloneJson(reference));
    }
    async releaseRetention(expected) {
        ...
        if (current === undefined) {
            ...
            if (tombstone !== undefined && !sameRetention(tombstone, expected)) {
                throw new PromotedArtifactContractError('PROMOTED_ARTIFACT_STALE_RELEASE', `released retention reference ${expected.referenceId} does not match stale caller authority`);
            }
            return 'absent';
        }
        if (!sameRetention(current, expected)) {
            throw new PromotedArtifactContractError('PROMOTED_ARTIFACT_STALE_RELEASE', `retention reference ${expected.referenceId} changed before release`);
        }
...
export class PromotedArtifactRegistry {
    store;
    sha256;
    producerInvalidation;
    constructor(store, sha256, producerInvalidation) {
        this.store = store;
        this.sha256 = sha256;
        this.producerInvalidation = producerInvalidation;
    }
    async load(identity, expectedAuthority, options) {
        const body = await this.store.getBody(identity);
        if (body === undefined) {
            throw new PromotedArtifactContractError('PROMOTED_ARTIFACT_NOT_FOUND', `promoted artifact ${identity.artifactId}@${identity.contentDigest} is missing`);
        }
        if (!samePromotedArtifactIdentity(body.identity, identity)) {
            throw new PromotedArtifactContractError('PROMOTED_ARTIFACT_DIGEST_MISMATCH', 'registry returned a body under a different exact identity');
        }
        await verifyPromotedArtifactBody(body, this.sha256);
        const records = await this.store.getPromotionRecords(identity);
        if (records.length === 0) {
            throw new PromotedArtifactContractError('PROMOTED_ARTIFACT_NOT_PROMOTED', `artifact ${identity.artifactId}@${identity.contentDigest} has no committed promotion record`);
        }
        for (const record of records) {
            ...
            if (!samePromotedArtifactIdentity(record.artifact, identity)) {
                throw new PromotedArtifactContractError('PROMOTED_ARTIFACT_DIGEST_MISMATCH', 'registry returned promotion evidence for a different exact artifact');
            }
        ...
        if (revocation !== undefined) {
            ...
            if (!options.allowRevoked) {
                throw new PromotedArtifactContractError('PROMOTED_ARTIFACT_REVOKED', `artifact ${identity.artifactId}@${identity.contentDigest} is revoked for fresh selection (${revocation.revocationPolicy})`, identity);
            }
        }
        ...
        if (promotion === undefined) {
            throw new PromotedArtifactContractError('PROMOTED_ARTIFACT_AUTHORITY_MISMATCH', `artifact ${identity.artifactId}@${identity.contentDigest} is not promoted for the exact package/CDI/Governance authority`);
        }
    ...
    async promote(input) {
        ...
        let recomputedCandidateDigest;
        try {
            recomputedCandidateDigest = await computeCanonicalJsonDigest(input.semanticMaterial, this.sha256);
        }
        catch (error) {
            throw new PromotedArtifactContractError('PROMOTION_VALIDATION_DRIFT', `promotion could not recompute validated Candidate semantic digest: ${error instanceof Error ? error.message : String(error)}`);
        }
        if (recomputedCandidateDigest !== input.validation.identity.candidateContentDigest) {
            throw new PromotedArtifactContractError('PROMOTION_VALIDATION_DRIFT', 'promotion semantic material no longer matches the exact Candidate digest produced by deterministic validation');
        }
    ...
    async resolveExact(identity, expectedAuthority) {
     ...
     */
    async recoverExact(identity, expectedAuthority) {
     ...
     */
    async resolveExactDetailed(identity, expectedAuthority) {
    ...
    async selectVersion(input) {
        ...
        if (binding === undefined) {
            throw new PromotedArtifactContractError('PROMOTED_ARTIFACT_VERSION_NOT_FOUND', `version ${input.artifactId}@${input.version} is not bound`);
        }
    ...
    async bindAlias(input) {
        ...
        if (input.artifact.artifactId !== input.artifactId) {
            throw new PromotedArtifactContractError('INVALID_PROMOTED_ARTIFACT', 'alias logical artifactId must match exact artifact identity');
        }
        const body = await this.store.getBody(input.artifact);
        if (body === undefined) {
            throw new PromotedArtifactContractError('PROMOTED_ARTIFACT_NOT_FOUND', 'alias cannot target a missing promoted body');
        }
        await verifyPromotedArtifactBody(body, this.sha256);
        if ((await this.store.getPromotionRecords(input.artifact)).length === 0) {
            throw new PromotedArtifactContractError('PROMOTED_ARTIFACT_NOT_PROMOTED', 'alias cannot target an unpromoted body');
        }
        const revocation = await this.store.getRevocation(input.artifact);
        if (revocation !== undefined) {
            assertRevocationRecord(revocation, input.artifact);
            throw new PromotedArtifactContractError('PROMOTED_ARTIFACT_REVOKED', 'alias cannot target a revoked artifact', input.artifact);
        }
        ...
        if (!samePromotedArtifactIdentity(bound.artifact, input.artifact)) {
            throw new PromotedArtifactContractError('PROMOTED_ARTIFACT_STALE_SELECTION', 'alias store returned an exact artifact different from the requested binding');
        }
        return bound;
    }
    async selectAlias(input) {
        const alias = await this.store.getAlias(input.artifactId, input.alias);
        if (alias === undefined) {
            throw new PromotedArtifactContractError('PROMOTED_ARTIFACT_ALIAS_NOT_FOUND', `alias ${input.artifactId}:${input.alias} is not bound`);
        }
        ...
        if (input.expectedRevision !== undefined && input.expectedRevision !== alias.revision) {
            throw new PromotedArtifactContractError('PROMOTED_ARTIFACT_STALE_SELECTION', `alias ${input.artifactId}:${input.alias} moved from expected revision ${input.expectedRevision} to ${alias.revision}`);
        }
    ...
    async revoke(identity, input) {
        ...
        const body = await this.store.getBody(identity);
        if (body === undefined) {
            throw new PromotedArtifactContractError('PROMOTED_ARTIFACT_NOT_FOUND', 'cannot revoke a missing artifact');
        }
        await verifyPromotedArtifactBody(body, this.sha256);
        if ((await this.store.getPromotionRecords(identity)).length === 0) {
            throw new PromotedArtifactContractError('PROMOTED_ARTIFACT_NOT_PROMOTED', 'cannot revoke a body without committed promotion authority');
        }
        const record = {
        ...
        if (record.cachePolicy === 'invalidate-produced-results' && this.producerInvalidation !== undefined) {
            try {
                ...
                throw new PromotedArtifactContractError('PROMOTED_ARTIFACT_INVALIDATION_FAILED', `revocation is committed but producer-result invalidation failed: ${error instanceof Error ? error.message : String(error)}`);
            }
        }
        return record;
    }
    ...
    async resolveRetained(referenceId) {
        requireNonEmpty(referenceId, 'retention.referenceId');
        const reference = await this.store.getRetention(referenceId);
        if (reference === undefined) {
            throw new PromotedArtifactContractError('PROMOTED_ARTIFACT_RETENTION_NOT_FOUND', `retention reference ${referenceId} is not live`);
        }
        if (reference.referenceId !== referenceId) {
            throw new PromotedArtifactContractError('INVALID_PROMOTED_ARTIFACT', 'retention store returned a different reference identity');
        }
        const authorityBinding = normalizePromotedArtifactAuthority(reference.authorityBinding);

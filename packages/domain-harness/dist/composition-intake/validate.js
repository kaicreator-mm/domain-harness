...
function requireNonEmptyString(value, field) {
    if (typeof value !== 'string' || value.trim().length === 0) {
        throw new CompositionIntakeError('INVALID_COMPOSITION_INTAKE', `environment declaration field "${field}" must be a non-empty string`);
    }
}
function requireMajor(value, field) {
    if (typeof value !== 'number' || !Number.isInteger(value) || value < 0) {
        throw new CompositionIntakeError('INVALID_COMPOSITION_INTAKE', `environment declaration field "${field}" must be a non-negative integer`);
    }
    return value;
}
function requireCapabilities(value) {
    if (!Array.isArray(value)) {
        throw new CompositionIntakeError('INVALID_COMPOSITION_INTAKE', 'environment declaration field "hostCapabilities" must be an array');
    }
    for (const capability of value) {
        if (typeof capability !== 'string' || !CAPABILITY_ID_PATTERN.test(capability)) {
            throw new CompositionIntakeError('INVALID_COMPOSITION_INTAKE', `environment host capability "${String(capability)}" is not a valid name@major capability id`);
        }
    }
    return value;
}
...
function validateEnvironment(environment) {
    if (!isRecord(environment)) {
        throw new CompositionIntakeError('INVALID_COMPOSITION_INTAKE', 'environment declaration must be an object');
    }
    ...
    if (!isRecord(environment.implementation) ||
        !isRecord(environment.sha256) ||
        typeof environment.sha256.digestUtf8 !== 'function') {
        throw new CompositionIntakeError('INVALID_COMPOSITION_INTAKE', 'environment must declare a concrete runtime implementation and a sha256 port');
    }
 ...
 */
function canonicalCompatibilityTargetMaterial(environment) {
...
export async function computeCompatibilityTargetDigest(environment) {
    validateEnvironment(environment);
    return environment.sha256.digestUtf8(canonicalCompatibilityTargetMaterial(environment));
}
function provenanceMismatch(detail) {
    throw new CompositionIntakeError('PROVENANCE_CHAIN_MISMATCH', `composition provenance contradicts the exact selected identity: ${detail}`);
}
/**
 ...
 * ```
 */
function validateProvenanceChain(promotion, selection, selected) {
    if (promotion.semanticIdentity !== selected.semanticIdentity) {
        provenanceMismatch(`promotion decision names semantic identity "${promotion.semanticIdentity}" but the selected identity is "${selected.semanticIdentity}"`);
    }
    if (promotion.revisionIdentity === undefined) {
        ...
        provenanceMismatch(`promotion decision pins revision "${promotion.revisionIdentity}" but the selected revision is "${selected.revisionIdentity}"`);
    }
    if (promotion.contentDigest !== undefined && promotion.contentDigest !== selected.contentDigest) {
        provenanceMismatch(`promotion decision digest "${promotion.contentDigest}" contradicts the selected digest "${selected.contentDigest}"`);
    }
    if (selection.semanticIdentity !== selected.semanticIdentity) {
        provenanceMismatch(`application selection names semantic identity "${selection.semanticIdentity}" but the selected identity is "${selected.semanticIdentity}"`);
    }
    if (selection.revisionIdentity === undefined) {
        ...
        provenanceMismatch(`application selection pins revision "${selection.revisionIdentity}" but the selected revision is "${selected.revisionIdentity}"`);
    }
    if (selection.contentDigest === undefined) {
        ...
        provenanceMismatch(`application selection digest "${selection.contentDigest}" contradicts the selected digest "${selected.contentDigest}"`);
    }
}
/** N08: the contract and implementation refs must not share one identity tuple. */
function rejectRoleIdentityCollapse(runtimeContract, runtimeImplementation) {
    if (runtimeContract.semanticIdentity === runtimeImplementation.semanticIdentity &&
        ...
        throw new CompositionIntakeError('ROLE_IDENTITY_COLLAPSE', 'runtime contract and runtime implementation references carry the same full identity tuple; ' +
 ...
 */
function validateSelectedPackageMapping(selected, compiledPackage) {
    const wrapper = compiledPackage;
    if (!isRecord(wrapper) || !isRecord(wrapper.manifest)) {
        throw new CompositionIntakeError('INVALID_COMPOSITION_PACKAGE', 'compiled package manifest is missing or malformed; exact selected mapping cannot be validated');
    }
    const declared = wrapper.manifest;
    for (const field of ['domainId', 'domainVersion', 'packageId']) {
        if (typeof declared[field] !== 'string' || declared[field].length === 0) {
            throw new CompositionIntakeError('INVALID_COMPOSITION_PACKAGE', `compiled package manifest identity field "${field}" is missing or malformed; exact selected mapping cannot be validated`);
        }
    }
    const mismatches = [];
    if (selected.semanticIdentity !== declared.domainId) {
        mismatches.push(`semanticIdentity: selected "${selected.semanticIdentity}" but compiled package domainId "${declared.domainId}"`);
    }
    if (selected.revisionIdentity !== declared.domainVersion) {
        mismatches.push(`revisionIdentity: selected "${selected.revisionIdentity}" but compiled package domainVersion "${declared.domainVersion}"`);
    }
    if (selected.contentDigest !== declared.packageId) {
        mismatches.push(`contentDigest: selected "${selected.contentDigest}" but compiled package packageId "${declared.packageId}"`);
    }
    if (mismatches.length > 0) {
        throw new CompositionIntakeError('SELECTED_IDENTITY_MISMATCH', 'the selected Domain Data identity does not map exactly onto the concrete compiled package', mismatches);
    }
}
/**
 ...
 */
async function validatePackageAgainstEnvironment(request) {
    try {
        ...
        if (!(error instanceof PackageActivationError))
            throw error;
        if (error.code === 'INCOMPATIBLE_PACKAGE') {
            throw new CompositionIntakeError('INCOMPATIBLE_SELECTED_COMPOSITION', 'the selected compiled package is incompatible with the declared runtime compatibility ' +
                'target; explicit upstream reselection is required and the runtime never substitutes ' +
                'latest, default or another revision', error.details);
        }
        throw new CompositionIntakeError('INVALID_COMPOSITION_PACKAGE', `the supplied compiled package failed integrity validation: ${error.message}`, [`packageActivationCode=${error.code}`, ...error.details]);
    }
}
function validateRuntimeReferenceBindings(request, targetDigest) {
    const { runtimeContract, runtimeImplementation, compatibilityTarget, environment } = request;
    if (runtimeContract.revisionIdentity === undefined ||
        runtimeContract.revisionIdentity !== String(environment.runtimeContractMajor)) {
        throw new CompositionIntakeError('RUNTIME_CONTRACT_MISMATCH', 'the declared runtime contract revision does not match the concrete runtime contract major', [
    ...
    if (runtimeImplementation.semanticIdentity !== implementation.identity) {
        implementationMismatches.push(`identity: declared "${runtimeImplementation.semanticIdentity}" but concrete "${implementation.identity}"`);
    }
    if (runtimeImplementation.revisionIdentity === undefined) {
        ...
        implementationMismatches.push(`version: declared "${runtimeImplementation.revisionIdentity}" but concrete "${implementation.version}"`);
    }
    if (runtimeImplementation.contentDigest === undefined) {
        ...
        implementationMismatches.push(`build: declared "${runtimeImplementation.contentDigest}" but concrete "${implementation.build}"`);
    }
    if (implementationMismatches.length > 0) {
        throw new CompositionIntakeError('RUNTIME_IMPLEMENTATION_MISMATCH', 'the declared concrete runtime implementation identity/version/build does not match the environment', implementationMismatches);
    }
    const targetMismatches = [];
    if (compatibilityTarget.revisionIdentity === undefined ||
        compatibilityTarget.revisionIdentity !== environment.targetProfileId) {
        targetMismatches.push(`targetProfileId: declared "${compatibilityTarget.revisionIdentity ?? '<absent>'}" but concrete "${environment.targetProfileId}"`);
    }
    if (compatibilityTarget.contentDigest !== undefined &&
        compatibilityTarget.contentDigest !== targetDigest) {
        targetMismatches.push(`contentDigest: declared "${compatibilityTarget.contentDigest}" but canonical target digest "${targetDigest}"`);
    }
    if (targetMismatches.length > 0) {
        throw new CompositionIntakeError('COMPATIBILITY_TARGET_MISMATCH', 'the declared compatibility target does not match the concrete validated target', targetMismatches);
    }
}
 ...
 */
export async function validateSelectedComposition(request) {
    if (!isRecord(request)) {
        throw new CompositionIntakeError('INVALID_COMPOSITION_INTAKE', 'composition intake request must be an object');
    }
    ...
    validateProvenanceChain(request.promotionDecision, request.applicationSelection, request.selectedDomainData);
    ...
    const emittedCompatibilityTarget = adoptCompatibilityTargetRef({

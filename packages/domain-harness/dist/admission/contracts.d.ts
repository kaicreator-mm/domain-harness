...
export type AdmissionTurnSource = {
    ...
    readonly durableRecoveryActionId: string;
...
export type CentralAdmissionErrorCode = 'ADMISSION_INVALID_TURN_SOURCE' | 'ADMISSION_UNKNOWN_STATE' | 'ADMISSION_UNKNOWN_GUARD' | 'ADMISSION_INVALID_PREDICATE_SHAPE' | 'ADMISSION_EVALUATION_INPUT_INVALID' | 'ADMISSION_PINNED_BASELINE_UNAVAILABLE' | 'ADMISSION_INVALID_HARD_INVARIANTS' | 'ADMISSION_EFFECT_TOOL_UNBOUND' | 'ADMISSION_EFFECT_JOURNAL_CONFLICT' | 'ADMISSION_EFFECT_AMBIGUOUS' | 'ADMISSION_EFFECT_FAILED';
export declare class CentralAdmissionError extends Error {
    readonly code: CentralAdmissionErrorCode;
    constructor(code: CentralAdmissionErrorCode, message: string);
}
export type AdmissionDenialReason = 'schema' | 'hard-invariant' | 'guard' | 'no-candidate-transition';
...
export type CentralAdmissionOutcome = {
    ...
    readonly admitted: AdmittedAdmission;
} | {

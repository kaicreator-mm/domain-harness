...
export function projectionTargetKey(projectionId, key) {
    return `${projectionId}/${key}`;
}
function authoritativeFacts(observation) {
    switch (observation.kind) {
        case 'workflow-instance': {
            ...
            if (typeof snapshot.stateRevision !== 'number' ||
                ...
                throw new DacBridgeError('INVALID_REFERENCE', 'observation.snapshot.stateRevision must be a safe integer');
            }
        ...
        case 'projection': {
            ...
            if (snapshot === null ||
                ...
                throw new DacBridgeError('INVALID_REFERENCE', 'observation.snapshot must be a ProjectionSnapshot with a non-empty revision');
            }
            return {
        ...
        case 'revision': {
            if (observation === null ||
                ...
                throw new DacBridgeError('INVALID_REFERENCE', 'revision observation requires a non-empty revision');
            }
            return {
        ...
        default:
            throw new DacBridgeError('INVALID_REFERENCE', `unknown authoritative observation kind "${String(observation.kind)}"`);
    }
}
function snapshotRefFacts(snapshotRef) {
 ...
 */
export function resolvedObservedBasis(correlation) {
 ...
 */
export function classifyObservedBasis(correlation, authoritative, options) {
    ...
    if (basis === undefined) {
        if (options.requireBasis) {
            throw new DacBridgeError('BASIS_REQUIRED', 'this operation requires an observed basis (stale-observation semantics) and none was correlated; fail closed instead of assuming current');
        }
    ...
    if (observed.sourceKind !== facts.sourceKind) {
        throw new DacBridgeError('TARGET_MISMATCH', `observed basis source kind "${observed.sourceKind}" cannot be classified against authoritative source kind "${facts.sourceKind}"; a reference is never silently reinterpreted`);
    }
    if (observed.targetKey !== undefined &&
        facts.targetKey !== undefined &&
        observed.targetKey !== facts.targetKey) {
        throw new DacBridgeError('TARGET_MISMATCH', `observed basis target "${observed.targetKey}" does not match authoritative target "${facts.targetKey}"; a reference is never silently reinterpreted`);
    }
...
//# sourceMappingURL=stale-basis.js.map
...
import { adoptDacV003RegistryReference, assertDacV003ExactnessProfile, isDacV003Reference, } from '../dac-v003/guards.js';
import { isDomainIntentRef, isOutcomeRef, isSemanticTargetRef, isSnapshotRef, isViewRef, isWatchRef, } from '../dac-bridge/guards.js';
import { isRuntimeActivationRef, isRuntimeBindingRef } from '../dac/guards.js';
import { DAC_V003_COMPATIBILITY_AUTHORITY_SCOPE, DacV003CompatibilityError, } from './contracts.js';
...
function requireNonEmptyString(value, field) {
    if (typeof value !== 'string' || value.trim().length === 0) {
        throw new DacV003CompatibilityError('INVALID_COMPATIBILITY_REQUEST', `${field} must be a non-empty string`);
    }
}
function requireAdopted(value, field) {
    if (!isDacV003Reference(value)) {
        throw new DacV003CompatibilityError('INVALID_COMPATIBILITY_REQUEST', `${field} must be an adopted DAC v0.0.3 reference; foreign, v0.0.2 or forged objects fail closed`);
    }
    return value;
}
 ...
 */
const MINTED_CAPABILITY_REQUIREMENTS = new WeakSet();
const MINTED_PORT_REQUIREMENTS = new WeakSet();
const MINTED_SATISFACTION_EVIDENCE = new WeakSet();
const MINTED_COMPATIBILITY_VALIDATIONS = new WeakSet();
const MINTED_COMPATIBILITY_RESULTS = new WeakSet();
/**
 ...
 */
export function adoptDomainUXDefinitionRef(input) {
 ...
 */
export function adoptDacV003CompatibilityTargetRef(input) {
...
export function dacV003TargetProfileKey(target) {
    return `${target.contractProfileIdentity}@${target.revisionIdentity}`;
}
function validateStrength(input) {
    if (input.strength !== 'required' &&
        input.strength !== 'optional' &&
        input.strength !== 'conditional') {
        throw new DacV003CompatibilityError('INVALID_COMPATIBILITY_REQUEST', `requirement strength must be required/optional/conditional, got "${String(input.strength)}"`);
    }
    if (input.strength === 'conditional') {
        ...
        throw new DacV003CompatibilityError('INVALID_COMPATIBILITY_REQUEST', 'a condition descriptor may only accompany a conditional requirement');
    }
    if (input.qualifications !== undefined) {
        if (!Array.isArray(input.qualifications)) {
            throw new DacV003CompatibilityError('INVALID_COMPATIBILITY_REQUEST', 'requirement qualifications must be an array');
        }
 ...
 */
export function adoptDacV003CapabilityRequirement(input) {
 ...
 */
export function adoptDacV003PortRequirement(input) {
 ...
 */
export function adoptDacV003RequirementSatisfactionEvidence(input) {
    ...
    if (!SATISFIABLE_ROLES.has(requirement.role)) {
        throw new DacV003CompatibilityError('INVALID_SATISFACTION_EVIDENCE', `satisfaction evidence must link a capability/port/host-binding requirement, but the linked reference has role "${requirement.role}"`);
    }
    const provider = requireAdopted(input.provider, 'satisfaction evidence provider');
    if (UX_FAMILY_ROLES.has(provider.role)) {
        throw new DacV003CompatibilityError('PRESENTATION_ADAPTER_CANNOT_SATISFY', `a "${provider.role}" reference is UX/presentation-family and can never satisfy a Runtime capability/Port/Host-Binding requirement; a renderer adapter does not acquire Runtime effect authority by accessing the same host platform`);
    }
    if (!PROVIDER_ROLES.has(provider.role)) {
        throw new DacV003CompatibilityError('INVALID_SATISFACTION_EVIDENCE', `satisfaction evidence provider must be a concrete runtime-host-binding or runtime-implementation reference, but has role "${provider.role}"`);
    }
    ...
    if (!Array.isArray(input.provenance) || input.provenance.length === 0) {
        throw new DacV003CompatibilityError('INVALID_SATISFACTION_EVIDENCE', 'satisfaction evidence requires at least one provenance reference establishing how satisfaction was determined (P6)');
    }
    const provenanceRefs = input.provenance.map((entry, index) => requireAdopted(entry, `satisfaction evidence provenance[${index}]`));
...
export function isDomainUXDefinitionRefValue(value) {
...
export function isDacV003CompatibilityTargetRefValue(value) {
...
export function isDacV003CompatibilityValidationRefValue(value) {
    return (isDacV003Reference(value) &&
...
export function isDacV003CompatibilityResultRefValue(value) {
    return (isDacV003Reference(value) &&
        value.role === 'compatibility-result' &&
        value.authorityScope === DAC_V003_COMPATIBILITY_AUTHORITY_SCOPE);
}
/** True only for validation records actually minted by the validator. */
export function isDacV003CompatibilityValidationValue(value) {
 ...
 */
export function refuteDacV003LifecycleBindingInput(...values) {
    for (const value of values) {
        if (isRuntimeBindingRef(value) || isRuntimeActivationRef(value)) {
            throw new DacV003CompatibilityError('LIFECYCLE_BINDING_INPUT_REJECTED', 'a v0.0.2 lifecycle RuntimeBinding/Activation reference is not a host-binding requirement, concrete host binding, or compatibility-validation input; RuntimeBindingRef is a later lifecycle stage and the roles are never interchangeable');
        }
        if (isRuntimeBindingEvidenceValue(value)) {
            throw new DacV003CompatibilityError('LIFECYCLE_BINDING_INPUT_REJECTED', '#307 runtime binding evidence is stage-4 lifecycle evidence and can never be presented into the stage-3 compatibility path');
        }
        if (isDacV003Reference(value) &&
            (value.role === 'runtime-binding' || value.role === 'runtime-activation')) {
            throw new DacV003CompatibilityError('LIFECYCLE_BINDING_INPUT_REJECTED', `a v0.0.3 "${value.role}" reference is a lifecycle role and can never occupy a host-binding requirement, concrete host binding, or satisfaction-provider position`);
        }
    }
}
// ---------------------------------------------------------------------------
 ...
 * semantic identity.
 */
export function expectDacV003UxRoleAnchor(role, anchor) {
    if (anchor === undefined)
        return;
    const guard = UX_ROLE_ANCHOR_GUARDS[role];
    if (guard === undefined) {
        throw new DacV003CompatibilityError('INVALID_COMPATIBILITY_REQUEST', 'ux-recovery-correlation has no #308 bridge anchor role; recovery correlation evidence is owned by the external-operation lane');
    }
    if (!guard(anchor)) {
        throw new DacV003CompatibilityError('INVALID_COMPATIBILITY_REQUEST', `UX semantic-role requirement "${role}" anchor must be a #308 bridge-minted reference of the matching renderer-independent role`);
    }
}
// ---------------------------------------------------------------------------
// Single-authority resolution (§6.3 / §12.2).
// ---------------------------------------------------------------------------
function authorityMismatch(what) {
 ...
 * scope fails closed.
 */
export function resolveDacV003CompatibilityAuthority(value) {
    ...
    if (isDacV003CompatibilityResultRefValue(value)) {
        ...
        if (link === undefined) {
            authorityMismatch(`a compatibility-result reference must link exactly one compatibility-validation reference, found ${links.length}`);
        }
        if (link.authorityScope !== DAC_V003_COMPATIBILITY_AUTHORITY_SCOPE) {
            authorityMismatch('the linked compatibility-validation reference is not in the single DAC v0.0.3 compatibility-validation authority scope');
        }
        return { authorityScope: value.authorityScope, validationIdentity: link.primaryIdentity };
    }
    throw new DacV003CompatibilityError('INVALID_COMPATIBILITY_RESULT', 'value is not a minted compatibility validation/result view');
}
/**
 ...
 */
export function assertSameDacV003CompatibilityAuthority(validationView, resultView) {
    ...
    if (left.authorityScope !== right.authorityScope ||
        left.validationIdentity !== right.validationIdentity) {
        authorityMismatch(`compatibility validation and result views resolve to different authorities (${left.authorityScope}|${left.validationIdentity} vs ${right.authorityScope}|${right.validationIdentity}); a second validation authority for the same exact subject/target is forbidden`);
    }
}
/** Registers a validation record minted by the validator (module-internal). */
export function registerMintedCompatibilityValidation(validation) {
    MINTED_COMPATIBILITY_VALIDATIONS.add(validation);
}
/** Registers a result record minted by this module (module-internal). */
export function registerMintedCompatibilityResult(result) {

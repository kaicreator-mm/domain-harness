import { IdentityContractError, canonicalJsonStringify, } from '../contracts/identity.js';
import { ProcessCommandContractError, } from '../contracts/process-command.js';
...
export function normalizeDurableProcessData(value) {
...
function requireResolvedAt(disposition) {
    if (disposition.resolvedAt === undefined || disposition.resolvedAt.length === 0) {
        fail('COMMAND_OUTCOME_MISSING', `message ${disposition.messageId} has terminal disposition without resolvedAt`);
    }
    return disposition.resolvedAt;
}
function baseOutcome(disposition) {
    return {
 ...
 */
export function deriveCommandOutcome(disposition, processedResolution) {
    if (disposition.disposition === 'accepted' || disposition.disposition === 'processing') {
        if (processedResolution !== undefined) {
            fail('COMMAND_OUTCOME_MISMATCH', `message ${disposition.messageId} is unresolved but a terminal resolution was supplied`);
        }
        return null;
    }
    ...
    if (disposition.disposition === 'processed') {
        if (processedResolution === undefined) {
            fail('COMMAND_OUTCOME_MISSING', `processed message ${disposition.messageId} requires applied/rejected outcome data`);
        }
    ...
    if (disposition.disposition === 'failed') {
        if (processedResolution !== undefined) {
            fail('COMMAND_OUTCOME_MISMATCH', `failed message ${disposition.messageId} cannot carry an applied/rejected resolution`);
        }
    ...
    if (processedResolution !== undefined) {
        fail('COMMAND_OUTCOME_MISMATCH', `abandoned message ${disposition.messageId} cannot carry an applied/rejected resolution`);
    }
    return { ...base, status: 'abandoned' };
}
function outcomeIdentityMatches(disposition, outcome) {
    return (outcome.messageId === disposition.messageId &&
 ...
 */
export function assertCommandOutcomeAlignment(disposition, outcome) {
    if (disposition.disposition === 'accepted' || disposition.disposition === 'processing') {
        if (outcome !== null) {
...
function resolutionFromOutcome(outcome) {
...
function assertRequestIdentity(state, request) {
    if (!sameAddress(request.target, state.instance.address) ||
    ...
    if (request.expectedTargetSequence !== state.disposition.targetSequence) {
        fail('TARGET_SEQUENCE_MISMATCH', `expected target sequence ${request.expectedTargetSequence}, durable sequence is ${state.disposition.targetSequence}`);
    }
}
/**
 ...
 */
export function prepareProcessedCommandTurn(state, request) {
    ...
    if (state.disposition.disposition !== 'processing') {
        fail('COMMAND_TURN_NOT_PROCESSING', `message must be processing before processed-turn commit; got ${state.disposition.disposition}`);
    }
    ...
    if (state.instance.stateRevision !== request.expectedStateRevision) {
        fail('STATE_REVISION_MISMATCH', `expected state revision ${request.expectedStateRevision}, durable revision is ${state.instance.stateRevision}`);
    }
    if (!Number.isSafeInteger(request.expectedStateRevision) ||
    ...
    if (request.nextLifecycle === 'recovery_required') {
        fail('NORMAL_OUTCOME_CANNOT_REQUIRE_RECOVERY', 'applied/rejected command resolution cannot force recovery_required; use technical failure path');
    }
    ...
    const output = request.output === undefined ? undefined : normalizeJsonValue(request.output, 'command output');
    const processedDisposition = {
        ...state.disposition,

...
export class JournaledSkillRunner {
    ...
    async run(request) {
        ...
        const input = this.#schemas.validate(skill.inputSchema, request.input, 'invalid_input', `Skill '${skill.skillId}' input`);
    ...
    async execute(skill, input, request, effectId, attempt) {
        ...
        try {
            ...
            return this.#schemas.validate(skill.outputSchema, raw, 'invalid_output', `Skill '${skill.skillId}' output`);
        }
        finally {
            if (timer !== undefined)
                clearTimeout(timer);
...
function validateCompiledSkill(skill) {
    if (skill.skillId.trim().length === 0)
        throw new Error('Compiled Skill skillId must be non-empty');
...
//# sourceMappingURL=journaled-skill-runner.js.map
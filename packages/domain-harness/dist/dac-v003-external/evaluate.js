...
function providerOrderMaterial(observation) {
...
function integerTokenValue(material) {
    if (!INTEGER_TOKEN.test(material.token))
        return undefined;
...
export function reconcileDacV003ExternalOperation(request) {
    ...
    const upstreamV002Evidence = (request.upstreamV002Evidence ?? []).map((e) => {
        ...
        if (e.correlation.runtimeOperation.effectId !==
            logicalOperation.reference.primaryIdentity) {
            throw new DacV003ExternalError('CORRELATION_CONFLICT', `upstream #309 evidence correlates runtime effect "${e.correlation.runtimeOperation.effectId}", not this episode's logical operation "${logicalOperation.reference.primaryIdentity}"`);
        }
        const evidenceAuthority = e.correlation.externalAuthority;
        if (evidenceAuthority.authorityId !== externalAuthority.reference.primaryIdentity ||
            evidenceAuthority.authorityScope !== externalAuthority.reference.authorityScope) {
            throw new DacV003ExternalError('CORRELATION_CONFLICT', 'upstream #309 evidence correlates a different external authority than this reconciliation episode');
        }
        return e;
    });
...
export function evaluateDacV003SafeRetry(request) {
    ...
    if (request.intendedSemanticTargets !== undefined) {
        ...
        if (JSON.stringify(currentTargets) !== JSON.stringify(intendedTargets)) {
            throw new DacV003ExternalError('IDENTITY_MISMATCH', `the intended semantic targets [${intendedTargets.join(', ')}] differ from logical operation "${logicalOperation.reference.primaryIdentity}" targets [${currentTargets.join(', ')}]; a changed semantic target (identity or role) creates a NEW LogicalOperationRef (§4 rule 2)`);
        }
    }
    const reasons = [];

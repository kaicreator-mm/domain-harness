...
import { adoptDacV003RegistryReference, isDacV003Reference, } from '../dac-v003/guards.js';
import { isExternalObservationEvidence, isExternalReconciliationOutcome, } from '../external-authority/guards.js';
import { DAC_V003_ATTEMPT_EVIDENCE_CLASSES, DAC_V003_EXTERNAL_OPERATION_ADAPTER_VERSION, DAC_V003_EXTERNAL_OUTCOME_CLASSES, DAC_V003_EXTERNAL_OUTCOME_NAMESPACE, DAC_V003_OUTCOME_CLASS_FROM_V002_CLASSIFICATION, DAC_V003_RECONCILIATION_ACTION_SEMANTICS, DAC_V003_RECOVERY_CAPABILITY_FAMILIES, DacV003ExternalError, } from './contracts.js';
/**
 ...
 */
const MINTED_EXTERNAL_ROLE_WRAPPERS = new WeakSet();
function freezeMinted(value) {
    return Object.freeze(value);
}
function requireNonEmptyString(value, field) {
    if (typeof value !== 'string' || value.trim().length === 0) {
        throw new DacV003ExternalError('INVALID_EXTERNAL_BINDING', `${field} must be a non-empty string`);
    }
}
...
function copyRefs(value, field) {
    const arr = requireArray(value, field);
    const validated = [];
    for (const entry of arr) {
        if (!isDacV003Reference(entry)) {
            throw new DacV003ExternalError('INVALID_EXTERNAL_BINDING', `${field} must contain only adopted DAC v0.0.3 references; foreign, v0.0.2 or forged objects fail closed`);
        }
        validated.push(entry);
    }
    return validated;
}
function wrapperRole(value) {
    return value.reference.role;
}
function isMintedRoleWrapper(value) {
    if (value === null || typeof value !== 'object')
        return false;
    if (!MINTED_EXTERNAL_ROLE_WRAPPERS.has(value))
        return false;
    const candidate = value;
    return (candidate.adapter === DAC_V003_EXTERNAL_OPERATION_ADAPTER_VERSION &&
        typeof candidate.reference === 'object' &&
        candidate.reference !== null &&
        isDacV003Reference(candidate.reference));
}
function expectMintedRole(role, value) {
    if (!isMintedRoleWrapper(value)) {
        throw new DacV003ExternalError('ROLE_MISMATCH', `expected a minted dac-v003 "${String(role)}" role wrapper, but received a foreign or forged object; DAC v0.0.3 external-operation roles are adopted only through this surface`);
    }
    if (wrapperRole(value) !== role) {
        throw new DacV003ExternalError('ROLE_MISMATCH', `expected a "${String(role)}" role wrapper, but received "${wrapperRole(value)}"; DAC v0.0.3 external-operation roles are never interchangeable`);
    }
    return value;
}
function sameAuthorityIdentity(authority, other) {
    return (authority.reference.primaryIdentity === other.reference.primaryIdentity &&
...
export function adoptDacV003ExternalAuthorityRef(input) {
...
export function isDacV003ExternalAuthorityRef(v) {
    return isMintedRoleWrapper(v) && wrapperRole(v) === 'external-authority';
}
export function expectDacV003ExternalAuthorityRef(v) {
    expectMintedRole('external-authority', v);
}
// ---------------------------------------------------------------------------
// LogicalOperationRef
// ---------------------------------------------------------------------------
 ...
 */
export function adoptDacV003LogicalOperationRef(input) {
    if (input === null || typeof input !== 'object') {
        throw new DacV003ExternalError('INVALID_EXTERNAL_BINDING', 'adoption input must be an object');
    }
    ...
    const externalAuthority = expectMintedRole('external-authority', input.externalAuthority);
    const semanticTargetRefs = copyRefs(input.semanticTargetRefs, 'semanticTargetRefs');
    const originatingCorrelationRefs = copyRefs(input.originatingCorrelationRefs, 'originatingCorrelationRefs');
    if (input.idempotencyIdentity !== undefined) {
        assertDacV003IdempotencyBoundToLogicalEffect(expectMintedRole('idempotency-identity', input.idempotencyIdentity), externalAuthority, input.operationSemanticIdentity);
    }
...
export function isDacV003LogicalOperationRef(v) {
    return isMintedRoleWrapper(v) && wrapperRole(v) === 'logical-operation';
}
export function expectDacV003LogicalOperationRef(v) {
    expectMintedRole('logical-operation', v);
}
/**
 ...
 */
export function assertDacV003LogicalOperationContinuity(prior, next) {
    const mismatches = [];
    if (!sameAuthorityIdentity(prior.externalAuthority, next.externalAuthority)) {
        mismatches.push(`externalAuthority: prior "${prior.externalAuthority.reference.primaryIdentity}" vs next "${next.externalAuthority.reference.primaryIdentity}"`);
    }
    if (prior.operationSemanticIdentity !== next.operationSemanticIdentity) {
        mismatches.push(`operationSemanticIdentity: prior "${prior.operationSemanticIdentity}" vs next "${next.operationSemanticIdentity}"`);
    }
    ...
    if (JSON.stringify(priorTargets) !== JSON.stringify(nextTargets)) {
        mismatches.push(`semanticTargetRefs: prior [${priorTargets.join(', ')}] vs next [${nextTargets.join(', ')}]`);
    }
    if (mismatches.length > 0) {
        throw new DacV003ExternalError('IDENTITY_MISMATCH', `logical operation "${prior.reference.primaryIdentity}" cannot continue a changed intent (${mismatches.join('; ')}); a materially changed command semantics, external authority, or semantic target creates a NEW LogicalOperationRef`);
    }
}
// ---------------------------------------------------------------------------
...
export function adoptDacV003ProviderOperationRef(input) {
    if (input === null || typeof input !== 'object') {
        throw new DacV003ExternalError('INVALID_EXTERNAL_BINDING', 'adoption input must be an object');
    }
    requireNonEmptyString(input.providerOperationId, 'providerOperationId');
    const externalAuthority = expectMintedRole('external-authority', input.externalAuthority);
    let correlatedLogicalOperation;
    if (input.correlatedLogicalOperation !== undefined) {
        correlatedLogicalOperation = expectMintedRole('logical-operation', input.correlatedLogicalOperation);
        if (!sameAuthorityIdentity(correlatedLogicalOperation.externalAuthority, externalAuthority)) {
            throw new DacV003ExternalError('CORRELATION_CONFLICT', 'provider operation correlation binds a logical operation of a different external authority; one provider operation lives inside exactly one authority scope');
        }
    }
    const continuationProofRefs = copyRefs(input.continuationProofRefs, 'continuationProofRefs');
    if (input.continuationOfProviderOperationId !== undefined &&
        input.continuationOfProviderOperationId === input.providerOperationId) {
        throw new DacV003ExternalError('IDENTITY_MISMATCH', 'a provider operation cannot be recorded as its own continuation');
    }
    if (input.continuationOfProviderOperationId !== undefined &&
        continuationProofRefs.length === 0) {
        throw new DacV003ExternalError('IDENTITY_MISMATCH', 'referencing the same provider operation across local retries requires provider contract/evidence proving the continuation (§6 rule 5); without proof a distinct provider operation identity must be represented');
    }
    const preservedAmbiguityWithProviderOperationIds = requireArray(input.preservedAmbiguityWithProviderOperationIds, 'preservedAmbiguityWithProviderOperationIds').slice();
    for (const id of preservedAmbiguityWithProviderOperationIds) {
        ...
        if (id === input.providerOperationId) {
            throw new DacV003ExternalError('IDENTITY_MISMATCH', 'a provider operation cannot preserve ambiguity with itself');
        }
    }
    const reference = adoptDacV003RegistryReference('provider-operation', {
        ...
        ...(correlatedLogicalOperation === undefined
            ? {}
            : {
                logicalOperationIdentity: correlatedLogicalOperation.reference.primaryIdentity,
            }),
        ...(input.locatorHints === undefined ? {} : { locatorHints: input.locatorHints }),
        ...(input.opaque === undefined ? {} : { opaque: input.opaque }),
    });
    const wrapper = freezeMinted({
        ...
        preservedAmbiguityWithProviderOperationIds: freezeMinted(preservedAmbiguityWithProviderOperationIds),
    });
    MINTED_EXTERNAL_ROLE_WRAPPERS.add(wrapper);
    return wrapper;
}
export function isDacV003ProviderOperationRef(v) {
    return isMintedRoleWrapper(v) && wrapperRole(v) === 'provider-operation';
}
export function expectDacV003ProviderOperationRef(v) {
    expectMintedRole('provider-operation', v);
}
// ---------------------------------------------------------------------------
// AttemptRef
// ---------------------------------------------------------------------------
 ...
 */
export function adoptDacV003AttemptRef(input) {
    if (input === null || typeof input !== 'object') {
        throw new DacV003ExternalError('INVALID_EXTERNAL_BINDING', 'adoption input must be an object');
    }
    ...
    if (DAC_V003_ATTEMPT_EVIDENCE_CLASSES.indexOf(input.evidenceClass) === -1) {
        throw new DacV003ExternalError('INVALID_EXTERNAL_BINDING', `"${String(input.evidenceClass)}" is not a DAC v0.0.3 attempt evidence class`);
    }
    const logicalOperation = expectMintedRole('logical-operation', input.logicalOperation);
    if (input.attemptIdentity === logicalOperation.reference.primaryIdentity) {
        throw new DacV003ExternalError('IDENTITY_MISMATCH', `attempt identity "${input.attemptIdentity}" collapses into the logical operation identity; LogicalOperation 1 -> 0..n distinct Attempts must stay separately identifiable (conformance C67)`);
    }
    let providerOperation;
    if (input.providerOperation !== undefined) {
        providerOperation = expectMintedRole('provider-operation', input.providerOperation);
        if (!sameAuthorityIdentity(providerOperation.externalAuthority, logicalOperation.externalAuthority)) {
            throw new DacV003ExternalError('CORRELATION_CONFLICT', 'attempt binds a provider operation of a different external authority than its logical operation');
        }
    }
...
export function isDacV003AttemptRef(v) {
    return isMintedRoleWrapper(v) && wrapperRole(v) === 'attempt';
}
export function expectDacV003AttemptRef(v) {
    expectMintedRole('attempt', v);
}
/**
 ...
 */
export function assertDacV003AttemptIdentitiesDistinct(attempts) {
    ...
    for (const attempt of attempts) {
        ...
        if (priorOwner !== undefined && priorOwner !== logicalOperationIdentity) {
            throw new DacV003ExternalError('IDENTITY_MISMATCH', `attempt identity "${attemptIdentity}" is reused across logical operations`);
        }
        identityOwner.set(attemptIdentity, logicalOperationIdentity);
        const key = `${logicalOperationIdentity}|${attemptIdentity}`;
        if (seen.has(key)) {
            throw new DacV003ExternalError('IDENTITY_MISMATCH', `duplicate attempt identity "${attemptIdentity}" for logical operation "${logicalOperationIdentity}"; a new delivery/replay/transport execution requires a NEW AttemptRef (conformance C67)`);
        }
        seen.add(key);
    }
}
 ...
 */
export function maxJustifiedMeaningForAttemptEvidence(evidenceClass) {
...
export function adoptDacV003AuthoritativeEffectRecordRef(input) {
    if (input === null || typeof input !== 'object') {
        throw new DacV003ExternalError('INVALID_EXTERNAL_BINDING', 'adoption input must be an object');
    }
...
export function isDacV003AuthoritativeEffectRecordRef(v) {
    return isMintedRoleWrapper(v) && wrapperRole(v) === 'authoritative-effect-record';
}
export function expectDacV003AuthoritativeEffectRecordRef(v) {
    expectMintedRole('authoritative-effect-record', v);
}
// ---------------------------------------------------------------------------
// IdempotencyIdentityRef
// ---------------------------------------------------------------------------
/** Adopt an idempotency identity with issuer + promised dedup scope (§7). */
export function adoptDacV003IdempotencyIdentityRef(input) {
    if (input === null || typeof input !== 'object') {
        throw new DacV003ExternalError('INVALID_EXTERNAL_BINDING', 'adoption input must be an object');
    }
    ...
    requireNonEmptyString(input.boundLogicalEffectSemanticIdentity, 'boundLogicalEffectSemanticIdentity');
    const externalAuthority = expectMintedRole('external-authority', input.externalAuthority);
    const rule = input.equivalenceRule;
    if (rule === null ||
        typeof rule !== 'object' ||
        ['semantic', 'payload-byte', 'provider-defined'].indexOf(rule.kind) === -1) {
        throw new DacV003ExternalError('INVALID_EXTERNAL_BINDING', 'equivalenceRule.kind must be one of semantic | payload-byte | provider-defined');
    }
    if (rule.establishedFromEvidence && rule.ruleIdentity === undefined) {
        throw new DacV003ExternalError('INVALID_EXTERNAL_BINDING', 'an equivalence rule established from evidence must name the rule identity (byte and semantic equality are both insufficient alone, §7 rule 3)');
    }
    const guaranteeEvidenceRefs = copyRefs(input.guaranteeEvidenceRefs, 'guaranteeEvidenceRefs');
...
export function isDacV003IdempotencyIdentityRef(v) {
    return isMintedRoleWrapper(v) && wrapperRole(v) === 'idempotency-identity';
}
export function expectDacV003IdempotencyIdentityRef(v) {
    expectMintedRole('idempotency-identity', v);
}
function assertDacV003IdempotencyBoundToLogicalEffect(idempotencyIdentity, externalAuthority, operationSemanticIdentity) {
    if (!sameAuthorityIdentity(idempotencyIdentity.externalAuthority, externalAuthority) ||
        idempotencyIdentity.boundLogicalEffectSemanticIdentity !== operationSemanticIdentity) {
        throw new DacV003ExternalError('IDEMPOTENCY_REUSE_FORBIDDEN', `idempotency identity "${idempotencyIdentity.reference.primaryIdentity}" is bound to a different logical effect (issuer "${idempotencyIdentity.issuer}", scope "${idempotencyIdentity.promisedDeduplicationScope}", bound semantic identity "${idempotencyIdentity.boundLogicalEffectSemanticIdentity}"); one idempotency identity must never be reused for a semantically different command, target or effect (conformance C66)`);
    }
}
/**
 ...
 */
export function assertDacV003IdempotencyReuseForLogicalEffect(idempotencyIdentity, logicalOperation, intendedCommandSemanticIdentity) {
    assertDacV003IdempotencyBoundToLogicalEffect(idempotencyIdentity, logicalOperation.externalAuthority, intendedCommandSemanticIdentity);
 ...
 */
export function isDacV003IdempotencyGuaranteeProven(idempotencyIdentity) {
    if (!isMintedRoleWrapper(idempotencyIdentity))
        return false;
    return (idempotencyIdentity.guaranteeEvidenceRefs.length > 0 &&
 ...
 */
export function adoptDacV003ExternalObservationRef(input) {
    if (input === null || typeof input !== 'object') {
        throw new DacV003ExternalError('INVALID_EXTERNAL_BINDING', 'adoption input must be an object');
    }
    requireNonEmptyString(input.observationIdentity, 'observationIdentity');
    requireNonEmptyString(input.producer, 'producer');
    if (DAC_V003_EXTERNAL_OUTCOME_CLASSES.indexOf(input.observedClass) === -1) {
        throw new DacV003ExternalError('INVALID_EXTERNAL_BINDING', `"${String(input.observedClass)}" is not a DAC v0.0.3 external-operation outcome class`);
    }
    const externalAuthority = expectMintedRole('external-authority', input.externalAuthority);
    const logicalOperation = expectMintedRole('logical-operation', input.logicalOperation);
    if (!sameAuthorityIdentity(logicalOperation.externalAuthority, externalAuthority)) {
        throw new DacV003ExternalError('CORRELATION_CONFLICT', 'observation binds a logical operation of a different external authority; an external observation observes exactly one authority\'s truth semantics');
    }
    const providerOperations = requireArray(input.providerOperations, 'providerOperations').slice();
    for (const providerOperation of providerOperations) {
        expectMintedRole('provider-operation', providerOperation);
        if (!sameAuthorityIdentity(providerOperation.externalAuthority, externalAuthority)) {
            throw new DacV003ExternalError('CORRELATION_CONFLICT', 'observation references a provider operation of a different external authority');
        }
    }
    const effectRecords = requireArray(input.effectRecords, 'effectRecords').slice();
    for (const effectRecord of effectRecords) {
        expectMintedRole('authoritative-effect-record', effectRecord);
        if (!sameAuthorityIdentity(effectRecord.externalAuthority, externalAuthority)) {
            throw new DacV003ExternalError('CORRELATION_CONFLICT', 'observation references an authoritative effect record of a different external authority');
        }
    }
    if (input.providerCurrentness !== undefined) {
        ...
        if (currentness === null || typeof currentness !== 'object') {
            throw new DacV003ExternalError('INVALID_EXTERNAL_BINDING', 'providerCurrentness must be an object of provider-exposed currentness metadata');
        }
        for (const field of ['version', 'sequence', 'providerObservedAt', 'adapterReceivedAt']) {
            const value = currentness[field];
            if (value !== undefined)
                requireNonEmptyString(value, `providerCurrentness.${field}`);
        }
    }
...
export function isDacV003ExternalObservationRef(v) {
    return isMintedRoleWrapper(v) && wrapperRole(v) === 'external-observation';
}
export function expectDacV003ExternalObservationRef(v) {
    expectMintedRole('external-observation', v);
}
// ---------------------------------------------------------------------------
// Recovery / external capability declarations
// ---------------------------------------------------------------------------
function adoptCapabilityDeclaration(role, input) {
    if (input === null || typeof input !== 'object') {
        throw new DacV003ExternalError('INVALID_EXTERNAL_BINDING', 'adoption input must be an object');
    }
    requireNonEmptyString(input.capabilityIdentity, 'capabilityIdentity');
    requireNonEmptyString(input.issuer, 'issuer');
    if (DAC_V003_RECOVERY_CAPABILITY_FAMILIES.indexOf(input.capabilityFamily) === -1) {
        throw new DacV003ExternalError('INVALID_EXTERNAL_BINDING', `"${String(input.capabilityFamily)}" is not a DAC v0.0.3 recovery/external capability family`);
    }
    const externalAuthority = expectMintedRole('external-authority', input.externalAuthority);
    const applicabilityConditions = requireArray(input.applicabilityConditions, 'applicabilityConditions').slice();
    for (const condition of applicabilityConditions) {
        requireNonEmptyString(condition, 'applicabilityConditions entry');
    }
    const satisfactionEvidenceRefs = copyRefs(input.satisfactionEvidenceRefs, 'satisfactionEvidenceRefs');
 ...
 */
export function declareDacV003RecoveryCapability(input) {
 ...
 */
export function declareDacV003ExternalCapability(input) {
    return adoptCapabilityDeclaration('external-capability', input);
}
export function isDacV003RecoveryCapabilityRef(v) {
    return isMintedRoleWrapper(v) && wrapperRole(v) === 'recovery-capability';
}
export function expectDacV003RecoveryCapabilityRef(v) {
    expectMintedRole('recovery-capability', v);
}
export function isDacV003ExternalCapabilityRef(v) {
    return isMintedRoleWrapper(v) && wrapperRole(v) === 'external-capability';
}
export function expectDacV003ExternalCapabilityRef(v) {
    expectMintedRole('external-capability', v);
}
/**
 ...
 */
export function assertDacV003CapabilitySatisfaction(declarations, requiredFamily) {
    for (const declaration of declarations) {
        if (!isMintedRoleWrapper(declaration)) {
            throw new DacV003ExternalError('ROLE_MISMATCH', 'capability declarations must be minted by this surface; foreign or forged objects fail closed');
        }
    }
    const matching = declarations.filter((d) => d.capabilityFamily === requiredFamily && d.satisfactionEvidenceRefs.length > 0);
    if (matching.length === 0) {
        throw new DacV003ExternalError('CAPABILITY_DECLARATION_NOT_PROVEN', `no declaration of capability family "${requiredFamily}" carries satisfaction evidence; a declared capability is not evidence that the bound provider/integration satisfies it (EXTERNAL_AUTHORITY §10)`);
    }
}
// ---------------------------------------------------------------------------
...
export function dacV003ExternalOutcome(value) {
    if (DAC_V003_EXTERNAL_OUTCOME_CLASSES.indexOf(value) === -1) {
        throw new DacV003ExternalError('INVALID_EXTERNAL_BINDING', `"${String(value)}" is not a dac-v003 external-operation outcome class; Reference/Compatibility dispositions and Authoring/Capability Exchange outcomes are separate namespaces and must never be conflated`);
    }
 ...
 */
export function isDacV003ExternalOutcome(v) {
    if (v === null || typeof v !== 'object')
        return false;
    const candidate = v;
    return (candidate.namespace === DAC_V003_EXTERNAL_OUTCOME_NAMESPACE &&
        typeof candidate.value === 'string' &&
        DAC_V003_EXTERNAL_OUTCOME_CLASSES.indexOf(candidate.value) !== -1);
}
// ---------------------------------------------------------------------------
 ...
 */
export function adoptDacV003ObservationFromV002Evidence(evidence, bindings) {
    if (!isExternalObservationEvidence(evidence)) {
        throw new DacV003ExternalError('INVALID_UPSTREAM_EVIDENCE', 'upstream evidence must be a genuine #309 ExternalObservationEvidence minted by the external-authority adapter; foreign or forged records fail closed');
    }
    if (evidence.correlation.runtimeOperation.effectId !==
        bindings.logicalOperation.reference.primaryIdentity) {
        throw new DacV003ExternalError('CORRELATION_CONFLICT', `upstream #309 evidence correlates runtime effect "${evidence.correlation.runtimeOperation.effectId}", not logical operation "${bindings.logicalOperation.reference.primaryIdentity}"; adoption requires exact correlation`);
    }
    const evidenceAuthority = evidence.correlation.externalAuthority;
    if (evidenceAuthority.authorityId !== bindings.externalAuthority.reference.primaryIdentity ||
        evidenceAuthority.authorityScope !== bindings.externalAuthority.reference.authorityScope) {
        throw new DacV003ExternalError('CORRELATION_CONFLICT', 'upstream #309 evidence correlates a different external authority than the v0.0.3 binding');
    }
    const observedClass = DAC_V003_OUTCOME_CLASS_FROM_V002_CLASSIFICATION[evidence.classification];
...
export function isDacV003UpstreamV002Evidence(v) {
    return isExternalObservationEvidence(v) || isExternalReconciliationOutcome(v);
}
export function expectDacV003UpstreamV002Evidence(v) {
    if (!isDacV003UpstreamV002Evidence(v)) {
        throw new DacV003ExternalError('INVALID_UPSTREAM_EVIDENCE', 'upstream evidence must be a genuine #309 ExternalObservationEvidence or ExternalReconciliationOutcome; foreign or forged records fail closed');
    }
}
// ---------------------------------------------------------------------------
 ...
 */
export function refuteDacV003HarnessSideIdentityAsExternalAuthority(v) {
    if (isDacV003Reference(v) && v.role !== 'external-authority') {
        throw new DacV003ExternalError('EXTERNAL_IDENTITY_FORBIDDEN', `a "${v.role}" DAC v0.0.3 reference is Harness-side composition/Runtime identity and can never substitute external Business SoR / effect-authority identity`);
    }
    if (isMintedRoleWrapper(v) && wrapperRole(v) !== 'external-authority') {
        throw new DacV003ExternalError('EXTERNAL_IDENTITY_FORBIDDEN', `a "${wrapperRole(v)}" role wrapper is not external Business SoR / effect-authority identity`);
    }
}
/**
 ...
 */
export function refuteDacV003AttemptAsLogicalOperation(v) {
    if (isDacV003AttemptRef(v)) {
        throw new DacV003ExternalError('ROLE_MISMATCH', 'an AttemptRef identifies one delivery/execution attempt and can never substitute the LogicalOperationRef it belongs to (conformance C67)');
    }
}
/**
 ...
 * authority scope.
 */
export function refuteDacV003ProviderOperationAsAuthoritativeEffectRecord(v) {
    if (isDacV003ProviderOperationRef(v)) {
        throw new DacV003ExternalError('ROLE_MISMATCH', 'a ProviderOperationRef identifies a provider-issued job/operation and can never substitute an AuthoritativeEffectRecordRef (conformance C68)');
    }
}
const LOCAL_CAUSE_KINDS = [
    'local-timeout',
    'local-cancel',
    'local-interrupt',
    'local-abandonment',
...
export function refuteDacV003LocalCauseAsRemoteTruth(input) {
    if (LOCAL_CAUSE_KINDS.indexOf(input.cause) === -1) {
        throw new DacV003ExternalError('INVALID_EXTERNAL_BINDING', `"${String(input.cause)}" is not a local cause kind`);
    }
    throw new DacV003ExternalError('LOCAL_CAUSE_FORBIDDEN', `local cause "${input.cause}" can never establish remote truth "${String(input.claimedRemoteTruth)}"; a local timeout/cancel/interrupt/abandonment is not authoritative proof of remote non-commit or rollback (conformance C64; EXTERNAL_AUTHORITY §2/§10)`);
}
/** Live operation roles forbidden in the declarative composition handoff (§13). */
const COMPOSITION_FORBIDDEN_LIVE_ROLES = [
    'logical-operation',
    'attempt',
    'provider-operation',
 ...
 */
export function assertDacV003CompositionHandoffIsDeclarative(items) {
    for (const item of items) {
        if (isMintedRoleWrapper(item) &&
            COMPOSITION_FORBIDDEN_LIVE_ROLES.indexOf(wrapperRole(item)) !== -1) {
            throw new DacV003ExternalError('COMPOSITION_LIVE_OPERATION_STATE_FORBIDDEN', `a "${wrapperRole(item)}" instance is live operation state and must not enter the declarative composition handoff (EXTERNAL_AUTHORITY §13)`);
        }
        if (isDacV003Reference(item) && COMPOSITION_FORBIDDEN_LIVE_ROLES.indexOf(item.role) !== -1) {
            throw new DacV003ExternalError('COMPOSITION_LIVE_OPERATION_STATE_FORBIDDEN', `a "${item.role}" reference is live operation state and must not enter the declarative composition handoff (EXTERNAL_AUTHORITY §13)`);
        }
    }
}

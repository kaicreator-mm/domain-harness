...
export function serializeChildMessageIdentity(effectId) {
    assertNonEmpty('effectId', effectId);
    return JSON.stringify(['domain-harness-child-message-v2', effectId]);
}
export async function deriveChildMessageId(sha256, effectId) {
...
function ackFromJson(value, expectedMessageId, expectedTarget, effectId) {
    ...
    if ((status !== 'accepted' && status !== 'duplicate') ||
        ...
        throw new DomainMessageEffectJournalInvariantError(`completed Domain Message effect ${effectId} contains an invalid target ACK`);
    }
    return {
        status,
        messageId,
...
//# sourceMappingURL=journaled-domain-message-effect.js.map
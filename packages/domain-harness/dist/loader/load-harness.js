...
import { HarnessDefinitionError, validateChildGraph, validateWorkflowStructure, } from './static-validation.js';
import { eventSpecSchema, harnessManifestSchema, skillSidecarSchema, workflowFileSchema, } from './schemas.js';
...
function normalizeInvoke(raw) {
    ...
    if (typeof raw.skill === 'string')
        return { kind: 'skill', ref: raw.skill, ...common };
    if (typeof raw.tool === 'string')
        return { kind: 'tool', ref: raw.tool, ...common };
    if (typeof raw.script === 'string')
        return { kind: 'script', ref: raw.script, ...common };
    if (typeof raw.workflow === 'string')
        return { kind: 'workflow', ref: raw.workflow, ...common };
    if (typeof raw.expr === 'string')
        return { kind: 'expr', expression: raw.expr, ...common };
    ...
    for (const entry of entries) {
        ...
        try {
            ...
            const inputSchema = sidecar.input
                ? await loadJsonSchema(await safeExistingPath(directory, sidecar.input.schema), `skill:${id}:input:${sidecar.input.schema}`, schemas, ajv, issues)
                : undefined;
            const outputSchema = await loadJsonSchema(await safeExistingPath(directory, sidecar.output.schema), `skill:${id}:output:${sidecar.output.schema}`, schemas, ajv, issues);
            if (!outputSchema)
                continue;
            skills.set(id, {
                id,
...
async function loadWorkflows(root, issues) {
    ...
    for (const entry of entries) {
        ...
        try {
            const raw = workflowFileSchema.parse(await readYaml(sourcePath));
            const states = {};
            for (const [stateId, rawState] of Object.entries(raw.states)) {
                ...
                if (normalizedDone?.schemaPath)
                    issues.push(`workflow:${id}.${stateId}.on.done cannot declare an event payload schema`);
                if (normalizedError?.schemaPath)
                    issues.push(`workflow:${id}.${stateId}.on.error cannot declare an event payload schema`);
                ...
                states[stateId] = {
                    ...
                    ...(rawState.invoke ? { invoke: normalizeInvoke(rawState.invoke) } : {}),
                    done,
                    error,
                    events,
                };
...
export async function loadHarness(options) {
    const root = await realpath(resolve(options.root));
    const issues = [];
    const manifestPath = await safeExistingPath(root, 'harness.yaml');
    const manifest = harnessManifestSchema.parse(await readYaml(manifestPath));
    ...
    for (const workflow of workflows.values()) {
        ...
        for (const state of Object.values(workflow.states)) {
            ...
            for (const [routeClass, routes] of [['done', state.done], ['error', state.error]]) {
                for (const [index, route] of routes.entries()) {
                    if (route.when)
                        compileExpression(route.when, `${workflow.id}.${state.id}.on.${routeClass}[${index}].when`, issues);
                }
            }
            for (const [eventName, event] of Object.entries(state.events)) {
                if (event.schemaPath) {
                    const eventSchema = await loadJsonSchema(await safeExistingPath(root, event.schemaPath), `event:${workflow.id}:${state.id}:${eventName}:${event.schemaPath}`, schemas, ajv, issues);
                    if (eventSchema)
                        event.schema = eventSchema;
                }
                for (const [index, route] of event.routes.entries()) {
                    if (route.when)
                        compileExpression(route.when, `${workflow.id}.${state.id}.on.${eventName}[${index}].when`, issues);
                }
            }
    ...
    if (issues.length > 0)
        throw new HarnessDefinitionError(issues);

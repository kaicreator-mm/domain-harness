...
function requireNonEmptyString(value, field) {
    if (typeof value !== 'string' || value.trim().length === 0) {
        throw new GovernanceContractError('INVALID_GOVERNANCE_IDENTITY', `${field} must be a non-empty string`);
    }
    return value;
}
export function assertGovernanceBaselineIdentity(identity) {
    ...
    if (!isContentDigest(identity.contentDigest)) {
        throw new GovernanceContractError('INVALID_GOVERNANCE_IDENTITY', 'contentDigest must be a non-empty exact content digest');
    }
    if (identity.version !== undefined)
        requireNonEmptyString(identity.version, 'version');
}
...
export async function computeGovernanceBaselineIdentity(input, sha256) {
    ...
    const contentDigest = await computeCanonicalJsonDigest(semanticDigestMaterial(schemaVersion, input.semantics), sha256);
    return version === undefined
        ? { domainId, governanceId, schemaVersion, contentDigest }
        : { domainId, governanceId, schemaVersion, version, contentDigest };
}
export async function createGovernanceBaselineBody(input, sha256) {
    return {
...
export async function verifyGovernanceBaselineBody(body, sha256) {
    ...
    const expected = await computeCanonicalJsonDigest(semanticDigestMaterial(body.identity.schemaVersion, body.semantics), sha256);
    if (expected !== body.identity.contentDigest) {
        throw new GovernanceContractError('GOVERNANCE_DIGEST_MISMATCH', `Governance Baseline ${body.identity.governanceId} content digest does not match canonical semantics`);
    }
}
export function assertGovernancePackageCdiBinding(binding) {
    if (typeof binding.domainId !== 'string' || binding.domainId.trim().length === 0) {
        throw new GovernanceContractError('INVALID_PACKAGE_CDI_BINDING', 'package/CDI binding domainId must be non-empty');
    }
    if (typeof binding.packageId !== 'string' || binding.packageId.trim().length === 0) {
        throw new GovernanceContractError('INVALID_PACKAGE_CDI_BINDING', 'package/CDI binding packageId must be non-empty');
    }
    if (!isContentDigest(binding.domainIntelligenceContentDigest)) {
        throw new GovernanceContractError('INVALID_PACKAGE_CDI_BINDING', 'package/CDI binding domainIntelligenceContentDigest must be exact and non-empty');
    }
}
export function createGovernanceBaselineAuthorityBinding(packageCdi, governanceBaseline) {
    ...
    if (packageCdi.domainId !== governanceBaseline.domainId) {
        throw new GovernanceContractError('GOVERNANCE_DOMAIN_MISMATCH', `package/CDI domain ${packageCdi.domainId} does not match Governance Baseline domain ${governanceBaseline.domainId}`);
    }
    return {
...
function asHumanOperatorAuthority(value) {
    if (typeof value !== 'object' || value === null)
        return undefined;
    const candidate = value;
    if (candidate.kind !== 'human-operator-governance')
        return undefined;
    if (typeof candidate.actorId !== 'string' || candidate.actorId.trim().length === 0)
        return undefined;
    if (candidate.governingBaseline === undefined)
        return undefined;
    try {
 ...
 */
export function resolveGovernanceClassification(classification, authority) {
    if (classification !== 'non-governance')
        return 'governance-critical';
    if (asHumanOperatorAuthority(authority) === undefined) {
        throw new GovernanceContractError('NON_GOVERNANCE_REQUIRES_OPERATOR_AUTHORITY', 'non-governance classification requires explicit human/operator governance authority');
    }

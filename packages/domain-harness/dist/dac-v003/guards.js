...
import { DAC_V003_BASELINE, DAC_V003_DISPOSITION_NAMESPACE, DAC_V003_PROFILE_REQUIREMENTS, DAC_V003_REFERENCE_DISPOSITIONS, DAC_V003_REFERENCE_ADAPTER_VERSION, DAC_V003_ROLE_REGISTRY, DacV003ReferenceError, } from './contracts.js';
/**
...
function baselineMatches(input) {
    return (input.contract === DAC_V003_BASELINE.contract &&
...
function requireNonEmptyString(value, field) {
    if (typeof value !== 'string' || value.trim().length === 0) {
        throw new DacV003ReferenceError('INVALID_REFERENCE', `${field} must be a non-empty string`);
    }
}
function requireExactIdentityString(value, field) {
    requireNonEmptyString(value, field);
    if (isMutableAliasToken(value)) {
        throw new DacV003ReferenceError('MUTABLE_ALIAS_REJECTED', `${field} "${value}" is a mutable alias (latest/current/head-style) and can never substitute an exact identity`);
    }
}
 ...
 */
const ADOPTED_V003_REFERENCES = new WeakSet();
function structurallyValidAdoptedReference(value) {
    if (value === null || typeof value !== 'object')
        return false;
    if (!ADOPTED_V003_REFERENCES.has(value))
        return false;
    const candidate = value;
    return (candidate.adapter === DAC_V003_REFERENCE_ADAPTER_VERSION &&
        typeof candidate.role === 'string' &&
        DAC_V003_ROLE_REGISTRY.indexOf(candidate.role) !== -1 &&
...
function requireAdoptedRefArray(value, field) {
    if (value === undefined)
        return;
    ...
    for (const entry of value) {
        if (!structurallyValidAdoptedReference(entry)) {
            throw new DacV003ReferenceError('INVALID_REFERENCE', `${field} must contain only adopted DAC v0.0.3 references; foreign, v0.0.2 or forged objects fail closed`);
        }
    }
}
/** Shared adoption core: validates, nominalizes and freezes one reference. */
function adoptDacV003Reference(role, input) {
    ...
    if (DAC_V003_ROLE_REGISTRY.indexOf(role) === -1) {
        throw new DacV003ReferenceError('INVALID_REFERENCE', `"${String(role)}" is not a canonical DAC v0.0.3 registry role`);
    }
    const { baseline } = input;
    if (baseline === null ||
        typeof baseline !== 'object' ||
        !baselineMatches(baseline)) {
        throw new DacV003ReferenceError('UNSUPPORTED_DAC_BASELINE', `reference baseline must be exactly ${DAC_V003_BASELINE.contract}@${DAC_V003_BASELINE.version} semantic freeze commit ${DAC_V003_BASELINE.semanticFreezeCommit} tree ${DAC_V003_BASELINE.semanticFreezeTree}`);
    }
    ...
    if (input.locatorHints !== undefined) {
        if (!Array.isArray(input.locatorHints)) {
            throw new DacV003ReferenceError('INVALID_REFERENCE', 'locatorHints must be an array of non-authoritative discovery hints');
        }
    ...
    if (input.derivationOperationRef !== undefined) {
        if (!structurallyValidAdoptedReference(input.derivationOperationRef)) {
            throw new DacV003ReferenceError('INVALID_REFERENCE', 'derivationOperationRef must be an adopted DAC v0.0.3 reference');
        }
    }
    ...
    if (input.opaque !== undefined &&
        ...
        throw new DacV003ReferenceError('INVALID_REFERENCE', 'opaque must be an object of unknown/provisional source fields');
    }
    ...
    if (role === 'runtime-host-binding-requirement') {
        if (input.requiredHostBindingRole === undefined) {
            throw new DacV003ReferenceError('INVALID_REFERENCE', 'runtime-host-binding-requirement must declare requiredHostBindingRole (what host-binding semantic role is required)');
        }
        requireNonEmptyString(input.requiredHostBindingRole, 'requiredHostBindingRole');
    }
    if (role === 'runtime-host-binding') {
        if (input.semanticIdentity === undefined) {
            throw new DacV003ReferenceError('INVALID_REFERENCE', 'runtime-host-binding requires semanticIdentity naming the concrete adapter/binding implementation');
        }
    }
    if (role === 'runtime-interaction-contract') {
        if (input.semanticIdentity === undefined || input.revisionIdentity === undefined) {
            throw new DacV003ReferenceError('INVALID_REFERENCE', 'runtime-interaction-contract requires semanticIdentity and exact revisionIdentity (immutable/versioned semantic contract; no floating current target)');
        }
    }
    ...
    const locatorHints = copyStrings(input.locatorHints);
    const lifecycleAuthorityRefs = copyRefs(input.lifecycleAuthorityRefs);
    const parentRefs = copyRefs(input.parentRefs);
    const provenanceRefs = copyRefs(input.provenanceRefs);
    const evidenceRefs = copyRefs(input.evidenceRefs);
    const materialInputRefs = copyRefs(input.materialInputRefs);
 ...
 */
export function adoptDacV003RegistryReference(role, input) {
 ...
 */
export function adoptRuntimeHostBindingRequirementRef(input) {
    ...
    return adoptDacV003Reference('runtime-host-binding-requirement', input);
}
/**
 ...
 */
export function adoptRuntimeHostBindingRef(input) {
    // Role minimums (semanticIdentity) enforced by the core before minting.
    return adoptDacV003Reference('runtime-host-binding', input);
}
/**
...
function expectRole(role, value, description) {
    if (!roleGuard(role, value)) {
        ...
        throw new DacV003ReferenceError('ROLE_MISMATCH', `expected a ${description} reference, but received ${actual}; DAC v0.0.3 roles are never interchangeable`);
    }
}
export function isRuntimeHostBindingRequirementRef(v) {
    return roleGuard('runtime-host-binding-requirement', v);
}
export function isRuntimeHostBindingRef(v) {
    return roleGuard('runtime-host-binding', v);
}
export function isRuntimeInteractionContractRef(v) {
    return roleGuard('runtime-interaction-contract', v);
}
export function expectRuntimeHostBindingRequirementRef(v) {
    expectRole('runtime-host-binding-requirement', v, 'runtime-host-binding-requirement');
}
export function expectRuntimeHostBindingRef(v) {
    expectRole('runtime-host-binding', v, 'runtime-host-binding');
}
export function expectRuntimeInteractionContractRef(v) {
 ...
 */
export function verifyDacV003ReferenceIdentity(reference, expectation) {
    const mismatches = [];
    const fields = [
    ...
    if (mismatches.length > 0) {
        throw new DacV003ReferenceError('IDENTITY_MISMATCH', `exact identity mismatch on "${reference.role}" reference: ${mismatches.join('; ')}`);
    }
}
function slotValue(reference, slot) {
 ...
 */
export function assertDacV003ExactnessProfile(reference, profile) {
    const missing = [];
    const collect = (p) => {
        ...
        if (requirements.selectedDomainDataLifecycleAuthorities &&
            ...
            if (!hasPromotion)
                missing.push('P3:lifecycleAuthorityRefs:promotion-decision');
            if (!hasSelection)
                missing.push('P3:lifecycleAuthorityRefs:application-selection');
        }
    ...
    collect(profile);
    if (missing.length > 0) {
        throw new DacV003ReferenceError('PROFILE_REQUIREMENT_UNMET', `reference "${reference.role}" does not carry the required slots for exactness profile ${profile}: ${missing.join(', ')}`);
    }
}
/**
 ...
 */
export function assertDacV003RevisionDigestConsistency(references) {
    const seen = new Map();
    for (const reference of references) {
        ...
        if (prior === undefined) {
            ...
            throw new DacV003ReferenceError('REVISION_DIGEST_CONTRADICTION', `revision "${reference.revisionIdentity}" in scope "${reference.authorityScope}" carries contradictory authoritative digests "${prior.digest}" and "${reference.contentDigest}"; no identity-preserving reconciliation exists`);
        }
    }
}
/**
 ...
 */
export function refuteDacV003ExternalBusinessSoRIdentity(value) {
    if (structurallyValidAdoptedReference(value)) {
        throw new DacV003ReferenceError('EXTERNAL_IDENTITY_FORBIDDEN', `a "${value.role}" DAC v0.0.3 reference is Harness-side composition/Runtime identity and can never substitute external Business SoR identity`);
    }
}
/** Construct a namespaced reference/compatibility disposition value. */
export function dacV003ReferenceDisposition(value) {
    if (DAC_V003_REFERENCE_DISPOSITIONS.indexOf(value) === -1) {
        throw new DacV003ReferenceError('INVALID_DISPOSITION', `"${String(value)}" is not a dac-v003 reference/compatibility disposition; External Operation Outcomes and Authoring/Capability Exchange Outcomes are separate namespaces and must never be conflated`);
    }
 ...
 */
export function isDacV003ReferenceDisposition(value) {
    if (value === null || typeof value !== 'object')
        return false;
    const candidate = value;
    return (candidate.namespace === DAC_V003_DISPOSITION_NAMESPACE &&
        typeof candidate.value === 'string' &&
        DAC_V003_REFERENCE_DISPOSITIONS.indexOf(candidate.value) !== -1);
}
/**
 ...
 * producible.
 */
export function classifyDacV003RequiredTargetState(state) {
